...
//上传mp4和jpg到cutv
public class UploadMp4Agent implements Runnable{
	...
	public void run_() 
	{
		...
		FTPClient ftp=new FTPClient();
			...
			String sql = "select * from channel_chaitiao_f4v where Status=1 and Upload=0 and ModifiedDate>UNIX_TIMESTAMP()-72000 order by id desc limit 10";//20小时
			ResultSet rs = tu.executeQuery(sql);
			while(rs.next())
			{
				paths.add(new String[]{tu.convertNull(rs.getString("Title")),rs.getInt("id")+""});
			}
			tu.closeRs(rs);
		...
		{
			String[] o = (String[])paths.get(i);
			String path_ = o[0];
			String itemid = o[1];
			//System.out.println("UploadMp4Agent paths size:"+paths.size()+","+i+","+path_);
			boolean upload = false;
    	if(path_.contains("19uK7fc") || path_.contains("r55uf2J") || path_.contains("123bSdn") || path_.contains("15WDQ9o"))
    	{
			String path_mp4 = path_.replace(".f4v", ".mp4");
			...
			try {
				...
				ftp_target = "/"+ftp_target.replace("19uK7fc","82j7Cab").replace("r55uf2J","3f3z39a").replace("123bSdn", "14lCwST").replace("15WDQ9o", "2M7G4kd");
				//System.out.println("上传的文件为："+files2[1].getPath());
				
				System.out.println("ftp upload:"+path_mp4+","+ftp_target);
				boolean upload1 = FileUtil.FtpCopy(ftp,path_mp4, ftp_target);//第二个参数是要上传的文件的路径，第三个参数是上传后的路径，都带文件名
								
				//传图片
				String photo1 = rootfolder + "screenshot/" + path_.replace(rootfolder, "");
				photo1 = photo1.replace(".f4v", ".jpg");
				//System.out.println("photo1:"+photo1);
				String photo2 = photo1.replace(rootfolder,"");
				photo2 ="/"+ photo2.replace("19uK7fc","82j7Cab").replace("r55uf2J","3f3z39a").replace("123bSdn", "14lCwST").replace("15WDQ9o", "2M7G4kd");
				photo2=Util.ClearPath(photo2);
				ftp.changeWorkingDirectory("/");
				
				if(!new File(photo1).exists())
				{
					ChaiAgentPhoto.Capture2(path_);
				}
				
				boolean upload2 = FileUtil.FtpCopy(ftp, photo1, photo2);
				//System.out.println("ftp upload2:"+photo1+","+photo2);
				
				long endFtp = System.currentTimeMillis();

				if(upload1&&upload2) upload = true;
					
				System.out.println("上传时间是:"+(endFtp-startFtp)+","+upload1+","+upload2); 
			} catch (SocketException e) {
				System.out.println("upload error:"+path_);
				Log.SystemLog("拆条", "上传出现问题："+path_);
    		...
    		{
    			try {
					new TableUtil().executeUpdate("update channel_chaitiao_f4v set Upload=1 where id="+itemid);
	...
	public void run_0902() 
	{
    	if(path.contains("19uK7fc") || path.contains("r55uf2J"))
    	{
			...
			try {
				...
				String mp42 = mp4_prev_upload.replace("_.mp4", ".mp4");
				String ftp_target2 = mp42.replace(rootfolder,"");
				ftp_target2 = "/"+ftp_target2.replace("19uK7fc","82j7Cab").replace("r55uf2J","3f3z39a");
				System.out.println("ftp upload2:"+mp42+","+ftp_target2);
				FileUtil.FtpCopy(ftp,mp42, ftp_target2);//第二个参数是要上传的文件的路径，第三个参数是上传后的路径，都带文件名
				...
				if(!new File(photo1).exists())
				{
					ChaiAgentPhoto.Capture2(path);
				}
				
				FileUtil.FtpCopy(ftp, photo1, photo2);
				System.out.println("ftp upload3:"+photo1+","+photo2);
				ftp.disconnect();
				long endFtp = System.currentTimeMillis();
