...
public class TidePublishFile implements Runnable{
    ...
    public PublishItem getItem() {
        return item;
    }

    public void setItem(PublishItem item) {
        this.item = item;
    }
    ...
    public boolean copyFile() throws MessageException, SQLException {
        System.out.println("copy file begin...." + item.getId());
        ...
        long ftp_time = 0;
        String ps_name = "";
        System.out.println("list 100:" + (System.currentTimeMillis() - begin_) + "," + item.getId());
        ...
        if (ps.getCopyMode() == 1) {
            ...
            System.out.println("copy mode," + item.getId());
        ...
        if (Copy_Success) {
            ...
            if (message.startsWith("ftp error：")) {
                ...
                if (errornumber < 5) {
                    ...
                    RedisUtil.getInstance().putMessage(getItem(), RedisUtil.PUBLISH_MESSAGE_KEY);//将当前文件发布任务  重新放置在待发布队列
                    tu.executeUpdate(Sql);
                } else {
