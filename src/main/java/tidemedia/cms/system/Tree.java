...
public class Tree extends Table{
	...
	public Tree() throws MessageException, SQLException {
		super();
	}
	...
	public static int getChannelNumber()
	{
		if(!inited)
		{
			...
			try {
				...
				if (Rs.next()) {
					Tree.channel_number = Rs.getInt(1);
					Tree.inited = true;
				}
				tu.closeRs(Rs);						
	...
	public String listTreeByChannelRecommend(UserInfo user,int channelID) throws SQLException,MessageException 
	{
		...
		for (int i = 0; i < options.length; i++) {
			...
			JS += listChannel_JS(channelid, varName, user,Tree.getChannelNumber());
		}
	...
	public String listChannel_Publish_Files_JS(UserInfo user)
		...
		while (Rs.next()) {
			...
			if (type == 0)
				...
				// new Channel(new Channel(channelid).getParentTableChannel());
...
//		//当前登录用户绑定了租户,此频道下面是租户频道,不是公共频道，不是视频库下的pgc,app,用戶，互動
//		if(user.getCompany()!=0&&(!a.equals(""))&&(!a.equals("publishsite"))&&(!a.equals("pgc"))&&(!a.equals("app"))&&(!a.equals("user"))&&(!a.equals("interaction"))){
//			//获取符合条件的租户频道,替换ch对象，后面逻辑不变
//			id = getChannelID(id,user);
//			ch = CmsCache.getChannel(id);
//			
...
//				if(user.getCompany()!=0&&(!ch.getApplication().equals(""))){
//					channelid = getChannelID(channelid,user);
...
//				}
	...
	//获取租户频道
	public int getChannelID(int parent,UserInfo user) throws MessageException, SQLException{
