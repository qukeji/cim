package tidemedia.tcenter.service.publicity;
...
public class PublicityService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PublicityService.class);

    public  ModelMap getListModelMap(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap, List<String> additionaCustomList) throws Exception {
        // 定制化字段列表
...
//            response.sendRedirect("../approve/content_approve.jsp?id="+id);
            response.sendRedirect("../custom/issuedManager_approvePreview?id="+id);
            return null;
        }
//        else {
//            //
...
//        logger.info("cols 列为: " + cols);
     ...
     * @throws Exception
     */
    public String getListByByCustom(List<String> additionaCustomList) throws Exception{
        String customs = "";
        if (additionaCustomList != null && additionaCustomList.size() > 0) {
    ...
    public JSONArray getPublicityTree(int channelid, UserInfo userInfo) throws MessageException, SQLException, JSONException {
        Tree2019 tree2019=new Tree2019();
        JSONArray jsonArray=tree2019.listChannel_json(channelid,"",userInfo,100);
        logger.info("获取列表页json"+jsonArray);
        return jsonArray;
    }
}
