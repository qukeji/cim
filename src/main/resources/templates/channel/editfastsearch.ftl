		...
		<div class="modal-body pd-20 overflow-y-auto">
			<div class="config-box">
				<ul>
					...
					<li class="block">
						<div class="row">
							<label class="left-fn-title">检索项目名称：</label>
							<label class="wd-500">
						...
						<div class="row">
							...
							<label class="wd-500">
								<input class="form-control" placeholder="" type="text" id="wheresql" >
							</label>
						</div>
						<div class="row mg-t--10">
							<label class="left-fn-title"> </label>
							<label class="d-flex align-items-center tx-gray-800 tx-12">
								<i class="icon ion-information-circled tx-16 tx-gray-900 mg-r-5"></i>示例，已发稿件：Status=1
							</label>
						</div>
						<!--列表项目脚本-->
						<div class="row">
							<label class="left-fn-title">JS脚本：</label>
	...
	<script>
		...
		;$(function () {
			...
			if(itemid && isadd == 0 ){
				...
				$.ajax({   
					type: "GET",
					url: "${request.contextPath}/channel/list/fastsearch/get",
					data: {"id":itemid},
					success: function(d){
						console.log(d)
						if(d.code==200){
							$("#itemname").val(d.data.title);
							$("#wheresql").val(d.data.wheresql);
		...
		function confirmItem(){
			...
			if( !title ){
				dialog.showAlert("请输入快捷检索项目名称!","danger") ;
				return false ;
			}
			
			var wheresql = $("#wheresql").val(); 
//			if( $.trim(wheresql) == "" ){
//				dialog.showAlert("请填写条件!","danger") ;
//				return false ;
//			}
			var fieldValue = $("#fieldName").attr("data-value");
			var script = $("#itemScript").val() ;
			
			if(!script && !wheresql){
				dialog.showAlert("条件和JS脚本至少填写一项!","danger") ;
				return false ;
			}
			...
			if( isadd == 1 ){  //新增项目
				...
				$.ajax({
					type: "post",
					url: "${request.contextPath}/channel/list/fastsearch/add",
					data: {
						"title" : title  , "channel" : channelid , "wheresql" : wheresql , "script" : script , "status" : 0
					},
					success: function(d){
						console.log(d)
						if(d.code==200){
							dialog.showAlert("新增成功！") ;
							top.TideDialogClose({suffix:'_2',recall:true,returnValue:{status:2}});			
						}else{
				...
				$.ajax({
					type: "post",
					url: "${request.contextPath}/channel/list/fastsearch/update",
					data: {
						"id" : itemid , "title" : title , "wheresql" : wheresql , "script" : script
					},
					success: function(d){
						console.log(d)
						if( d.code==200 ){
		...
		function deleteItem(){
			var c = confirm("确定要删除本项目吗？")
  			if ( c == true ){
  				$.ajax({
					type: "post",
					url: "${request.contextPath}/channel/list/fastsearch/delete",
					data: {
						"id" : itemid 
					},
					success: function(d){
		...
		$("#chooseField").click(function(){
			var url = "${request.contextPath}/html/listconfig/list-relatedfield.ftl?channelid="+channelid;
		...
		function setReturnValue(obj){
			...
			$("#fieldName").attr("data-value" , $.trim( obj.filedValue ) ) ;
		}
	</script>
</body>
