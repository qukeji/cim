...
<%
    ...
    try{
        ...
        int channelid_review = jurong.getInt("review");//审核频道编号
        int webchannelId = jurong.getInt("webCollect");
        int weixinchannelId = jurong.getInt("weixinCollect");
        int weibochannelId = jurong.getInt("weiboCollect");

        Channel channel_hot = CmsCache.getChannel(channelId_hot);
        Channel channel_web = CmsCache.getChannel(webchannelId);
        Channel channel_weixin = CmsCache.getChannel(weixinchannelId);
        Channel channel_weibo = CmsCache.getChannel(weibochannelId);

        TableUtil tu2 = new TableUtil();
        String countSql_hot2 = "select count(*) from "+channel_hot.getTableName()+" where Active=1";
        String countSql_web = "select count(*) from "+channel_web.getTableName()+" where Active=1";
        String countSql_weixin = "select count(*) from "+channel_weixin.getTableName()+" where Active=1";
        String countSql_weibo = "select count(*) from "+channel_weibo.getTableName()+" where Active=1";
        ...
        String countSql = "";
        //查询线索汇聚总数量
        int count = getcount(countSql_web)+getcount(countSql_weixin)+getcount(countSql_weibo);
        ...
        Channel channel_review=CmsCache.getChannel(14271);
...
<%!
    ...
    public int getcount(String sql) throws MessageException, SQLException
    {
        int count = 0 ;
        TableUtil tu = new TableUtil();
        ResultSet rs = tu.executeQuery(sql);
