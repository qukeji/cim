...
public static JSONObject getPgcChannel(int company_id,int columnid,String columnname) throws MessageException, SQLException, JSONException {
	...
	String channelSql = "select * from channel where application = 'pgc_doc'";
...
//判断聚现资源是否存在
public JSONObject add(int sourcechannelid,int juxian_sourceid,int company_id,String modify_date,HashMap map) throws MessageException, SQLException, JSONException {
	JSONObject reJson = new JSONObject();
	...
	String sql_exist = "select * from "+CmsCache.getChannel(sourcechannelid).getTableName()
			...
			+" and juxian_sourceid="+juxian_sourceid;
...
public JSONObject enteringDate(JSONObject data) throws SQLException, MessageException, JSONException {
	...
	String Summary = data.getString("desc");//摘要
	String tag = data.getString("tag");//标签
	...
	if(!Content.equals("")){
		Content = fun_content(Content,tableName,channelId,zuhu_id);
	}
	...
	map.put("Photo",Photo);
	map.put("juxian_sourceid",juxian_sourceid+"");
	...
	if(PgcChannel_id==0){
		...
		reJson = add(PgcChannel_id,juxian_sourceid,juxian_companyid,modify_date,map);
	}
	...
	if(CurrentSourceChannel.equals("")){
		...
		for(int j=0;j<CurrentSourceChannels.length;j++){
			...
			map.put("doc_type","0");
			map.put("ModifiedDate", dateStr);
			reJson = add(CurrentSourceChannel_id,juxian_sourceid,juxian_companyid,modify_date,map);
		}
	}
