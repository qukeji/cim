...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,packagestatus,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="androidPack.jsp?id=<%=ChannelID%>" class="btn btn-outline-info">新建</a>
				<%if(canApprove){%>
				...
				<%}%>
					...
					<a href="javascript:deleteFile2();" class="btn btn-outline-info">打包</a>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">打包状态</th>
...
while(Rs.next())
{
	...
	int packagestatus = Rs.getInt("packagestatus");
	String Statuspack=""; 
	if(packagestatus==0){
		Statuspack = "<span class='tx-orange'>未打包</span>";
	}else if(packagestatus==1){
		Statuspack = "<span class='tx-success'>打包中</span>";
	}else if(packagestatus==2){
		Statuspack = "<span class='tx-success'>打包成功</span>";
	}else if(packagestatus==3){
		Statuspack = "<span class='tx-danger'>打包失败</span>";
	}
	int TopStatus = 0;
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				<%if(packagestatus==2){%>
    			    <a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="下载"><i class="icon fa fa-download tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
				<%if(packagestatus==0){%>
				    <a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="打包"><i class="icon fa fa-download tx-18 " aria-hidden="true"></i></a>
				<%}%>
				    <a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="复制"><i class="icon fa fa-clone tx-18 handle-icon" aria-hidden="true"></i></a>
				    <a href="javascript:deleteFile1(<%=id_%>);" class="btn pd-0 mg-r-5" title="删除"><i class="icon fa fa-trash-o tx-18 handle-icon" aria-hidden="true"></i></a>
