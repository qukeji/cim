<%@ page import="tidemedia.cms.system.*,java.util.Date,org.json.*,
				tidemedia.cms.user.*"%>
...
if(childGroup==1){
	dir="app";
	Title="APP管理";
	arr = ChannelUtil.getApplicationChannel("app",companyid,1,userinfo_session);
}else if(childGroup==2){
	dir="userManage";	
	Title="用户管理";
	arr = ChannelUtil.getApplicationChannel("user",companyid,1,userinfo_session);
}else if(childGroup==3){
	dir="standard";	
	Title="互动管理";
	arr = ChannelUtil.getApplicationChannel("interaction",companyid,1,userinfo_session);
}else if(childGroup==4){
	dir="app_setting";	
	Title="配置管理";
	arr = ChannelUtil.getApplicationChannel("app_config",companyid,1,userinfo_session);
}else if(childGroup==5){
	dir="app_auto_package";	
	Title="在线打包";
	arr = ChannelUtil.getApplicationChannel("app_package",companyid,1,userinfo_session);
}
...
<div class="br-logo">
    ...
    <div class="br-mainpanel with-second-nav br-mainpanel-file " id="js-source">
		<%
		if(arr.length()==0){%>
			<iframe src="error.jsp" id="content_frame" style="width: 100%;height: 100%;"  frameborder="0" onload="changeFrameHeight(this)"></iframe>
		<%}else{%>
...
//=============================================================================================================
			...
			for(var i=0;i<json.length;i++){
				...
				var html = '<li>';
				html += '<a href="javascript:;" channelid="'+json[i].channelid+'" load="1"><i class="fa fisrtNav fa-home" have="0"></i> <span>'+json[i].sitename+'</span></a>';
				html += '</li>';
				menu.append(html);
				activeFn();
			}
        ...
        function activeFn(){
        	var timer = setInterval(function(){
        		if($(".sidebar-menu li a:first")){
        			var activeChannel = $(".sidebar-menu li a:first");
					try {
...
//				$(this).addClass("clicked");//设置不可重复点击
			...
			if(load==1) 
			{
				...
				$.ajax({
					...
					if(!clickFirstChild){  //回显到第一个二级栏目
					    var activeChild = $(".sidebar-menu>li.active a:first").next(".treeview-menu").find("li a:first");
						activeChild.trigger("click");
					    //setTimeOut(activeChild.parent("li").addClass('active'),50)
						clickFirstChild = true ;
					}
				}});
			}
			else
