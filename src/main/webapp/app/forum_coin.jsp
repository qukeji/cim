...
String ListSql = "select id,Title,Weight,GlobalID,score,User,Photo,userid,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<%if(canApprove){%>
					<a href="javascript:approve();" class="btn btn-outline-info" hidden="hidden">生效</a>
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%if(canApprove){%>
						<a href="javascript:approve();" class="nav-link" hidden="hidden">生效</a>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-200 wd-author">用户</th>
							<th class="tx-12-force tx-mont tx-medium">动作</th>
							<th class="tx-12-force tx-mont tx-medium">分值</th>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">发生时间</th>
							<%if(IsComment==1){%>
			...
			<div class="row">
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  <p class="card-text"><%if(IsPhotoNews==1){%><i class="icon ion-image tx-22 tx-warning lh-0 valign-middle"></i><%}%><%=Title%></p>
					  <div class="row mg-l-0 mg-r-0 mg-t-5">				                  					                  
						  ...
						  <%if(active==1 && canApprove){%>
						  <a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="发布"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true" hidden="hidden"></i></a>
						  <%}%>
						  <%if(active==0 && userinfo_session.isAdministrator()){%>
						  <a href="javascript:resume(<%=id_%>);" class="btn pd-0 mg-r-5" title="恢复"><i class="fa fa-reply tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}else{%>
						  <a href="javascript:Preview2(<%=id_%>);" class="btn pd-0 mg-r-5" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true" hidden="hidden"></i></a>
						  <a href="javascript:Preview3(<%=id_%>);" class="btn pd-0 mg-r-5" title="正式地址预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true" hidden="hidden"></i></a>
						  <%}%>
					  </div>
					</div>
				  </div>
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%}%>
						<a href="javascript:editDocument1();" class="nav-link"hidden="hidden">编辑</a>
						<a href="javascript:deleteFile2();" class="nav-link"hidden="hidden" hidden="hidden">撤销</a>
						<%if(!channel.getRecommendOut().equals("")){%>
						<a href="javascript:recommendOut()" class="nav-link"hidden="hidden">推荐</a>
						<%}%>
						<%if(!channel.getAttribute1().equals("")){%>
