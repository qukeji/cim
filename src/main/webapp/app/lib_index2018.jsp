...
//=============================================================================================================
		...
		function get_menu_html(json,childGroup)
		{
			...
			if(json.child && json.child.length>0)
			{
				...
				{								
				    if(childNum==i){
					       html += '<li active><a href="#" load="'+json_[i].load+'" channelid="'+json_[i].id+'">';
						}else{
					...
					{ 
				        if(childNum==i){
							html += '<ul class="treeview-menu menu-open ">' + get_menu_html(json_[i]) + '</ul>';
						}else{
				...
				for(var i=0;i<json_.length;i++){
					...
					if(json_[i].child && json_[i].child.length>0) html += '<ul class="treeview-menu">' + get_menu_html(json_[i]) + '</ul>';
