...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(CreateDate) as CreateDate,Status,Active,Category,IsPhotoNews,username,Content,url,user_ip";
...
//按用户名搜索
if(!username.equals("")){
	String tempUsername=S_Title.replaceAll("%","\\\\%");
	WhereSql += " and username like '%" + channel.SQLQuote(username) + "%'";
}
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="javascript:addDocument();" class="btn btn-outline-info" style="display:none;">新建</a>
				<%if(canApprove){%>
					<a href="javascript:approve();" class="btn btn-outline-info">生效</a>
					<a href="javascript:Preview();" class="btn btn-outline-info">预览</a>
				<%}else{%>
					<a href="javascript:upOpenUrl();" class="btn btn-outline-info">预览</a>
				<%}%>
					<a href="javascript:editDocument1();" class="btn btn-outline-info">编辑</a>
					<a href="javascript:deleteFile2();" class="btn btn-outline-info" style="display:none;">撤稿</a>
			...
			<div class="dropdown hidden-md-1500-up hidden-sm-down mg-l-10">
				<a href="#" class="btn btn-outline-info" data-toggle="dropdown" style="display:none;">操作 <i class="fa fa-angle-down mg-l-5"></i></a>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">评论内容</th>
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-40">权重</th>
							<%}%>
							
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-60">评论时间</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80 wd-author">用户</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">用户IP</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	String usernamers	= convertNull(Rs.getString("username"));
	String url	= convertNull(Rs.getString("url"));
	String Content	= convertNull(Rs.getString("Content"));
	String user_ip	= convertNull(Rs.getString("user_ip"));
	...
	String 	CreateDate2=Util.FormatDate("yyyy-MM-dd HH:mm",CreateDate);
	String CreateDate_MM=Util.FormatDate("MM-dd",CreateDate);
	String CreateDate_HH=Util.FormatDate("HH:mm",CreateDate);
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            ...
            <td class="hidden-xs-down line-limit-length">
				<%=Content%>
			</td>
			<%if(IsWeight==1){%>
			...
			<td class="hidden-xs-down">
				<div class="content-time" title="<%=CreateDate2%>">
					<span class="article-md"><%=CreateDate_MM%></span>
					<span class="article-hm"><%=CreateDate_HH%></span>
				</div>
			</td>
			<td class="hidden-xs-down">
				<%=usernamers%>
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}%>
				<%if(active==1){%>
					<input id="input-url" type="hidden" value="<%=url%>">
					<a href="javascript:openUrl('<%=url%>')" class="btn pd-0 mg-r-5" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
