...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&startDate="+S_startDate+"&endDate="+S_endDate+"&username="+username+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1;
...
String ListSql = "select id,Title,Weight,GlobalID,User,userid,Content,txtContent,Photo,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,username";
...
if(!username.equals(""))
{
    String tempUsername=username.replaceAll("%","\\\\%");
	WhereSql += " and username like '%" + channel.SQLQuote(tempUsername) + "%'";
}
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%if(canApprove){%>
						<a href="javascript:approve();" class="nav-link" >生效</a>
						...
						<a href="javascript:deleteFile2();" class="nav-link" hidden="hidden">撤销</a>
						<%if(!channel.getRecommendOut().equals("")){%>
					...
					<div class="row">
						...
						<div class="mg-r-10 mg-b-30 search-item">
							<div class="input-group">
								...
								<input type="text" class="form-control search-author" placeholder="用户名" name="username" value="<%=username%>">
							</div>
						</div>
						<!--状态-->
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">帖子标题</th>
						
						
							<th class="tx-12-force tx-mont tx-medium  hidden-xs-down ">内容</th>
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-40">权重</th>
							<%}%>
							
						
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-60">发布时间</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">评论用户</th>
							...
							<%}%>
							<th class="tx-12-force wd-100 tx-mont tx-medium hidden-xs-down wd-150">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
while(Rs.next())
{
	...
	String usernamewl	= convertNull(Rs.getString("username"));
	int IsPhotoNews = Rs.getInt("IsPhotoNews");
	String Content	= convertNull(Rs.getString("txtContent"));
	...
	if(IsDelete!=1){
	if(Status==0)
		StatusDesc = "<span class='tx-orange'>已撤稿</span>";
	else if(Status==1)
		StatusDesc = "<span class='tx-success'>生效</span>";
	}else{
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%}else{%>
							...
							<a href="javascript:deleteFile3(<%=id_%>);" class="nav-link" hidden="hidden">撤销</a>
							<%if(!channel.getRecommendOut().equals("")){%>
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  ...
					  <div class="row mg-l-0 mg-r-0 mg-t-5">				                  					                  
						  ...
						  <a href="javascript:Preview3(<%=id_%>);" class="btn pd-0 mg-r-5" title="正式地址预览" hidden="hidden"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}%>
					  </div>
					</div>
				  </div>
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<a href="javascript:deleteFile2();" class="btn btn-outline-info" hidden="hidden">撤销</a>
				<%if(!channel.getRecommendOut().equals("")){%>
				<a href="javascript:recommendOut()" class="btn btn-outline-info" hidden="hidden">推荐</a>
				<%}%>
				<%if(!channel.getAttribute1().equals("")){%>
...
*/			
		...
		var menu = [
		<%if(canApprove){%>
			{'<i class="fa fa-search mg-r-5"></i>生效':function(menuItem,menu) {approve();} },
		<%}%>
