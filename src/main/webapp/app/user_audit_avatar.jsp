...
function auditavatar1(type,id,Status2){
	if(type == "agree"){
		message = "确定要通过这个用户头像审核吗？";
	}else if(type == "refuse"){
		message = "确定要驳回这个用户头像审核吗？";
	}
	if(Status2!=0){
...
function auditavatar(type){
	...
	if(obj.length==0){
		...
		if(type == "agree"){
			message = "确定要通过这"+obj.length+"个用户头像审核吗？";
		}else if(type == "refuse"){
			message = "确定要驳回这"+obj.length+"个用户头像审核吗？";
		}
	}
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as                                             ModifiedDate,Status,Active,Category,IsPhotoNews,Audit_avatar,Userid,IP,Status1,CreateDate";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">头像</th>
							<%if(IsWeight==1){%>
...
while(Rs.next())
{
	...
	String Audit_avatar	= convertNull(Rs.getString("Audit_avatar"));
	...
	{
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" Status2="<%=Status2%>"  OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<%}%>
			<%if(Audit_avatar==null||Audit_avatar.trim()==""){%>
			<td class="hidden-xs-down">
			<img src="http://jushi.tidemedia.com/images/user.png" width="60px" height=""/></td>
			<%}else{%>	
			<td class="hidden-xs-down">
			<img src="<%=Audit_avatar%>" width="60px" height=""/>
			</td>
			<%}%>
			<td class="hidden-xs-down">
				<%=StatusDesc%>
