	...
	function recommendOut1(id)
{
	...
	function auditavatar1(type,id,Status2){
		if(type == "agree"){
			message = "确定要通过这个用户昵称审核吗？";
		}else if(type == "refuse"){
			message = "确定要驳回这个用户昵称审核吗？";
		}
		if(Status2!=0){
	...
	function auditavatar(type){
		...
		if(obj.length==0){
			...
			if(type == "agree"){
				message = "确定要通过这"+obj.length+"个用户昵称审核吗？";
			}else if(type == "refuse"){
				message = "确定要驳回这"+obj.length+"个用户昵称审核吗？";
			}
		}
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,Audit_nickname,Userid,IP,Status1,CreateDate";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">昵称</th>
							<%if(IsWeight==1){%>
...
while(Rs.next())
{
	...
	String Audit_nickname	= convertNull(Rs.getString("Audit_nickname"));
