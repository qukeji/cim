...
String ListSql = "select id,Title,Photo,Weight,GlobalID,Content,becompanyid,bewatchuserid,siteflag,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">关注者</th>
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-40">权重</th>
							<%}%>
						
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">关注时间</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">被关注者</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	int becompanyid=Rs.getInt("becompanyid");
	int bewatchuserid=Rs.getInt("bewatchuserid");
	...
	String UserName	= CmsCache.getUser(bewatchuserid).getName();//convertNull(Rs.getString("userinfo.Name"));
	String StatusDesc = "";
	if(IsDelete!=1){
	if(Status==0)
    ...
    if(bewatchuserid==0||becompanyid!=0){
        ...
        ResultSet rs2 = tu2.executeQuery(sql2);
        if(rs2.next()){
           UserName=convertNull(rs2.getString("Title"));  
        }
