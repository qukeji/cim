...
String ListSql = "select id,Title,Photo,Weight,GlobalID,Content,Tel,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">验证码</th>
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-40">权重</th>
							<%}%>
						
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">发送时间</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">接收手机</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	String phone=convertNull(Rs.getString("Tel"));
	int TopStatus = 0;
