...
String ListSql = "select id,Title,Photo,Weight,GlobalID,Content,SubTitle,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">文章标题</th>
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-40">权重</th>
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">点赞日期</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">点赞用户</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	String SubTitle=convertNull(Rs.getString("SubTitle"));
	int TopStatus = 0;
