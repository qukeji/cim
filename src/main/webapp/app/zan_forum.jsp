...
String ListSql = "select id,Title,Weight,GlobalID,User,userid,Photo,targetuserid,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-200 wd-author">发帖用户</th>
							<th class="tx-12-force tx-mont tx-medium">帖子名称</th>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">点赞用户</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	String targetUserName	= CmsCache.getUser(targetuserid).getName();
	String StatusDesc = "";
	if(IsDelete!=1){
	if(Status==0)
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            ...
            <td >
              ...
              <%}%>
              <span class="pd-l-5 tx-black"><%=targetUserName%></span>
            </td>
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  ...
					  <div class="row mg-l-0 mg-r-0 mg-t-5">				                  					                  
						  ...
						  <%if(active==1 && canApprove){%>
						  <a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="生效" hidden="hidden"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}%>
						  <%if(active==0 && userinfo_session.isAdministrator()){%>
