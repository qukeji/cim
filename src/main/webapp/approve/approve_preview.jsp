...
/*if(id_aa!=0){//说明已提交审核
    ...
    ApproveItems approve_item2 = (ApproveItems) list.get(list.size()-1);
    int end_approveId=approve_item2.getId();
   String user= approve_item2.getUsers();//用户等于终审用户
%>
<html>
<head>
    <meta charset="utf-8">
    ...
    <script>
        ...
        var end_user="<%=user%>";//终审环节用户
        var end_approveId=<%=end_approveId%>;//终审环节id
        ...
        function Save(action){
            ...
            var url= "../approve/approve_sumbit.jsp?globalid="+GlobalID+"&ItemID=<%=ItemID%>"+"&ChannelID=<%=ChannelID%>"+"&action="+action+"&user="+user+"&approveId="+approveId+"&endApprove="+endApprove+"&actionMessage="+actionMessage+"&ApproveActionId=<%=id_aa%>";
            $.ajax({
                type: "GET",
                url: url,
...
<div class="br-header">
    ...
    <div class="br-header-right">
        <div class="btn-box" >
            ...
            <button type="button" class="btn btn-secondary tx-size-xs mg-r-5" onclick="Preview2()">本地预览</button>
            <%}%>
            <%if(end!=1){%>
            <button type="button" class="btn btn-secondary tx-size-xs mg-r-5" onclick="Save(0);">通过</button>
            <%}%>
			<%if(!(end==1&&action==1)){//不是终审驳回%>
