...
if(pindex!=-1)
	...
	boolean canList = channel.hasRight(userinfo_session,ChannelPrivilegeItem.CanList);//浏览权限
	boolean onlyMe = channel.hasRight(userinfo_session,ChannelPrivilegeItem.onlyMe);//只看自己
...
<style>
	...
	table .sh-td{}		
	table .pop-start{position: relative;}
	table .pop-box{position: absolute;left: -180px;top: 45px;display: none;z-index: 999;}
	table .pop-content{width: 400px;height: 290px;overflow: hidden;border-radius: 5px;
	background-color: #fff;box-shadow: 0px 0px 5px 1px #aaa;cursor: default;padding: 0 0 20px 0;}
	table .pop-title{margin: 0;padding: 0 0 0 20px;background: #f8f9fa;line-height: 40px;border-bottom: 1px solid #dbdbdb;	}
	table .pop-triangle{position: absolute;	right: 180px;top: -10px;	width: 0; height: 0;border-left: 10px solid transparent;border-right: 10px solid transparent;border-bottom: 10px solid #e9e9e9;}
    table .pop-text{padding:10px 20px;overflow-y:auto;height:240px;line-height:24px;}
    table  .pop-text em{font-style: normal;}
</style>
...
<script>
	...
	function approve_preview(id,actionId)
	{
		...
		window.open("../approve/approve_preview.jsp?ItemID=" + id + Parameter);
	}
</script>
</head>
...
//添加判断有内容类型的频道
String ListSql = "select id,Title,doc_type,item_type,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,ApproveStatus";
...
if(S_IsIncludeSubChannel==1)
{
	...
	if((userinfo_session.getRole()==1||canList)&&S_UserID>0){
		WhereSql += " and User="+S_UserID;
	}
	if(!canList&&onlyMe){//只选了'只看自己'
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<a href="javascript:Preview_();" class="nav-link">预览</a>
						<%}else{%>
						<a href="javascript:Preview_();" class="nav-link">预览</a>
						<%}%>
			...
			<div class="search-content bg-white">
				<form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
					<div class="row">
						...
						<!-- wd-200 -->
						<!--作者-->
						<%if(userinfo_session.getRole()==1||canList){%>
						<div class="mg-r-10 mg-b-30 search-item">
							<div class="input-group">
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">审核状态</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">稿件状态</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-60">日期</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80 wd-author">作者</th>
							<%if(IsComment==1){%>
							...
							<%}%>
							<th class="tx-12-force wd-130 tx-mont tx-medium">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
while(Rs.next())
{
	...
	String 	ModifiedDate2=Util.FormatDate("yyyy-MM-dd HH:mm",ModifiedDate);
	String ModifiedDate_MM=Util.FormatDate("MM-dd",ModifiedDate);
	String ModifiedDate_HH=Util.FormatDate("HH:mm",ModifiedDate);
	...
	{
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item" end="<%=end%>"  approve="1"  >
            <td class="valign-middle">
              <label class="ckbox mg-b-0">
				<input type="checkbox" name="id" ApproveStatus="<%=ApproveStatus%>" value="<%=id_%>"><span></span>
			  </label>
            </td>
            <td ondragstart="OnDragStart(event)">
              ...
              <%}else{%>
			  <i class="icon ion-clipboard drag-list tx-22 tx-warning lh-0 valign-middle" id="img_<%=j%>"></i>
              <%}%>
              <%if(TopStatus==0){%><%}else{%>
			...
			<td class="hidden-xs-down">
				<div class="content-time" title="<%=ModifiedDate2%>">
					<span class="article-md"><%=ModifiedDate_MM%></span>
					<span class="article-hm"><%=ModifiedDate_HH%></span>
				</div>
			...
			<td class="dropdown hidden-xs-down">
    			...
    			<a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-2" title="发布"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
				<%if(active==1){%>
					<%if(channel.getChannelTemplates(2).size()!=0){%>
					<a href="javascript:Preview2_(<%=id_%>);" class="btn pd-0 mg-r-2" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
					<%}%>
					<a href="javascript:approve_preview(<%=id_%>,<%=ApproveStatus%>);" class="btn pd-0 mg-r-2" title="审核预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
				<a href="#" data-toggle="dropdown" class="btn pd-y-3 tx-gray-500 hover-info handle-three"><i class="icon ion-more"></i></a>
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						<%if(active==0 && userinfo_session.isAdministrator()){%>
						<a href="javascript:resume(<%=id_%>);" class="nav-link">恢复</a>
						<%}else{%>
							<a href="javascript:Preview3_(<%=id_%>);" class="nav-link">正式地址预览</a>
							<%if(ApproveStatus==0){%>
	...
	if(listType==2)
	{
		...
		m++;
%>
		<td id="item_<%=id_%>" status="<%=Status%>" class="tide_item" class="c" end="<%=end%>"  approve="1">
			<div class="row">
				<div class="col-md">
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  ...
					  <div class="row mg-l-0 mg-r-0 mg-t-5">				                  					                  
						  ...
						  <%}%>
						  <%if(active==0 && userinfo_session.isAdministrator()){%>
						  <a href="javascript:resume(<%=id_%>);" class="btn pd-0 mg-r-2" title="恢复"><i class="fa fa-reply tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}else{%>
						  <a href="javascript:Preview2_(<%=id_%>);" class="btn pd-0 mg-r-2" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
						  <a href="javascript:Preview3_(<%=id_%>);" class="btn pd-0 mg-r-2" title="正式地址预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}%>
					  </div>
					</div>
				  </div>
...
<script>
	...
	function double_click()
	{
		jQuery("#content-table .tide_item").dblclick(function(){
			...
			obj.trigger("click");
			var ApproveStatus = obj.attr("ApproveStatus");
			if(ApproveStatus==0){
				var  dialog = new top.TideDialog();
				dialog.showAlert("请先提交审核！","danger")
				return;
			}
			var id = obj.val();
			var itemId = $("#item_"+id).attr("itemid");
			var actionid = $("#item_"+id).attr("actionid");
			approve_preview(itemId,actionid);
		});
	}
	...
	$(function(){    		
		...
		<%}%>
			{'<i class="fa fa fa-eye mg-r-5"></i>查看':function(menuItem,menu) {view(); }},
        ...
        function updateManagerDate(id){
			$.ajax({
				type: "get",
				url: "approve_result.jsp?globalid="+id,
