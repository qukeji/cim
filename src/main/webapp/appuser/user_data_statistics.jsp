...
<%!
    ...
    public  int gettotal1(int id) throws MessageException, SQLException{
        ...
        Channel channel = CmsCache.getChannel(id);
        String whereSql = " where Active=1";
        String sql = "select count(*) as total  from " + channel.getTableName() +" where from_unixtime(CreateDate,'%Y-%m-%d %H-%m-%s') >= DATE_ADD(NOW(),INTERVAL -7 day) ";
...
<%
    ...
    TideJson user = CmsCache.getParameter("user").getJson();
    int channelid = user.getInt("userid");
