...
<%
	...
	if(f_content!=null) has_content = true;
%>
<!DOCTYPE html>
<html>
<head>
...
<link rel="stylesheet" href="../style/fontnew/iconfont.css">
...
<!--<script type="text/javascript" charset="utf-8" src="../static/js/editor/weixin_editor.js"></script>-->
<%}%>
<script>
...
<form name="form" action="../content/document_submit.jsp" method="post" id = "form">
 ...
 <div class="br-header" id="doc_header">
	 ...
	 <div class="br-sideleft overflow-y-auto" id="togglenav">
	...
	<div class="br-header-right">
		<div class="btn-box" >
			...
			<a class="btn btn-primary tx-size-xs mg-r-5" href="javascript:Save();" id="Image1Href" style="display: flex;align-items: center;"><i class="icon iconfont icon-baocun" style="color:#fff"></i><span class="let-box">保存</span></a>
			<%if(ApproveScheme!=0){%>
				<a class="btn btn-secondary tx-size-xs mg-r-5" id="Image2Href" href="javascript:Save_approve();">提交审核</a>
			<%}else{
				if(cp.hasRight(userinfo_session,ChannelID,3)){%>
					<a class="btn btn-secondary tx-size-xs mg-r-5" id="Image2Href" href="javascript:Save_Publish();" style="display: flex;align-items: center;"><i class="icon iconfont icon-fasong" style="color:#fff"></i><span class="let-box">保存并发布</span></a>
			<%}}}else{%>
				...
				<%}%>
			<%}%>
			<a class="btn btn-secondary tx-size-xs mg-r-10" href="javascript:self.close();window.opener.document.location.reload();" style="display: flex;align-items: center;"><i class="icon iconfont icon-guanbi" style="color:#fff;font-size:17px;"></i><span class="let-box">关闭</span></a>
		</div>
	</div><!-- br-header-right -->
</div><!-- br-header -->
...
//	}
	...
	//has_content 是否有编辑器
%>
	<div class="br-mainpanel br-mainpanel-file overflow-hidden" id="form<%=(j+1)%>" <%=form_other%>
	<%if(j==0){%>
		<div class="row flex-row align-items-center mg-b-15" id="main_left">
		</div>
		<%if(has_content){%>
	<div class="row flex-row align-items-center mg-b-15 edit" id="main_content"   >
		...
		<%}%>
	<%}%>
	<div id="main_right" class="br-pagebody bg-white mg-l-20 mg-r-20 <%=padding%>" url="<%=url%>">
...
%>
	<div class="br-mainpanel br-mainpanel-file overflow-hidden" <%if(j!=0){%>style="display:none;"<%}%> url="" id="form<%=vercount1%>">
		<iframe id="iframe<%=vercount1%>" class="content-edit-frame" style="width:100%;min-height:960px;height:100%;" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto"
...
%>
	<div class="br-mainpanel br-mainpanel-file overflow-hidden"   <%if(j!=0){%>style="display:none;"<%}%> url="" id="form<%=vercount1%>">
		<iframe id="iframe<%=vercount1%>" name ="oneDocManySend" class="content-edit-frame" style="width:100%;min-height:600px;" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto"
...
<script src="/tcenter/common/2018/Tdrag.js"></script>
...
$("#tr_changeEditor input[name='editortype']").click(function(){
	...
	if(val==0){
		...
		if(!everLoadH5){
			var _html = '<iframe id="h5editor1_Frame" src="../h5/index.html?webname=${pageContext.request.contextPath}" width="100%" height="800" frameborder="0" scrolling="no" onload="inserBdContent()" class="fulllast"></iframe>' ;
			$("#tr_Content").hide();
			$("#tr_h5Editor").html(_html).show();
		   //$("#h5edit").html(_html).show();
			everLoadH5 = true ;
		}else{
			$("#tr_h5Editor").show();
			$("#tr_Content").hide();
