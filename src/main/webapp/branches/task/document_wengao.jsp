...
<%
    ...
    int task_channelid = channel_review.getId() ;//选题频道编号
	...
	if(item!=null)
	{
		...
		title = item.getTitle();
		topicid = item.getIntValue("topicid");
	}
...
<script>
$(function(){
	$("#field_topicname").after("\r\n<label><input type=\"button\" value=\"选择\" onclick=\"getTopicListHref()\" class=\"btn btn-primary tx-size-xs mg-l-10\"></label>");

});


function getTopicListHref(){
var content_ =$("input[name='topicid']").val();
var users_id =$("input[name='users_id']").val();
	var	dialog = new top.TideDialog();
		dialog.setWidth(600);
		dialog.setHeight(500);
		dialog.setUrl("../task/task_subject_select.jsp?id=<%=task_channelid%>&fieldName=topicname&fieldidName=topicid&user_id="+users_id+"&content="+content_);
