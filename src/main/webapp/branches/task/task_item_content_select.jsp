...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&startDate="+S_startDate+"&endDate="+S_endDate+"&User="+S_User+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1+"&topicid="+topicid;
...
<script>
	...
	function gopage(currpage) {
		var url = pageName + "?currPage=" + currpage + "&topicid=<%=topicid%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>";
		this.location = url;
	}
...
function addconfirm(){
	...
	if(obj.length==0){
		TideAlert("提示","请先选择要添加的文件！");
	}else{
	    if(confirm(message)){
			addconfirm_confirm(obj.id,obj.length);
		}
	}
	
}
function addconfirm_confirm(id,length){
	
	var url="../task/task_item_content_change.jsp?check=2&topicid=<%=topicid%>&ItemID=" + id + Parameter;
	$.ajax({
		type: "GET",
		url: url,
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="javascript:addconfirm();" class="btn btn-outline-info">确定添加</a>
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						<a href="javascript:addconfirm();" class="nav-link">确定添加</a>
		...
		<div class="search-box pd-x-20 pd-sm-x-30" >
			<div class="search-content bg-white">
				<form name="search_form" action="<%=pageName%>?id=<%=id%>&topicid=<%=topicid%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
					<div class="row">
						<!--标题-->
