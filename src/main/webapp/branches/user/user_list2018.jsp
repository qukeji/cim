...
function gopage(currpage) {
			var url = "user_list2018.jsp?GroupID=<%=GroupID%>&currPage=" + currpage + "&rowsPerPage=<%=rowsPerPage%><%=querystring%>";
			this.location = url;
		}
</script>
</head>
          ...
          selectOtherMonths: true
			...
			jQuery(document).ready(function() {
				...
				jQuery("#goToId").click(function() {
					...
					if (num < 1)
						num = 1;
					var href = "user_list2018.jsp?GroupID=<%=GroupID%>&currPage=" + num + "&rowsPerPage=<%=rowsPerPage%><%=querystring%>";
					document.location.href = href;
				});
			});
			
			function change(obj) {
				if (obj != null) this.location = "user_list2018.jsp?GroupID=<%=GroupID%>&rowsPerPage=" + obj.value + "<%=querystring%>";
			}
    </script>
</div>
</body>
