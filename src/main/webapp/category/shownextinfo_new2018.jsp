...
if(haschild!=0 || level_==0)
{
	...
	String sql = "select * from "+channel_name+"  where Status=1 and Parent="+itemid_+" order by id";
	...
	while(rs.next())
	{
		...
		if(hasN == 0)
		{                                                                                   
			str +="<li class=\"no_arrow\" hasNext=\""+hasN+"\" itemid=\""+itemid+"\" level=\""+Level+"\" globalid=\""+gid+"\"><a href=\"#\">"+Title+"</a><i class=\"fa fa-caret-right\"></i></li>";
		}
		else
		{
			str +="<li class=\"\" hasNext=\""+hasN+"\" itemid=\""+itemid+"\" level=\""+Level+"\" globalid=\""+gid+"\"><a href=\"#\">"+Title+"</a><i class=\"fa fa-caret-right\"></i></li>";
		}
	}
	str += "</ul></div>";
	out.println(str);
