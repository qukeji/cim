<%@ page import="tidemedia.cms.system.*,java.sql.*"%>
...
function defineForm()
{
	...
	myObject.ChannelName = "<%=channel.getName()%>";
 	...
 	var Feature = "dialogWidth:42em; dialogHeight:30em;center:yes;status:no;help:no";
	var retu = window.showModalDialog
	("../modal_dialog.jsp?target=channel/form_view.jsp&Scrolling=auto&ChannelID=" + myObject.ChannelID,myObject,Feature);
	if(retu!=null)
		window.location.reload();
}
...
function Publish()
{
	...
	var url1 = "../modal_dialog.jsp?target=channel/publish_app.jsp&ChannelID="+myObject.ChannelID;
	var url2 = "publish_app.jsp?ChannelID="+myObject.ChannelID;
	...
	<tr>
		...
		<table width="100%" height="38" border="0" cellpadding="0"
			...
			<tr>
				...
				<a href="javascript:deleteChannel();">
					<img src="../images/icon_deletechannel_s.gif" width="32" height="32" align="absmiddle" border="0"> 
						删除应用 
				</a>
				<%}%>
				<%if (channel.getType()==3){%>  
				...
				<%}%> 
				<%if (channel.isTableChannel()||channel.getType()==3) {%>
		...
		<table width="98%" border="0" align="center" cellpadding="0"
			...
			<%if (channel.getType() == 3){App p = new App(id);%>
			<tr>	
				<td>应用属性：</td>
          		<td>
          		[应用名称：<span class="font-blue"><%=channel.getName()%></span> 　　
          		页面地址：<a href="<%//=cf.SiteAddress+"/"+p.getFullTargetName()%>" target="_blank">
          		<span class="font-blue"><%=p.getTargetName()%>　　</span>
          		</a>
          		编号：<span class="font-blue"><%=channel.getId()%></span>　
          		目录名：<span class="font-blue"><%=channel.getFolderName()%></span>
          		标识名：<span class="font-blue"><%=channel.getSerialNo()%></span>
          		　]
          		</td>
          	</tr>
          	<%
          		if(!p.getTemplate().equals("")){
          	%>
          	<tr>
				<td>发布方式：</td>
				<td>
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="4" cellspacing="1"
					...
					<%
						...
						String Sql = "select * from channel_template where TemplateType=5 and Channel="+ channel.getId();
					...
					<tr bgcolor="#FFFFFF" <%=(IsInherit==1)?"class='rows2'":""%>>
						...
						<td>
							<input name="Submit21" type="button" class="tidecms_btn3" value="设置" onClick="editTemplate('<%=id_%>',5);">&nbsp;
