...
<div class="br-logo">
	...
	<%@include file="collect_tree.jsp"%><!--静态包含-->
    <!-- ########## START: MAIN PANEL ########## -->
...
//=============================================================================================================
			...
			var collect_could = <%=collect_could%> ;
			var collect_paihang = <%=collect_paihang%> ;
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				for(var i=0;i<json.length;i++){
					if(json[i].id!=collect_could&&json[i].id!=collect_paihang){
						continue;
					}
...
//====================================================================
		...
		function setChannelCookie(_id){
			$.ajax({
				...
				success: function(data){
					tidecms.setCookie("collect_path",data.trim());
				}
			});		
		}
		//返回最后一级导航
		...
		function backToOriginal(){
			...
			if(channel_path){
				...
				//channel_id_array = channel_id_array.slice(1);
			}else{
				activeFn();
				return ;
			}
