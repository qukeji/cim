...
function recommendOut1(id,type)
{
	var	dialog = new top.TideDialog();
		dialog.setWidth(500);
		dialog.setHeight(400);
		dialog.setUrl("out_index.jsp?ChannelID="+ChannelID+"&ItemID="+id+"&type="+type);
		dialog.setTitle('推荐');
...
{		
	var obj=getCheckbox();
	if(obj.length==0){
		TideAlert("提示","请先选择要推荐的文档！");
	}else if(obj.length>1){
		TideAlert("提示","请先选择一个要推荐的文档！");
	}else{
		recommendOut1(obj.id,1);
	}
}
//推荐
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="javascript:addtask();" class="btn btn-outline-info">生成选题</a>
