				...
				for(var i=0;i<json.length;i++){
					...
					if( json[i].load==1 || (json[i].child!=null && json[i].child.length>0) ){
						if(json[i].type==0){  //判断是否独立表单
							...
							html += '<a href="javascript:;" channelid="'+json[i].id+'" load="'+json[i].load+'" cloud="'+json[i].cloud+'"><i class="fa fisrtNav fa-home" have="1"></i> <span>大屏数据管理</span></a>';
						}					   
					}else{
						if(json[i].type==0){
