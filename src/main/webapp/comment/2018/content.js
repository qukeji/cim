...
//导入word 曲科籍  2015-10-15
function word_import()
{
	var dialog = new top.TideDialog();
	dialog.setWidth(600);
	dialog.setHeight(400);
	dialog.setUrl("../tcenter/util/word_upload.jsp?FolderName=&SiteId=3&ChannelID="+ChannelID);
	dialog.setTitle("导入word");
	dialog.show();
}

//导出excel  曲科籍  2015-10-10
function excel_export()
{
   
	 var url= "../util/excel_export.jsp?channelid=" + ChannelID;
		$.ajax({
				 type: "GET",
				 url: url,
                 dataType:"json",
				 success: function(msg){
											if(msg.message==""){
                                                ...
                                                window.frames["Download_excel"].location ="../util/excel_download.jsp?Charset=utf-8&FileName="+filename+"";
                                               //window.open("/cms/util/excel_download.jsp?FileName="+filename+"");
                                             }else{alert(msg.message)}
                                                 //window.open(msg.delete);
	...
	jQuery("#oTable").sortable({items:"tr:gt(0)",axis:"y",cursor:"move",
		...
		update:function(e,ui){
			...
			if(OrderNumber_next==0){
				OrderNumber_next = OrderNumber_prev - 1 ;
			}
