...
;(function($){
	$.contextMenu = {
		...
		createMenu: function(menu,cmenu) {
			...
			var $t = $('<table id="contentmenu" cellspacing=0 cellpadding=0></table>').click(function(){cmenu.hide(); return false;}); // We wrap a table around it so width can be flexible
			var $tr = $('<tr></tr>');
			var $td = $('<td></td>');
		...
		createShadow: function(cmenu) {
			cmenu.shadowObj = $('<div id="contextmenu_shadow" class="'+cmenu.shadowClass+'"></div>').css( {display:'none',position:"absolute", zIndex:9998, opacity:cmenu.shadowOpacity, backgroundColor:cmenu.shadowColor } );
		...
		show: function(t,e) {
			...
			if($("#content-table").hasClass("table-fixed")){
				...
				jQuery(t).find(":checkbox").prop("checked", true);
			}else{
				var checkboxAll = $("#content-table tr").find("td:first-child").find(":checkbox") ;
				checkboxAll.removeAttr("checked");	
				jQuery("tbody tr").removeClass("bg-gray-100");			
//				jQuery("#selectNo").trigger("click");
				jQuery(t).addClass("bg-gray-100");
				jQuery(t).find("td").find(":checkbox").prop("checked", true);
			}
			/**cmy add end**/
	...
	$.fn.contextMenu = function(menu,options) {
		...
		return this.each(function(){
			$(this).bind('contextmenu',function(e){
				cmenu.show(this,e);return false;});
		});
	};
})(jQuery);
