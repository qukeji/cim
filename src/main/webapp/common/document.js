...
function childAdjustFrameH(eleId){
	if(navigator.userAgent.indexOf("MSIE")>0||navigator.userAgent.indexOf("rv:11")>0||navigator.userAgent.indexOf("Firefox")>0){
		$(eleId).get(0).style.height = $(eleId).get(0).contentWindow.document.body.scrollHeight+"px";
	}else{
		$(eleId).get(0).style.height = $(eleId).get(0).contentWindow.document.documentElement.scrollHeight+"px";
	}
}
...
{	
	...
	var ChannelId = document.form.ChannelID.value;
	$.ajax({
		type:'POST',
		url:'../content/sys_checktitle.jsp',
		dataType:'json',
		async:false,
		data:{Title:title,ItemID:ItemID,ChannelID:ChannelId},
		success:function(data){
		    result = data.type;
		}
	});
	if(result==2){
		alert("当前栏目中有相同标题稿件存在，请修改后保存。");
		return false;
	}else if(result==1){
	    if(!confirm("当前栏目中有相同标题稿件存在，是否继续保存？")){
	        return false;
	    }
	}
...
//采用
function cloudItemJS(itemid,channelid)
{
	var url = "../recommend/cloud_out_js.jsp";
	scr = document.createElement('script')
	scr.src = url + '?ItemID=' + itemid + "&ChannelID=" + channelid;
	document.getElementById('ajax_script').appendChild(scr);
}
	...
	jQuery.ajax({
            ...
            success : function(data)
			{
				...
				else
				{
					...
					dialog.setUrl("../content/photo_list_from_content.jsp?imgs="+encodeURIComponent(img.toString())+"&channelid="+channelid);
...
function SaveApprove(action){
	...
	var title = $("#Title").val();
	var enTitle=encodeURIComponent(title);// encodeURI(title);
...
function checkEditorTranscode(){
	//$("#ueditor_0").contents().find(".td_editor_play_btn").unbind() ;
	//playEditorVedio();
	clearInterval(checkInterval) ;
	var noTransVideo = $("#ueditor_0").contents().find('[istranscode="true"]') ;
	if( noTransVideo.length > 0 ){
		noTransVideo.find(".td_editor_play_btn").hide();
		checked();
	...
	function checked(){
		...
		var globalids = "" ;
		noTransVideo.each(function (index,ele) {
			if(index==0){
				globalids += $(ele).attr("globalid") ;
			}else{
				globalids+= ","+ $(ele).attr("globalid") ;
			}
		})
		$.ajax({
			type: "GET",
			url: "/vms/video/get_video_transcode_status.jsp",
			data : { globalids:globalids} ,
			dataType : "json" ,
			success: function(data){
				if(data.list.length > 0){
					for(var i = 0 ; i < data.list.length ; i++){
						for(var j = 0 ; j < noTransVideo.length ; j++){
							if( noTransVideo.eq(j).attr("globalid") == data.list[i].globalid){
								console.log(44444)
								noTransVideo.eq(j).attr( "data-transcode" , data.list[i].transcodeStatus )
							}
						}
					}
					changeBackImg();
...
function changeBackImg(){
	var noTransVideo = $("#ueditor_0").contents().find('[istranscode="true"]') ;
	noTransVideo.each(function (index,ele) {
		var code = $(ele).attr("data-transcode") ;
		var tagename = $(ele).get(0).tagName ;
		if( code == 0 || code ==1){  //0等待转码  1转码中
			if( tagename == "VIDEO" ){
				$(ele).attr("poster","/tcenter/static/img/zm0.jpg")
			}else if( tagename == "DIV"){
				$(ele).find("img.tide_video_fake").attr("src","/tcenter/static/img/zm0.jpg")
			}
		}else if(code==2){   //转码成功
			if( tagename == "VIDEO" ){
				$(ele).attr({
					"poster":"","controls":"controls","istranscode":"false" 
				});
			}else if( tagename == "DIV"){
				$(ele).attr("istranscode","false").find("img.tide_video_fake").attr("src","../static/img/video_bg.jpg");
				$(ele).find(".td_editor_play_btn").show();
			}
		}else if(code==3){  //转码失败
			if( tagename == "VIDEO" ){
				$(ele).attr({
					"poster":"/tcenter/static/img/zm1.jpg","controls":"false","istranscode":"false" 
				}).get(0).pause();;
			}else if( tagename == "DIV"){
				$(ele).attr("istranscode","false").find("img.tide_video_fake").attr("src","/tcenter/static/img/zm1.jpg")
			}
		}
	})
}
function playEditorVedio(){
	
	$("#ueditor_0").contents().delegate(".td_editor_play_btn" , "click",function(){
		//视频预览
		//$(this).unbind();
		var flv = $(this).parents(".td_video_box").attr("data-url") ;
		if( ! $.trim(flv) ){
