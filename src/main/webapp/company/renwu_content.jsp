...
<%
    ...
    /*//获取选题频道下租户绑定的租户频道
    int company = CmsCache.getUser(userId).getCompany();
...
//当前登录用户绑定了租户,此频道下面是租户频道(聚融，大屏)
    ...
    if (userinfo_session.getCompany() != 0 && (a.startsWith("pgc_") || a.startsWith("screen") || a.startsWith("task") || a.startsWith("shenpian") || a.startsWith("shengao"))) {
        ...
        System.out.println("频道名字是"+channel.getName());
    }
    ...
    int me = getIntParameter(request, "me");
...
<head>
    ...
    <script>
        ...
        var Parameter = "&ChannelID=" + ChannelID + "&rowsPerPage=" + currRowsPerPage + "&currPage=" + currPage + "&me=1";
        ...
        function addrenwu(id, ChannelID) {
            window.open("renwu_add.jsp?SubjectId=" + id + "&ChannelID=" + ChannelID);
        }

        function Previewdtial(id) {
            window.open("renwu_operation.jsp?ItemID=" + id + "&ChannelID=<%=ChannelID%>");
        }
        ...
        function Preview2(id) {
            window.open("renwu_operation.jsp?ItemID=" + id + Parameter);
        }
        ...
        function editDocument3(itemid) {
            var url = "renwu_add.jsp?ItemID=" + itemid + "&ChannelID=" + ChannelID;
            window.open(url);
        }
        ...
        function editDocument4(id) {
            var url = "renwu_add.jsp?ItemID=" + id + "&ChannelID=" + ChannelID;
            window.open(url);
        }
...
<div class="br-mainpanel br-mainpanel-file" id="js-source">
    <div class="br-pageheader pd-y-15 pd-md-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            <span class="breadcrumb-item active">全部任务</span>
        </nav>
    </div>
    <!-- br-pageheader -->
    <%
...
// out.println(ListSql);
    ...
    <div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
        ...
        <div class="btn-group mg-l-auto hidden-sm-down">
            <a href="javascript:addrenwu('','<%=ChannelID%>');" class="btn btn-outline-info">新建</a>
            <%if (canApprove) {%>
    ...
    <div class="search-box pd-x-20 pd-sm-x-30" style="display:none;">
        <div class="search-content bg-white">
            <form name="search_form" action="<%=pageName%>?id=<%=id%>&me=1&rowsPerPage=<%=rowsPerPage%>"
                ...
                <div class="row">
                    ...
                    <div class="mg-r-10 mg-b-30 search-item">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control search-author" placeholder="提交人" name="User"
                                   value="<%=S_User%>">
                        </div>
                    </div>
                    <!--状态-->
        ...
        <div class="card bd-0 shadow-base">
            <table class="table mg-b-0 <%=listType==2?"table-fixed":""%>" id="content-table">
                ...
                <tr>
                    ...
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-350">所属选题</th>
                    ...
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150 wd-author">任务执行人</th>
                ...
                <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>"
                    ...
                    <td class="dropdown hidden-xs-down">
                        ...
                        <a href="javascript:Previewdtial('<%=id_%>');" class="svg-icon fa-svg-icon svg-fa-check-circle"
                           viewBox="0 0 512 512" title="查看详情"><i class="fa fa-check-circle tx-20"
