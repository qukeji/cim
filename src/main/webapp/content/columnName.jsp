...
String fieldName		= getParameter(request,"fieldName");//字段名
String categoryid		= getParameter(request,"fieldcategoryid");//字段名
String linkhref		= getParameter(request,"fieldlinkhref");//字段名
...
function next()
{
	...
	jQuery("#leftTd input:checked").each(function(i){
	   ...
	   _name = $(this).parent().siblings("span").text();
	});
	
	if (channelid!=""&&(channelid.indexOf(",")<0))
	{		
		$("#submitButton").attr("disabled",true);
		parent.$("#"+categoryid).attr("value",channelid);
		parent.$("#"+fieldName).attr("value",_name);
		$.ajax({
			url:'../content/channel_listHref.jsp?channelid='+channelid+"&TargetName=<%=TargetName%>",
			type:'get',
			success:function (res) {
				parent.$("#"+linkhref).attr("value",res.trim());
				top.TideDialogClose();
			}
        });
	}
