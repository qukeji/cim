...
//预览
function PreviewJuShi()
{
	var obj=getCheckbox();
	if(obj.length==0){
		ddalert("请先选择要预览的文件！","(function dd(){getDialog().Close({suffix:'html'});})()");
	}else if(obj.length>1){
		ddalert("请先选择一个预览的文件！","(function dd(){getDialog().Close({suffix:'html'});})()");
	}else{
		var JuShiurl="";
	jQuery("#content-table input:checked").each(function(i){
		var obj=jQuery(this).parent().parent().parent();		
			JuShiurl+=jQuery(obj).attr("jushiurl");
	     });
		 PreviewJuShi2(obj.id,JuShiurl);//window.open("../content/document_preview.jsp?ItemID=" + obj.id + Parameter);
	}
}
function PreviewJuShi2(id,JuShiurl)
{
	
	
	window.open("../content/document_previewjushi.jsp?ItemID=" + id +"&jushiurl="+ JuShiurl + Parameter);	
}
</script>
</head>
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="javascript:addDocument();" class="btn btn-outline-info">新建</a>
				 <%if(canApprove){%>
					...
					<a href="javascript:PreviewJuShi();" class="btn btn-outline-info">预览</a>
				 <%}else{%>
					<a href="javascript:PreviewJuShi();" class="btn btn-outline-info">预览</a>
				<%}%>
...
String ListSql = "select id,GlobalID,item_gid,Content,username,url,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down">用户名</th>
							<%if(IsWeight==1){%>
...
while(Rs.next())
{
	...
	int GlobalID=Rs.getInt("GlobalID");
        ...
        Document doc=new Document(item_gid);
	int myid=doc.getId();
	int mychannelid=doc.getChannelID();
	if(temp_gid!=0){
		globalids+=",";
	}
	temp_gid++;
	...
	{
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" jushiurl=<%=url%>  status="<%=Status%>" GlobalID="<%=GlobalID%>"  id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}else{%>
				<a href="javascript:PreviewJuShi2(<%=id_%>,'<%=url%>');" class="btn pd-0 mg-r-5" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
			  <!-- <a href="javascript:Preview3(<%=id_%>,);" class="btn pd-0 mg-r-5" title="正式地址预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a> -->
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<a href="javascript:PreviewJuShi2(<%=id_%>,'<%=url%>');" class="nav-link">预览</a>
