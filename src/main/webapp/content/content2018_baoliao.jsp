...
//System.out.println("CreateCatcontent2018_baoliaoegory:"+channel.hasRight(userinfo_session,ChannelPrivilegeItem.CreateCategory));
...
function getTipster(type){
   ...
   // var message = "确实要删除这"+obj.length+"项吗？";
    if(obj.length==0){
        ...
        var message = "确实是这篇文章爆料审核吗？";
        if(type==2){
            message = "确实是这篇文章爆料驳回吗？";
        }
        if(!confirm(message)){
            return;
        }
        tipster(obj,ChannelID,type);
    }
}
//爆料审核
function tipster(obj,ChannelID,type){
    var url="../content/reviewBaoliao.jsp?doc_id="+obj.id+"&type="+type+"&ChannelID="+ChannelID;
    $.ajax({
        type: "GET",
        url: url,
...
if(doc_type_channel== -1){
	 ListSql = "select id,Title,Summary,Content,Photo,Weight,GlobalID,phone,User,baoliao_userid,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
}else{
	 ListSql = "select id,Title,Summary,Content,doc_type,Photo,Weight,GlobalID,phone,User,baoliao_userid,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,baoliaostatus,baoliao_ispublic";
}	
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<%}else{%>
					<a href="javascript:Preview();" class="btn btn-outline-info">预览</a>
				<%}%>
				    <a href="javascript:getTipster(1);" class="btn btn-outline-info">爆料审核</a>
				    <a href="javascript:getTipster(2);" class="btn btn-outline-info">爆料驳回</a>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium wd-80">审核状态</th>
							<th class="tx-12-force tx-mont tx-medium wd-80">是否公开</th>
				            <%}%>
							<%if(IsWeight==1){%>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80 wd-author">爆料人</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80 wd-author">爆料人手机号</th>
							<%if(IsComment==1){%>
							...
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
while(Rs.next())
{
	...
	String baoliao_statustxt="";
	String baoliao_ispublictxt="";
	
	int baoliaostatus=Rs.getInt("baoliaostatus");
	switch(baoliaostatus){
	    case 0: 
	          baoliao_statustxt="<span class='tx-orange'>待审核</span>";
	          break;
	    case 1:
	          baoliao_statustxt="<span class='tx-success'>审核通过</span>";
	          break;
	    case 2:
	          baoliao_statustxt="<span class='tx-danger'>审核驳回</span>";
	          break;
	    
	}
	
	int baoliao_ispublic=Rs.getInt("baoliao_ispublic");
	
	switch(baoliao_ispublic){
	    case 1: 
	          baoliao_ispublictxt="<span class='tx-success'>公开</span>";
	          break;
	    case 0:
	          baoliao_ispublictxt="<span class='tx-danger'>非公开</span>";
	          break;
	}
	...
	//类型为视频并且标题为空时显示摘要内容
	/*if("".equals(content)&& Rs.getInt("doc_type")==1){
    	content = convertNull(Rs.getString("Summary"));
	}*/
	...
	int baoliao_userid	= Rs.getInt("baoliao_userid");
	String userName = CmsCache.getUser(baoliao_userid).getName();
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            ...
            <td ondragstart="OnDragStart(event)">
              <%if(IsPhotoNews==1){%>
			  <i class="fa drag-list fa-picture-o tx-18 tx-primary lh-0 valign-middle" id="img_<%=j%>"></i>
              <%}else{%>
              ...
              <%}%>
              <span class="pd-l-5 tx-black"><%=content%></span>
            </td>
			<%if(doc_type_channel != -1){%>
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}%>
				<%if(active==0 && userinfo_session.isAdministrator()&&canDelete){%>
				...
				<%if(TopStatus==1){%>
				<a href="javascript:CancleTop(<%=ChannelID%>,2,<%=id_%>);" class="btn pd-0 mg-r-5" title="撤销置顶">
					<i class="fa fa-arrow-circle-o-down tx-18 handle-icon" aria-hidden="true"></i>
				</a>
				<%}%>	-->
				<%if(active==1 && canApprove){%>
    			<a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-5" title="发布"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true"></i></a>
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%}else{%>
							...
							<%if(TopStatus==1){%>
							<a href="javascript:CancleTop(<%=ChannelID%>,2,<%=id_%>);" class="nav-link" title="">撤销置顶</a>
							<%}%>
