...
if(doc_type_channel== -1){
	 ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
}else{
	 ListSql = "select id,Title,doc_type,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
}	
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<%}%>
				<a href="javascript:submitWeixin();"  class="btn btn-outline-info hidden-md-1500-down">一键分发</a>
			</div>
			<!-- START: 按钮过多时的部分功能下拉 -->
			<div class="dropdown hidden-md-1500-up hidden-sm-down mg-l-10">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">					
						...
						<%}%>
						<a href="javascript:submitWeixin();" class="nav-link">一键分发</a>
					</nav>
				</div>
				<!-- dropdown-menu -->
			</div>
...
<script>
   ...
   /* /cms/lib/weixin_list.jsp*/
	function submitWeixin(){
        var obj=getCheckbox();
        var channelid =ChannelID;
        var ids = obj.id;
        if(obj.length==0){
        ...
        var url ="../content/weixin_list.jsp?channelid="+channelid+"&ids="+ids;
        var	dialog = new top.TideDialog();
        dialog.setWidth(500);
        dialog.setHeight(400);
        dialog.setUrl(url);
        dialog.setTitle('分发列表');
        dialog.setChannelName('微信群发频道');
        dialog.show();
