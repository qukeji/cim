...
function createLive(){
	...
	var url = "../content/jushi_live_juxian.jsp";
	$.ajax({
		url: url,
		type:"get",
		success:function(data){
			tempwindow.location=data.trim();
			//window.open();//打开聚现活动创建页面
		}
	});  
}
//同步聚现活动
function syncLive(){
	var url = "../content/juxian_live_message.jsp";
	$.ajax({
		url: url,
		type:"get",
		success:function(data){
...
%>		
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<%}%>
					<a href="javascript:createLive();" class="btn btn-outline-info">创建聚现直播</a>
					<a href="javascript:syncLive();" class="btn btn-outline-info">同步聚现直播</a>
				<%if(canWord>0){%>
					<a href="javascript:word_import();" class="btn btn-outline-info">导入WORD</a>
				<%}%>
				<%if(canExcel>0){%>
					<a href="javascript:excel_export();" class="btn btn-outline-info">导出EXCEL</a>
				<%}%>
			</div>
			<!-- START: 按钮过多时的部分功能下拉 -->
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%}%>
						<a href="javascript:createLive();" class="nav-link">创建聚现直播</a>
						<a href="javascript:syncLive();" class="nav-link">同步聚现直播</a>
					    <%if(canWord>0){%>
						<a href="javascript:word_import();" class="nav-link">导入WORD</a>
						<%}%>
						<%if(canExcel>0){%>
						<a href="javascript:excel_export();" class="nav-link">导出EXCEL</a>
					    <%}%>		
					</nav>
				</div>
				<!-- dropdown-menu -->
					...
					<div class="row">
						...
						<div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
							<select class="form-control select2" data-placeholder="状态" name="Status">
								...
								<option value="0" <%=(S_Status==0?"selected":"")%>>全部</option>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">作者</th>
							<%if(IsComment==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-200">评论</th>
							<%}%>
							<%if(IsClick==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">点击量</th>
							<%}%>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}%>
				<%if(active==0 && userinfo_session.isAdministrator()&&canDelete&&canDelete){%>
