...
<head>
    ...
    <script>
        ...
        function reply2(id){
            ...
            var url= "../wenzheng/politicsreply.jsp?ChannelID="+ChannelID+"&ItemID="+id+"&GlobalID="+GlobalID;
...
//添加判断有内容类型的频道
        ...
        if(doc_type_channel== -1){
            ListSql = "select id,Title,Photo,Status2,Weight,GlobalID,User,UserName,phone,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
        }else{
            ListSql = "select id,Title,doc_type,Status2,Photo,Weight,GlobalID,User,UserName,phone,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
        }
...
// out.println(ListSql);
    ...
    <div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
        ...
        <div class="btn-group mg-l-10 hidden-xs-down">
            ...
            <a href="javascript:list('Status2=3');" class="btn btn-outline-info list_nocall" id=list_nocall>待办理</a>
            <a href="javascript:list('Status2=1');" class="btn btn-outline-info list_calling">办理中</a>
            <a href="javascript:list('Status2=2');" class="btn btn-outline-info list_called">已办理</a>
        ...
        <div class="btn-group mg-l-auto hidden-sm-down">
            ...
            <a href="javascript:reply();" class="btn btn-outline-info">回复</a>
            <a href="javascript:ChangeStatus2();" class="btn btn-outline-info">办理状态</a>
            <%if(canApprove){%>
        ...
        <div class="search-content bg-white">
            <form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
                <div class="row">
                    ...
                    <div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
                        <select class="form-control select2" data-placeholder="处理状态" name="Status2">
                            <option label="Choose one"></option>
                            <option value="0">全部</option>
                            <option value="3" <%=(Status2==3?"selected":"")%>>待办理</option>
                            <option value="1" <%=(Status2==1?"selected":"")%>>办理中</option>
                            <option value="2" <%=(Status2==2?"selected":"")%>>已办理</option>
                        </select>
                    </div>
        ...
        <div class="card bd-0 shadow-base">
            <table class="table mg-b-0 <%=listType==2?"table-fixed":""%>" id="content-table">
                ...
                <tr>
                    ...
                    <%}%>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">办理状态</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">稿件状态</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">用户名</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">手机号</th>
                ...
                <%
                    ...
                    while(Rs.next())
                    {
                        ...
                        //
                        if(Status22==0){
                            Status2Desc="<span class='tx-danger'>待办理</span>";
                        }else if(Status22==1){
                            Status2Desc="<span class='tx-'>办理中</span>";
                        }else if(Status22==2){
                            Status2Desc="<span class='tx-success'>已办理</span>";
                        }
                    ...
                    <td class="dropdown hidden-xs-down">
                        ...
                        <div class="dropdown-menu dropdown-menu-right pd-10">
                            <nav class="nav nav-style-1 flex-column">
                                ...
                                <a href="javascript:reply(<%=id_%>);" class="nav-link">回复</a>
                                <%}%>
                                <a href="javascript:ChangeStatus2(<%=id_%>);" class="nav-link">办理状态</a>
                                
                                    <a href="javascript:approve(<%=id_%>);" class="nav-link">发布</a>
