...
int item_type1 = getIntParameter(request,"item_type");
String globalids="";
if(currPage<1)
	currPage = 1;
...
if(ApproveScheme!=0){
	response.sendRedirect("../approve/content_ziyuan_approve.jsp?id="+id);
	return ;
}
...
int S_IsIncludeSubChannel	=	getIntParameter(request,"IsIncludeSubChannel");
	System.out.println("S_IsIncludeSubChannel=="+S_IsIncludeSubChannel);
...
boolean canList = channel.hasRight(userinfo_session,ChannelPrivilegeItem.CanList);
boolean onlyMe = channel.hasRight(userinfo_session,ChannelPrivilegeItem.onlyMe);
...
//获取频道的功能菜单表标题是“查看”的status和isview
%>
<!DOCTYPE html>
<html>
<head>
...
//添加判断有内容类型的频道
String ListSql = "select id,Title,doc_type,item_type,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,PublishDate,Status,Active,Category,IsPhotoNews";
...
if(item_type1!=0)
{
		...
		WhereSql += " and item_type=" + item_type1;
}
ListSql += WhereSql;
CountSql += WhereSql;
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-10 hidden-xs-down">
				...
				<a href="javascript:list('item_type=5');" class="btn btn-outline-info list_photo4">轮播图</a>
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<a href="javascript:copy(1);" class="nav-link">移动</a>
						<%if(IsWeight!=1){%>
						<a href="javascript:sortableEnable();" class="nav-link">排序</a>
						<%}%>
						<%if(IsTopStatus){%>
						<a href="javascript:ChangeTop(<%=id%>,1);" class="nav-link">置顶</a>
						<a href="javascript:ChangeTop(<%=id%>,2);" class="nav-link">撤销置顶</a>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium wd-100">列表展现形式</th> 
							<%if(IsWeight==1){%>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
    			...
    			<a href="javascript:approve2(<%=id_%>);" class="btn pd-0 mg-r-0" title="发布"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
				<%if(active==1){%>
					<a href="javascript:Preview2_(<%=id_%>);" class="btn pd-0 mg-r-0" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
					<a href="javascript:Preview3_(<%=id_%>);" class="btn pd-0 mg-r-0" title="正式地址预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-10 hidden-xs-down">
				...
				<a href="javascript:list('Status1=-1');" class="btn btn-outline-info list_draft">草稿</a>
				<a href="javascript:list('Status1=1');" class="btn btn-outline-info list_publish">已发</a>
				<a href="javascript:list('item_type=5');" class="btn btn-outline-info list_delete">轮播图</a>
				<a href="javascript:list('IsDelete=1');" class="btn btn-outline-info list_delete">已删除</a>
				<a href="#" class="btn btn-outline-info btn-search">搜索</a>
			</div>
			<!-- btn-group -->
...
<script>
	...
	$(function() {
		...
		}else if (item_type_ == 5) {

			$(".list_photo4").addClass("active");
