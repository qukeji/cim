...
function getRadio(){
	...
	result = $("#item_"+id).attr("contentHref");

	if(doc_type==4){
		result = $("#item_"+id).attr("juxianliveid");
	}
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,juxian_liveid";
...
while(Rs.next())
{
	...
	int juxianLiveid = 0 ;
	if(doc_type==4){
		juxianLiveid = Rs.getInt("juxian_liveid");
	}

	if(listType==1)
	{
%>
		 <tr No="<%=j%>" ItemID="<%=id_%>" contentHref="<%=contentHref%>" juxianLiveid="<%=juxianLiveid%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
