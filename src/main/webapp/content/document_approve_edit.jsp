 ...
 <div class="br-header">
	...
	<div class="br-header-right">
		<div class="btn-box" >	      	
			...
			<%if(ApproveScheme!=0){%>
				<button type="button" class="btn btn-secondary tx-size-xs mg-r-5" id="Image2Href" onclick="Save_approve();">提交审核</button>
			<%}else{
				if(cp.hasRight(userinfo_session,ChannelID,3)){%>
			...
			-->
				...
				<button type="button" class="btn btn-secondary tx-size-xs mg-r-5" onclick="">本地预览</button>
				<button type="button" class="btn btn-secondary tx-size-xs mg-r-5" onclick="approveSave(1);" id="Image1Href">驳回</button>
				<button type="button" class="btn btn-secondary tx-size-xs mg-r-5" onclick="approveSave(0);" id="Image2Href">通过</button>
				<button type="button"  class="btn btn-secondary tx-size-xs mg-r-10" onclick="self.close();">关闭</button>	
		</div>
	</div><!-- br-header-right -->
</div><!-- br-header -->
...
do{
	...
	%>
	    ...
	    <div class="br-mainpanel br-mainpanel-file overflow-hidden" id="form<%=j+1 %>" <%if(j!=0)out.print("style=\"display:none;\"");%>>   
<!--	<div class="br-pagebody bg-white mg-l-20 mg-r-20">
	...
	if(fieldGroupID>0 && !fieldGroup.getUrl().equals(""))
	{
		out.println("<iframe id=\"iframe"+(j+1)+"\" frameborder=\"0\" style=\"width:100%;min-height:500px;height:100%;\" marginheight=\"0\" marginwidth=\"0\" scrolling=\"auto\" src=\""+url+"\" onload=\"changeFrameHeight(this)\" ></iframe>");
...
<%}%>
	...
	%>
		<!--审核预览-->
				<div class="col-sm-4 pd-x-0-force">
					<div class="br-pagebody mg-l-20 mg-r-40 pd-x-0-force" >
	...
	out.println("</div></div></div> </div></div>");j++;
...
<!--	<div class="br-pagebody bg-white mg-l-20 mg-r-20">
		<div class="br-content-box pd-20">  -->
<div class="br-pagebody bg-white mg-l-20 mg-r-20 "><div class="br-content-box pd-20"> <div class="center"  >
		<div class="article-title mg-l-15 mg-b-15 pd-r-30" >
  ...
  $(function(){
	'use strict';
		...
		$('.br-sideleft [data-toggle="tooltip"]').tooltip({
			...
			$.ajax({
				type: "get",
				url: "../approve/approve_info.jsp?globalid=<%=GlobalID%>&channelid=<%=ChannelID%>",
				success: function(msg){
...
//			});
	...
	{
		window.open("../approve/approve_preview_version.jsp?vid="+versionid+"&ItemID=" + <%=ItemID%> + Parameter);
	}
