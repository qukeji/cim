    ...
    <script>
        ...
        function Save(action){
            ...
            if(end==1&&action==1){
                user=end_user;
                endApprove=1;
                actionMessage="终审完成后再次驳回";
                approveId=end_approveId;
            }
            if(action==1&&actionMessage==""){
                alert("请输入驳回理由");
                return false ;
            }
            var url= "../approve/approve_sumbit.jsp?globalid="+GlobalID+"&ItemID=<%=ItemID%>"+"&ChannelID=<%=ChannelID%>"+"&title="+title+"&action="+action+"&user="+user+"&approveId="+approveId+"&endApprove="+endApprove+"&actionMessage="+actionMessage+"&ApproveActionId=<%=id_aa%>";
            $.ajax({
                type: "GET",
                url: url,
...
<div class="br-header">
    ...
    <div class="br-header-right">
        <div class="btn-box" >
            ...
            <%if(canApprove&&status==0){%>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="approve_(<%=ItemID%>);window.opener.location.reload();self.close();">发布</button>
            <%}%>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="Preview2_(<%=ItemID%>)">预览</button>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="editDocument(<%=ItemID%>)">编辑</button>
            <%if(status==1){%>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="deleteFile2_confirm(<%=ItemID%>,1);window.opener.location.reload();self.close();">撤稿</button>
            <%}%>
            <%if(!channel.getRecommendOut().equals("")){%>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="recommendOut1()">推荐</button>
            <%}%>

            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="copy(<%=ItemID%>,0);">复制</button>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="copy(<%=ItemID%>,1);">移动</button>
            <%if(IsTopStatus){%>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="ChangeTop(<%=ChannelID%>,1)">置顶</button>
            <button type="button" class="btn btn-primary tx-size-xs mg-r-5" onclick="ChangeTop(<%=ChannelID%>,2)">撤销置顶</button>
            <%}%>
            <button type="button"  class="btn btn-primary tx-size-xs mg-r-10" onclick="self.close();window.opener.document.location.reload();">关闭</button>
            ...
            <div class="mg-l-15 mg-b-15 pd-r-30">
                ...
                <div class="row flex-row align-items-top mg-b-15" id="">
                    ...
                    <%
                        ...
                        String value = item.getValue(field.getName());
                    %>
                    <label class="wd-content-lable d-flex " id="">
                        <%if(field.getFieldType().equals("switch")){
                            String flag = "false";
                            if(value.equals("1")){
                                flag = "true";//开
                            }
                        ...
                        <div class='toggle-wrapper'>
                            <div class='toggle toggle-modern primary' data-toggle-on='<%=flag%>'></div>
                        </div>
                        <%}else if(field.getFieldType().contains("radio")) {
                            ArrayList<String[]> fieldoptions = field.getFieldOptions();
                            String a = "";
                            for (int k = 0; k < fieldoptions.size(); k++) {
                                String[] ss = (String[]) fieldoptions.get(k);
                                if (value.equals(ss[1])) {
                                    a = ss[0];
                                    break;
                                }
                            }
                        %>
                            <%=item!=null?a:""%>
                        <%}else if(field.getFieldType().contains("checkbox")) {
                            ArrayList<String[]> fieldoptions = field.getFieldOptions();
                            String[] arr = Util.StringToArray(field.getValue(),",");
                            String a = "";
                            for (int k = 0; k < fieldoptions.size(); k++) {
                                String[] ss = (String[]) fieldoptions.get(k);
...
<script>
    ...
    //推荐
    function recommendOut1()
    {
        var	dialog = new top.TideDialog();
        dialog.setWidth(500);
        dialog.setHeight(400);
        dialog.setUrl("../recommend/out_index.jsp?ChannelID=<%=ChannelID%>&ItemID=<%=ItemID%>");
        dialog.setTitle('推荐');
    ...
    function copy(itemid,type){
        ...
        dialog.setWidth(500);
        dialog.setHeight(400);
        dialog.setUrl("../content/document_copy_index.jsp?ChannelID="+ChannelID+"&ItemID="+itemid+"&type="+type+"&look=1");
        dialog.setTitle('选择');
        dialog.show();
    }
    //发布
