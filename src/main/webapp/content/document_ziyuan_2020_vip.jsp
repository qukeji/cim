...
<link rel="stylesheet" href="../static/css/document_2020_vip.css">
<style type="text/css">
...
<script src="../static/js/document_2020_vip.js"></script>
...
function Save()
{
	...
	if(!editorNocode){  //有未转码成功的视频
		var	dialog = new TideDialog();
		dialog.showAlert("视频未转码完成，请等待完成后操作!","danger");
		return false ;
	}
	//有内容拦的情况下，保存前先处理内容栏
...
<div class="br-header" id="doc_header">
	...
	<div class="br-header-right">
		<div class="btn-box" >
			...
			<a class="btn btn-primary btn-sm  mg-r-5" href="javascript:Save();"  id="Image1Href" ><i class="fa fa-search mg-r-5"></i>保存</a>
			<%if(ApproveScheme!=0){%>
				<a class="btn btn-primary btn-sm mg-r-5" id="Image2Href" href="javascript:Save_approve();">提交审核</a>
			<%}else{
				if(cp.hasRight(userinfo_session,ChannelID,3)){%>
					<a class="btn btn-primary btn-sm mg-r-5" id="Image2Href" href="javascript:Save_Publish();"><i class="fa fa-paper-plane mg-r-5"></i>保存并发布</a>
			<%}}}else{%>
				...
				<%}%>
			<%}%>
			<a class="btn btn-primary btn-sm" href="javascript:self.close();window.opener.document.location.reload();"><i class="fa fa-power-off mg-r-5"></i>关闭</a>
		</div>
	</div><!-- br-header-right -->
</div><!-- br-header -->
