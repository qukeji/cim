...
//=========================================================================================
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				for(var i=0;i<json.length;i++){
					...
					//html += '<a href="#" channelid="'+json[i].id+'" load="'+json[i].load+'" cloud="'+json[i].cloud+'"><i class="fa fa-television"></i> <span>'+json[i].name+'</span>';
					html += '</a>';
		...
		function get_menu_html(json)
		{
			...
			if(json.child && json.child.length>0)
			{
				...
				for(var i=0;i<json_.length;i++){
					html += '<li><a href="#" load="'+json_[i].load+'" channelid="'+json_[i].id+'" cloud="'+json_[i].cloud+'">';
...
//				$(this).addClass("clicked");//设置不可重复点击
				...
				var cloud = $this.attr("cloud");//是否是云资源
                var haveChild = $this.find("i").attr("have");
				if(load==1){//需要请求接口

					if(cloud==1){//请求云资源菜单接口
						
						var url="tree_api_json.jsp?id="+channelid;
						$.ajax({
							type:"GET",
							dataType:"json",
							url:url,
							...
							if(json.length!=0){
								...
								for(var i=0;i<json.length;i++){
									...
									html += '<a href="javascript:;" cloud="'+json[i].cloud+'" load="'+json[i].load+'" channelid="'+json[i].id+'">';
				...
				//内容显示
				if(cloud==1){					
					changeFrameSrc( window.frames["content_frame"] , "../source/source_content.jsp?id="+channelid )					
				}else{
