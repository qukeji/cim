...
//			UseCompress = getIntParameter(item.getString());
		...
		//System.out.println("Watermark="+Watermark+",mask_location="+mask_location);
    } 
}
...
*/
					...
					boolean succ = FileUtil.waterMaskByIm4java(FilePath,mask_location);
...
var FilePath = "<%=superpath%>";
//window.parent.setPath(FilePath);
var frame = window.parent.window.document.getElementById("edui132_iframe");
frame.contentWindow.setPath(FilePath)
dialog.CloseDialog();
//frames.contentWindow.setPath(FilePath);

</script>
<%}else{%>
 var FilePath = "<%=superpath%>";
 var frame = window.parent.window.document.getElementById("edui132_iframe");
parent.getDialog().Close({close:function(){frame.contentWindow.setPath('<%=Util.JSQuote(ReturnValue)%>');<%=ReturnValue2%>;<%=ReturnFileSize%>}});
<%}%>
<%}else{%>
	alert('上传失败,请重试!');
<%}%>
