...
int Type1 =	getIntParameter(request,"Type1");
...
String S_startDate			=	getParameter(request,"S_startDate");
...
int S_Type				=	getIntParameter(request,"Type");
int S_company_type				=	getIntParameter(request,"company_type");
int S_Status1				=	getIntParameter(request,"Status1_");
...
<style>
    ...
    #content-table tr th:nth-child(2){min-width: 80px;}
...
function refusemedia(){
	...
	var itemid = jQuery("#content-table input:checked").val();
	var url="../content/refuse_media.jsp?id="+<%=id%>+"&itemid="+itemid;
	var	dialog = new top.TideDialog();
		dialog.setWidth(550);
		dialog.setHeight(320);
		dialog.setUrl(url);
		dialog.setTitle("媒体号驳回");
		dialog.show();
...
function getItemid(){
	...
	jQuery("#content-table input:checked").each(function(i){
		...
		if(i==0)
			itemid+=jQuery(obj).attr("ItemID");
		else
			itemid+=","+jQuery(obj).attr("ItemID");
	});
	var obj={length:jQuery("#content-table input:checked").length,itemid:itemid};
	return obj;
}
...
function getTitle(){
	...
	jQuery("#content-table input:checked").each(function(i){
		var obj=jQuery(this).parent().parent().parent();
		if(i==0)
			title+=jQuery(obj).attr("title_channel");
		else
			title+=","+jQuery(obj).attr("title_channel");
	});
	var obj={length:jQuery("#content-table input:checked").length,title:title};
	return obj;
}
//搜索二级下拉
function docheck(){
	...
	if(str==2){
		$("#company_type").prop("disabled",false);
	}
...
function auditmedia(){
	...
	if(obj.length==0){
		...
		TideAlert("提示","一次只能审核一个用户！");
		return;
	}else{
		message = "确定要通过这"+obj.length+"个用户媒体号审核吗？";
	}
	...
	$.ajax({
		...
		success:function(res){
			if(res.status == "success"){
				var url= "../content/media_approvedocument2018.jsp?ItemID=" + itemid + Parameter;
				  $.ajax({
					 type: "GET",
					 url: url,
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,IP,Status1,Type,Email,phone,jushi_userid";
...
{
	S_Type=S_Type-1;
		WhereSql += " and Type=" + S_Type;
}
if(S_Status1!=0)
{
	S_Status1=S_Status1-1;
		WhereSql += " and Status1=" + S_Status1;
}
if(S_company_type!=0)
{
		WhereSql += " and company_type=" + S_company_type;
}
ListSql += WhereSql;
CountSql += WhereSql;
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-10 hidden-xs-down">
				...
				<a href="javascript:list('Type1=-1');" class="btn btn-outline-info list_draft">个人</a>
				<a href="javascript:list('Type1=1');" class="btn btn-outline-info list_publish">企业</a>
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				...
				<a href="javascript:auditmedia();" class="btn btn-outline-info">通过</a>
				<a href="javascript:refusemedia();" class="btn btn-outline-info">驳回</a>
				...
				<form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
					<div class="row">
						...
						<div class="mg-r-10 mg-b-30 search-item">
							<input class="form-control search-title" placeholder="申请名称" type="text" name="Title" value="<%=S_Title%>" onClick="this.select()">
						</div>
						
						<!--类型-->
						<div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
							<select class="form-control select2" data-placeholder="类型" name="Type" id="Type" onchange="docheck()">
								<option label="Choose one"></option>
								<option value="0">全部</option>
								<option value="1" <%=(S_Type==1?"selected":"")%>>个人</option>
								<option value="2" <%=(S_Type==2?"selected":"")%>>企业</option>
							</select>
						</div>

						<!--企业分类-->
						<div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
							<select class="form-control select2" data-placeholder="企业分类" name="company_type" id="company_type" disabled="disabled">
								<option label="Choose one"></option>
								<option value="0">全部</option>
								<option value="1" <%=(S_company_type==1?"selected":"")%>>群媒体</option>
								<option value="2" <%=(S_company_type==2?"selected":"")%>>新闻媒体</option>
								<option value="3" <%=(S_company_type==3?"selected":"")%>>国家机构</option>
								<option value="4" <%=(S_company_type==4?"selected":"")%>>企业</option>
								<option value="5" <%=(S_company_type==5?"selected":"")%>>其他组织</option>
							</select>
						</div>
						<!--状态-->
						<div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
							<select class="form-control select2" data-placeholder="状态" name="Status1_">
								<option label="Choose one"></option>
								<option value="0">全部</option>
								<option value="1" <%=(S_Status1==1?"selected":"")%>>待审核</option>
								<option value="2" <%=(S_Status1==2?"selected":"")%>>审核通过</option>
								<option value="3" <%=(S_Status1==3?"selected":"")%>>审核驳回</option>
							</select>
						</div>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">类型</th>
							<%if(IsWeight==1){%>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-110">用户手机号</th>
...
while(Rs.next())
{
	int id_ = Rs.getInt("id");
  	int userid = Rs.getInt("jushi_userid");
	...
	if(Status2==0)
		...
		StatusDesc = "<span class='tx-success'>审核通过</span>";
	else if(Status2==2)
		StatusDesc = "<span class='tx-danger'>审核驳回</span>";
	}
	if(Type==0)
		TypeDesc = "<span class='tx-orange'>个人</span>";
	else if(Type==1)
		TypeDesc = "<span class='tx-success'>企业</span>";
	...
	{
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" Status2="<%=Status2%>" mold="<%=Type%>" userid="<%=userid%>" OrderNumber="<%=OrderNumber%>" title_channel="<%=Title%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td ondragstart="OnDragStart(event)">
              ...
              <%}%>
              <span class="pd-l-5 tx-black"><%=TypeDesc%></span>
            </td>
