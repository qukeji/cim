...
String querystring = "";
querystring = "&Content="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&startDate="+S_startDate+"&endDate="+S_endDate+"&User="+S_User+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1;
...
String ListSql = "select id,Title,Content,contact,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
//	WhereSql += " and Title like '%" + channel.SQLQuote(tempTitle) + "%'";
    WhereSql += " and Content like '%" + channel.SQLQuote(tempTitle) + "%'";
}
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%if(IsWeight==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">权重</th>
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-80">状态</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-160">日期</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150 wd-author">手机号</th>
							<%if(IsComment==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">评论</th>
							<%}%>
							<%if(IsClick==1){%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">点击量</th>
							<%}%>
...
while(Rs.next())
{
   ...
   //		Title = "[" + CmsCache.getChannel(category).getName() + "]" + Title;
	...
	String contact	= convertNull(Rs.getString("contact"));
	String StatusDesc = "";
	if(IsDelete!=1){
	if(Status==0)
			...
			<div class="row">
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  <p class="card-text"><%if(IsPhotoNews==1){%><i class="icon ion-image tx-22 tx-warning lh-0 valign-middle"></i><%}%><%=contact%>(<%=StatusDesc%>)</p>
