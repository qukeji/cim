...
String ListSql = "select id,Title,doc_type,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,state";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">状态</th>
							...
							<%}%>
							<th class="tx-12-force wd-80 tx-mont tx-medium ">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
while(Rs.next())
{
	...
	if(IsDelete!=1){
	if(Status==0)
		StatusDesc = "<span class='tx-orange'>推送失败</span>";
	else if(Status==1)
		StatusDesc = "<span class='tx-success'>推送成功</span>";
	}
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				<%if(!channel.getAttribute1().equals("")){%>
				   <a href="javascript:recommendIn();" class="btn pd-0 mg-r-5" title="引用"><i class="fa fa-reply tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%> 
