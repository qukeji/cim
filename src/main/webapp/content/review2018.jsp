...
String ListSql = "select id,Title,txtContent,username,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">帖子</th>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            ...
            <td ondragstart="OnDragStart (event)">
              ...
              <%}%>
              <span class="pd-l-5 tx-black"><%=txtContent%></span>
            </td>
			<%if(IsWeight==1){%>
