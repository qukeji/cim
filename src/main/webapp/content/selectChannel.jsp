...
String	fieldName	=	getParameter(request,"fieldName");//频道字段名
String	fieldName1	=	getParameter(request,"fieldName1");//标题
String	TargetName	=	getParameter(request,"TargetName");//目标文件名

int	type			=	getIntParameter(request,"type");//type为0，单选，type为1，多选
Channel channel = new Channel(channelid);
int siteid = channel.getSiteID();
if("manager".equals(flag)){
	channel = CmsCache.getChannel("company_s"+siteid+"_manager");
...
function next()
{
	...
	jQuery("#leftTd input:checked").each(function(i){
		if(i==0){
			...
			channelName += $(this).parent().siblings("span").text();
		}else{
			channelid+= ","+jQuery(this).val();
			channelName += ","+$(this).parent().siblings("span").text();
		}
	...
	if (channelid!="")//&&(channelid.indexOf(",")<0)
	{
		...
		parent.$("#"+fieldName+"_id").prop("value",channelid);
		parent.$("#"+fieldName+"_name").prop("value",channelName);
		if(fieldName1=="Title"||fieldName1=="MoreWindow"){
			parent.$("#"+fieldName1).prop("value",channelName);	
		}
		if(<%=type%>==0&&TargetName!=""){//单选才可以选择链接
			$.ajax({
				url:'../content/channel_listHref.jsp?channelid='+channelid+"&TargetName=<%=TargetName%>",
				type:'get',
				success:function (res) {
					parent.$("#"+fieldName+"_href").prop("value",res.trim());
					if(fieldName1=="href"){
						parent.$("#"+fieldName1).prop("value",res.trim());
					}
					top.TideDialogClose();
				}
			});	
