...
function recommend()
{
	...
	top.location = "select_button_child_submit_items.jsp?GlobalID="+GlobalID+"&ChannelID=" + ChannelID + "&LinkChannelID="+LinkChannelID+"&ChildGlobalID=" + obj.id+"&fieldgroup="+fieldgroup;
}
...
%>
			...
			<div class="dropdown hidden-sm-up">
				...
				<div class="dropdown-menu pd-10">
					<nav class="nav nav-style-1 flex-column">
						<a href="select_button_child_content2018.jsp?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" class="btn btn-outline-info list_all">全部</a>
						<a href="select_button_child_content2018.jsp?id=<%=id%>&Status1=-1&rowsPerPage=<%=rowsPerPage%>" class="btn btn-outline-info list_draft">草稿</a>
						<a href="select_button_child_content2018.jsp?id=<%=id%>&Status1=1&rowsPerPage=<%=rowsPerPage%>" class="btn btn-outline-info list_publish">已发</a>
						<a href="select_button_child_content2018.jsp?id=<%=id%>&IsDelete=1&rowsPerPage=<%=rowsPerPage%>" class="btn btn-outline-info list_delete">已删除</a>
		...
		<div class="search-box pd-x-20 pd-sm-x-30" style="display:none;">
			<div class="search-content bg-white">
				<form name="search_form" action="select_button_child_content2018.jsp?rowsPerPage=<%=rowsPerPage%>&ChannelID=<%=ChannelID%>&LinkChannelID=<%=LinkChannelID%>&GlobalID=<%=GlobalID%>&fieldgroup=<%=fieldgroup%>&LinkChannelID2=<%=LinkChannelID2%>" method="post" onsubmit="document.search_form.OpenSearch.value='1';">
					<div class="row">
						<!--标题-->
			...
			<div class="card bd-0 shadow-base">
				...
				<%if(listType==1){%>
					<thead>
						...
						<!--	<th class="tx-12-force tx-mont tx-medium hidden-xs-down">内容类型</th>  -->
							<%if(IsWeight==1){%>
...
<script>
	...
	function goToPage(){
		...
		if(num<1)
			num=1;
		var href="select_button_child_content2018.jsp?currPage="+num+"&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>";
...
function changeOld(s,id)
{
	...
	document.location.href = "select_button_child_content2018.jsp?id="+id+"<%=querystring%>&rowsPerPage="+value;
}
	$(function() {
		'use strict';
