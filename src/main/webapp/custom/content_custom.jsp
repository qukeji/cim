<%@ page import="tidemedia.cms.system.*,
               tidemedia.cms.system.ChannelUtil2,
				tidemedia.cms.base.*,
				tidemedia.cms.util.*,
				tidemedia.cms.user.*,
				java.util.*,
...
/**
  ...
  *		郭庆光			20150318			增加自定义搜索功能.标题、用户、包涵子频道搜索条件不可隐藏。
											创建时间、状态可以隐藏。
  */
...
//out.println("S_city============"+getParameter(request,"S_city"));
String globalids="";
if(currPage<1)
	currPage = 1;
...
/*String  searchList= channel.getListSearchField();//获取频道管理中配置的搜索字段列表
String searcharr[] = searchList.split("\r\n");
boolean hasSearchList=false;//是否启动字段搜索功能--
*/
//int ChannelSearch = getIntParameter(request,"ChannelSearch");
ChannelUtil2 util2=new ChannelUtil2(ChannelID,request);
util2.setInit(true);
util2.initSearchList(ChannelID,request);
querystring+=util2.getQuerystring();
String html=util2.getSearchHtml(ChannelID,request);
String List_html=util2.getListHeader(ChannelID);
String whereShowList=util2.getWhereSql();




String titleFieldName=channel.getFieldByFieldName("Title").getDescription();
//out.println("S_Title="+getParameter(request,"S_Title"));
//out.println("S_city="+getParameter(request,"S_city"));
//out.println("S_age="+getParameter(request,"S_age"));
//out.println("listFieldType=="+listFieldType.toString());
...
*	列表显示列
*/
ArrayList listShowValue = new ArrayList<String>();
/*String  showList= channel.getListShowField();//列表页显示列
String	showarr[] = showList.split("\r\n");
//boolean hasShowList=false;
//ArrayList listShowFieldName = new ArrayList<String>();//字段名称
//ArrayList listShowDescription = new ArrayList<String>();//中文描述
...
for(int i=0;i<showarr.length;i++){
	if(!showarr[i].equals("")){
		...
		hasShowList=true;
		listShowFieldName.add(showarr[i]);
		listShowDescription.add(channel.getFieldByFieldName(showarr[i]).getDescription());
	}
}
String whereShowList = showList.replace("\r\n",",");
//out.println("whereShowList="+whereShowList);
*/
...
<div class="viewpane_c1" id="SearchArea" style="display:<%=(S_OpenSearch==1?"":"none")%>">
  ...
  <tr>
    ...
    <td><form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post">检索：

	<%=titleFieldName%>
	<input name="Title" type="text" size="18" class="textfield" value="<%=S_Title%>"  onClick="this.select()">
<%if(util2.isHasSearchList()){%>
			<%out.println(html);%>
		
		<%
}

if(util2.isShowSearchCreateDate()){%>
	...
	<!-- 图片新闻
		 ...
		 包含子频道
		  <input type="checkbox" name="IsIncludeSubChannel" value="1" <%=(S_IsIncludeSubChannel==1?"checked":"")%>> 
			<input type="hidden" name="ChannelSearch" value="1">
...
if(whereShowList.length()>0){
	ListSql += ","+whereShowList;
}
if(listType==2)
{
...
String searchsql=util2.getSearchSql_good(request,ChannelID);//添加sql的语句（检索）

WhereSql+=searchsql;
//out.println("WhereSql"+WhereSql);
ListSql += WhereSql;
CountSql += WhereSql;
...
<thead>
		<tr id="oTable_th">
					...
					<%if(util2.isHasShowList()){
						out.println(List_html);
					}%>
					<%if(IsWeight==1){%>
...
while(Rs.next())
{
	...
	if(util2.isHasShowList()){
		for(int i=0;i<util2.getListShowFieldName().size();i++){
//ArrayList listShowFieldName = new ArrayList<String>();//字段名称
//ArrayList listShowDescription = new ArrayList<String>();//中文描述
			
			listShowValue.add(Util.convertNull(Rs.getString(String.valueOf(util2.getListShowFieldName().get(i)))));
...
%>
  <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
	...
	<%if(IsPhotoNews==1){%><img id="img_<%=j%>" src="../images/icon/170.png" title="图片新闻"><%}else{%><img id="img_<%=j%>" src="../images/icon/172.png"/><%}%> <%=Title%></td>
	<%
		if(util2.isHasShowList()){
		for(int i=0;i<listShowValue.size();i++){
	%>
			
			<td class="v1" align="center" valign="middle"><%=String.valueOf(listShowValue.get(i))%></td>
	<%
		}
	}
	%>
