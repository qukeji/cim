...
function select_change()
{
	...
	document.location.href = "../content/video_content2.jsp?id="+value;
}
</script>
</head>
...
分类：<select name="ChannelID" id="ChannelID" onchange="select_change();">
 <option value="4347">全部</option>
<%ArrayList categorys = CmsCache.getChannel(4347).listAllSubChannels(Channel.Category_Type);if(categorys!=null && categorys.size()>0){for(int i = 0;i<categorys.size();i++){Channel subcategory = (Channel)categorys.get(i);%>
                    <option value="<%=subcategory.getId()%>" <%=(subcategory.getId()==id)?"selected":""%>><%=subcategory.getName()%></option>
<%}}%>
                  </select>
	</div>
...
<%if(TotalPageNumber>0){%> 
        <div class="viewpane_pages">
            ...
            <div class="center" style="margin:0 0 0 230px;"><a href="video_content2.jsp?currPage=1&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="首页"><img src="../images/viewpane_pages1.png" alt="首页" /></a><%if(currPage>1){%><a href="video_content2.jsp?currPage=<%=currPage-1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="上一页"><img src="../images/viewpane_pages2.png" alt="上一页" />上一页</a><%}%><%if(currPage<TotalPageNumber){%><a href="video_content2.jsp?currPage=<%=currPage+1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="下一页">下一页<img src="../images/viewpane_pages3.png" alt="下一页" /></a><%}%><a href="video_content2.jsp?currPage=<%=TotalPageNumber%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="末页"><img src="../images/viewpane_pages4.png" alt="末页" /></a></div>
		 <%}%>
            <div class="right">
...
jQuery("#goToId").click(function(){
		...
		if(num<1)
			num=1;
		var href="video_content2.jsp?currPage="+num+"&id="+page.id+"&rowsPerPage="+page.rowsPerPage+page.querystring;
...
<embed src="http://club.vodone.com/images/flashplayer/vodoneplayer.swf?site=http://club.vodone.com&vid=06IKIIMKJGQIGGPGLHPHGGIGH&itemid='+itemid+'" type="application/x-shockwave-flash" wmode="transparent" allowFullScreen="true" allowScriptAccess="always" quality="high" width="480" height="420"></embed>\
	...
	var obj={html:html,length:jQuery("#oTable input:checked").length};
	return obj;
}
</script>
</body>
