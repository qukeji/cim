...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,FROM_UNIXTIME(PublishDate) as PublishDate";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">方案标题</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">创建作者</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">创建日期</th>
...
while(Rs.next())
{
	...
	PublishDate=Util.FormatDate("yyyy-MM-dd HH:mm",PublishDate);
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
			    <a href="javascript:Preview2(<%=id_%>);" class="btn pd-0 mg-r-8" title="预览"><i class="fa fa-eye  tx-18 handle-icon" aria-hidden="true"></i></a>
                <a href="javascript:editDocument(<%=id_%>);" class="btn pd-0 mg-r-8" title="编辑"><i class="fa fa-edit tx-18 handle-icon" aria-hidden="true"></i></a>
                <a href="javascript:deleteFile1(<%=id_%>);" class="btn pd-0 mg-r-5" title="删除"><i class="fa fa-trash tx-18 handle-icon" aria-hidden="true"></i></a>
			</td>
