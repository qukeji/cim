...
window.onload = function()
{
	...
	*/
	...
	GetE('ChannelID').value = document.location.search.substr(1);
...
function Ok()
{
	...
	if(!bHasImage)
	{
		if(document.image_form.sourceType[0].checked)//upload
		{
			if ( GetE('txtUrl').value.length == 0 )
			{
	...
	else
	{
		if(document.image_form.sourceType[0].checked)//upload
		{
			if ( GetE('txtUrl1').value.length != 0 )
			{
				document.image_form.submit();
				return;
			}
...
function UpdatePreviewImage( e, skipId )
{
