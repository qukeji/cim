...
*/
int parentid = getIntParameter(request,"globalid");
if(parentid ==0){
parentid = getIntParameter(request,"parentid");
}
int category2 = getIntParameter(request,"category");
...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&startDate="+S_startDate+"&endDate="+S_endDate+"&User="+S_User+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1+"&parentid="+parentid;
...
<script>
	...
	var Parameter = "&ChannelID=" + ChannelID + "&rowsPerPage=" + currRowsPerPage + "&currPage=" + currPage+"&parentid=<%=parentid%>&category=<%=category2%>";
	...
	function list(str) {
		var url = pageName + "?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>&parentid=<%=parentid%>";
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
				<a href="javascript:addDocument_new();" class="btn btn-outline-info">新建</a>
				<%if(canApprove){%>
			...
			<div class="dropdown mg-l-auto hidden-md-up">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						<a href="javascript:addDocument_new();" class="nav-link">新建</a>
						<%if(canApprove){%>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            <td class="valign-middle" GlobalID="<%=GlobalID%>">
			...
			<td class="dropdown hidden-xs-down">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						<a href="javascript:Preview22(<%=GlobalID%>);" class="nav-link">绑定文章</a>
...
function addDocument_new()
{
	var url="../content/document.jsp?ItemID=0&ChannelID=" + ChannelID+"&Parent="+parentid;
	window.open(url);
}
function change(s,id)
{
	...
	document.location.href = "content_special.jsp?id="+id+"&rowsPerPage="+value+"&globalid=<%=parentid%>&category=<%=category2%>";
}
