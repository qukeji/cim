...
<script>
	var editorNocode = true ; //是否有未转码成功的视频_编辑器
	var editorNocodeField = true ; //是否有未转码成功的视频_字段
</script>
...
<script>
	$(function(){
	    $("#CalculateType").hide();
		//$("#field_CalculateType").after("\r\n<label><input type=\"button\" value=\"设置\" onclick=\"getusers()\" class=\"btn btn-primary tx-size-xs mg-l-10\"></label>");

		//$("#field_participant").after("\r\n<label><input type=\"button\" value=\"选择\" onclick=\"showUser()\" class=\"btn btn-primary tx-size-xs mg-l-10\"></label>");

		//$("#field_jixiao_permission").after("\r\n<label><input type=\"button\" value=\"选择\" onclick=\"getdevices()\" class=\"btn btn-primary tx-size-xs mg-l-10\"></label>");
	    $("#jixiao_permission").val("<%=userList%>");
		$("#Caption_enable").hide();
...
function showUser(){
    ...
    dialog.setUrl("../jixiao/user_index.jsp");
...
<%if(j==0){%>
			<div class="row flex-row align-items-center mg-b-15" id="tr_<%=field_title.getName()%>" >
			  ...
			  <label class="wd-content-lable d-flex" id="field_Title">
				<input class="form-control" placeholder="" type="text" id="Title" name="Title" size="80"  <%if(field_title.getName() == "CalculateType"){%>style="display:none"<%}%> value="<%=item!=null?Util.HTMLEncode(item.getValue("Title")):""%>" onkeyup="checkTitle();" style="border:none;">
			 </label>
