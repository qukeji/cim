...
<div class="br-mainpanel br-mainpanel-file " id="js-source">
    <iframe src="jixiao_look.jsp?" id="content_frame" style="width: 100%;height: 100%;" frameborder="0"
...
//=============================================================================================================
        ...
        $.ajax({
            type: "GET", dataType: "json", url: url, success: function (json) {
                var menu = $('.sidebar-menu');
                for (var i = 0; i < json.length; i++) {
                    var html = '';
                    if (json[i].arr && json[i].arr.length > 0) {
                        html = '<li class="treeview active">';
                    } else {
                        html = '<li class="active">';
                    }
                    html += '<a href="javascript:;" schemeid="' + json[i].id + '"><i class="fa fisrtNav fa-home" have="1"></i> <span>' + json[i].name + '</span></a>';

                    if (json[i].arr && json[i].arr.length > 0) {
                        html += '<ul class="treeview-menu menu-open">' + get_menu_html(json[i].arr) + '</ul>';
                    }
                    html += '</li>';
                    menu.append(html);
                    activeFn();
    ...
    function get_menu_html(json_) {
        var html = "";
        if (json_.length > 0) {
            for (var i = 0; i < json_.length; i++) {

                if (i == 0) {
                    html += '<li class="active"><a href="#" reportId="' + json_[i].reportId + '">';
                } else {
                    html += '<li class=""><a href="#" reportId="' + json_[i].reportId + '">';
                }
                html += '<i class="fa fa-angle-right" ></i>';
                html += '<span>' + json_[i].reportName + '</span></a>';
                html += '</li>';
            }
        }
    ...
    $.sidebarMenu = function (menu) {
        ...
        $(menu).on('click', 'li a', function (e) {
            ...
            var reportName = $(this).find("span").html();
            var schemeId = $(this).parent().parent().prev().attr("schemeid");
            var schemeName = $(this).parent().parent().prev().find("span").html();
            var $this = $(this);
            ...
            changeFrameSrc( window.frames["content_frame"] , "look_perfor_content.jsp?reportid="+reportid+"&schemeName="+schemeName+"&reportName="+reportName+"&schemeId="+schemeId);
