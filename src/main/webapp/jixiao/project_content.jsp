...
//当前登录用户绑定了租户,此频道下面是租户频道(聚融，大屏)
    ...
    String userIds = CmsCache.getParameter("jixiao_view_userid").getJson().getString("user_id");
    if (userIds.length() <= 0) {//没有配置可以看绩效方案的用户
        response.sendRedirect("../noperm.jsp");
    } else {
        int uid = userinfo_session.getId();
        boolean flag = false;//是否有权限访问,默认没有权限
        String[] arr = userIds.split(",");
        for (int i = 0; i < arr.length; i++) {
            int uid1 = Integer.parseInt(arr[i]);
            if (uid == uid1) {
                flag = true;
                break;
            }
...
<div class="br-mainpanel br-mainpanel-file" id="js-source">
    <div class="br-pageheader pd-y-15 pd-md-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
            <span class="breadcrumb-item active">工作绩效管理系统 / 绩效管理</span>
        </nav>
    </div>
    <!-- br-pageheader -->
        <%
...
%>
    ...
    <div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
        ...
        <div class="btn-group mg-l-auto hidden-sm-down">
            <%if (role != 3) {%>
            <a href="javascript:addDocument_before();" class="btn btn-outline-info">新建</a>
            <%}%>
            <a href="javascript:look_jixiao_before();" class="btn btn-outline-info">查看</a>
            <%if (role != 3) {%>
            <a href="javascript:editDocument1_before();" class="btn btn-outline-info">编辑</a>
            <%if (canDelete) {%>
            ...
            <table class="table mg-b-0 <%=listType==2?"table-fixed":""%>" id="content-table">
                ...
                <%
                    ...
                    while (Rs.next()) {
                        ...
                        int company = doc.getIntValue("CompanyId");
                        if (company != 0) {//租户用户
                            if (company != companyId) {//不能查看不是同组的记录
                                continue;
                            }
                        }
                ...
                <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>"
                    ...
                    <td class="hidden-xs-down"><%=AssessIndicator%>
                    </td>
                    ...
                    <td class="hidden-xs-down">
                        <div class='toggle-wrapper'>
                            <div class='toggle toggle-light primary' data-toggle-on='<%=enable%>' itemid='<%=id_%>'
                                 field='enable'></div>
                        </div>
                    </td>
                    <td class="dropdown hidden-xs-down">
                        <a href="javascript:look_jixiao_before('<%=id_%>');"
                           class="svg-icon fa-svg-icon svg-fa-check-circle" viewBox="0 0 512 512" title="预览"><i
                                class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
                        <a href="javascript:editDocument1_before('<%=id_%>');"
                           class="svg-icon fa-svg-icon svg-fa-check-circle" viewBox="0 0 512 512" title="编辑"><i
                                class="fa fa-pencil-square-o tx-18 handle-icon" aria-hidden="true"></i></a>
                        <a href="javascript:deleteFile1('<%=id_%>');"
                           class="svg-icon fa-svg-icon svg-fa-check-circle" viewBox="0 0 512 512" title="删除"><i
                                class="fa fa-trash-o tx-18 handle-icon" aria-hidden="true"></i></a>
                        <!-- dropdown-menu -->
                    </td>
                </tr>
                <%
                            ...
                            <div class="card bd-0">
                                ...
                                <div class="card-body bd-t-0 rounded-bottom">
                                    ...
                                    <div class="row mg-l-0 mg-r-0 mg-t-5">
                                        ...
                                        <%} else {%>
                                        <a href="javascript:Preview2_(<%=id_%>);" class="btn pd-0 mg-r-5" title="预览"><i
                                                class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
                                        <a href="javascript:Preview3_(<%=id_%>);" class="btn pd-0 mg-r-5"
                                           title="正式地址预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
                                        <%}%>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        $(function () {
            ...
            var menu = [
                ...
                <%}%>
                {
                    '<i class="fa fa fa-eye mg-r-5"></i>查看': function (menuItem, menu) {
                        look_jixiao_before();
                    }
                },
                {
                    '<i class="fa fa-edit mg-r-5 fa"></i>编辑': function (menuItem, menu) {
                        editDocument1_before();
                    }
                },
    ...
    <script>
        var role = <%=userinfo_session.getRole()%>;
        ...
        function addDocument_before() {
            if (role != 1) {
                TideAlert("提示", "不是系统管理员无法进行该操作！");
            } else {
                addDocument();
            }
        }
        ...
        function look_jixiao_before(id) {
            if (role != 1) {
                TideAlert("提示", "不是系统管理员无法进行该操作！");
            } else {
                look_jixiao(id);
		...
		function deleteFile(){
			var obj=getCheckbox();
			var message = "会删除方案下所有绩效数据，不可恢复，是否删除？";
			if(obj.length==0){
