...
int download		    =	getIntParameter(request,"download");
String ListSql="";
...
function downExcel(currpage)
{
	var url = pageName + "?currPage="+currpage+"&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>&download=1";
	this.location = url;
}
...
<div class="content_t1">
    ...
    <div class="content_new_post">
		<div class="top_button button_inline_block" onClick="downExcel(<%=currPage%>);" style="display:none;">
...
<div class="viewpane_c1" id="SearchArea" style="display:<%=(S_OpenSearch==1?"":"none")%>">
  ...
  <tr>
	...
	<!-- 图片新闻
		...
		<input type="submit" name="Submit" class="submit" class="tidecms_btn3" value="查找"><input type="hidden" name="OpenSearch" id="OpenSearch" value="1"></form></td>
    <td width="20"> </td>
  </tr>
</table>
...
<thead>
		<tr id="oTable_th">
					...
					<th class="v8"  align="center" valign="middle">定时发布日期</th>
    				<th class="v8"  align="center" valign="middle">最后修改日期</th>
...
ListSql = "select id,Title,OrderNumber,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,SetPublishDate,Status,Active,Category";
if(listType==2)
{
...
{   
	int id_ = Rs.getInt("id");
        int OrderNumber_data= Rs.getInt("OrderNumber");
       // System.out.println("OrderNumber_data....."+OrderNumber_data);
	...
	String SetPublishDate	= convertNull(Rs.getString("SetPublishDate"));
        if(!"".equals(SetPublishDate)){
            SetPublishDate = SetPublishDate+":00.0";
        }
	SetPublishDate=Util.FormatDate("yyyy-MM-dd HH:mm",SetPublishDate);
...
%>
  <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
	...
	<td class="v8" align="center" valign="middle"> <%=SetPublishDate%></td>
...
if(download==1){
	...
	ResultSet Rs3 = tu3.executeQuery(ListSql);
	...
	while(Rs3.next()){
		int itemid = Rs3.getInt("id");
		int globalid= Rs3.getInt("GlobalID");
		int userId = Rs3.getInt("User");
		UserInfo user = new UserInfo(userId);
		String userName = user.getName();
					Integer i=null;
		if(map.get(userName)!=null){
			 i =Integer.parseInt(String.valueOf(map.get(userName)));
		}
		if(i!=null){
	...
	if(true){
		...
		downmap =exceldriver.exportExcell("",userinfo_session.getId(),tomcatPath,sb+"");
		FileUtil fileutil = new FileUtil();
		String FolderName = "//cms/temp";
		String FileName = String.valueOf(downmap.get("excelname"));
		out.clear();out = pageContext.pushBody();
		fileutil.downloadFile(request,response,FolderName + "/" + FileName,"xls/downloadable",FileName,20);
   } 
}
*/  
%>
