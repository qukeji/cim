...
int task_status				=	getIntParameter(request,"task_status");
...
function popup()
{
	...
	var url='popup.jsp?itemid='+ obj.id+'&channelid=<%=ChannelID%>';
	var	dialog = new top.TideDialog();
		dialog.setWidth(600);
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,task_status,publisher_name,FROM_UNIXTIME(start) as start,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
if(task_status>0)
{
	WhereSql += " and task_status="+(task_status-1);
}
if(!S_Title.equals("")){
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-10 hidden-xs-down">
                ...
                <a href="javascript:list('task_status=1');" class="btn btn-outline-info list_task_status1">待审核</a>
                <a href="javascript:list('task_status=2');" class="btn btn-outline-info list_task_status2">审核通过</a>
                <a href="javascript:list('task_status=3');" class="btn btn-outline-info list_task_status3">毙稿</a>
                <a href="javascript:list('task_status=4');"class="btn btn-outline-info list_task_status4">已完成</a>
	...
	-->
				<%if(canDelete){%>
					<a href="javascript:deleteFile();" class="btn btn-outline-info"><font style="color:red;">删除</font></a>
				<%}%>
				<a href="javascript:popup();" class="btn btn-outline-info">选题详情</font></a>
			...
			<div class="dropdown hidden-md-1500-up hidden-sm-down mg-l-10">
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">					
						...
						<%if(IsWeight!=1){%>
						<a href="javascript:SortDoc1();" class="nav-link">排序</a>
						<%}%>
						<%if(IsTopStatus){%>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%}%>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">审核人</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120"	>执行人</th>
            				<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120"	>任务状态</th>
            				<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150"  >报题日期</th>
...
while(Rs.next())
{
	...
	String start	= convertNull(Rs.getString("start"));
	start=Util.FormatDate("yyyy-MM-dd HH:mm",start);

	int TopStatus = 0;

	int task_status_=Rs.getInt("task_status");
	String taskStatus_="";
	if(task_status_==0){
	  taskStatus_="未审核";
	}else if(task_status_==1){
	    taskStatus_="审核通过";
	}else if(task_status_==2){
	     taskStatus_="毙稿";
	}else if(task_status_==3){
	     taskStatus_="已完成";
	}
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<%
			    ...
			    int channelId1 = CmsCache.getParameter("daping").getJson().getInt("shenhejieguo");

			    String whereSql1 = " where Active=1 and parent="+GlobalID+" order by id desc limit 1";
			    //String sql="select * from channel where Active=1"
			   
                ArrayList<Document> arr1= ItemUtil.listItems(channelId1,whereSql1);
                String reviewer="";
                String review_date="";
                for(int j2=0;j2<arr1.size();j2++){
                	Document doc1 = arr1.get(j2);
                     reviewer = doc1.getValue("username");//审核人
...
<script>
	...
	$(function() {
		...
		if (taskStatus==1) {
			...
			$(".list_task_status1").addClass("active");

		} else if (taskStatus==2) {

			$(".list_task_status2").addClass("active");

		} else if (taskStatus==3) {

			$(".list_task_status3").addClass("active");

		} else if (taskStatus==4) {

			$(".list_task_status4").addClass("active");
...
*/			
		...
		var menu = [
			...
			{'<i class="fa fa-edit mg-r-5 fa"></i>查看详情':function(menuItem,menu) {popup(); }},
		<%if(!channel.getRecommendOut().equals("")){%>
