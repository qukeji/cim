...
String ListSql = "select id,Title,Photo,PublishDate,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">分类名称</th>
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-100">分类图标</th>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<%}else{%>
				<img src="<%=photoAddr%>" width="60px" height=""/>
			</td>
			<%}%>
			<td class="hidden-xs-down">
				<%=StatusDesc%>
