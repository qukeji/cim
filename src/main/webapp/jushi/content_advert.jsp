...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,onlinetime,Revoketime,position";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%}%>
							<th class="tx-12-force tx-mont tx-medium wd-120 wd-author">显示位置</th>
							<th class="tx-12-force tx-mont tx-medium wd-60">上线时间</th>
							<th class="tx-12-force tx-mont tx-medium wd-60">下线时间</th>
							...
							<%}%>
							<th class="tx-12-force wd-100 tx-mont tx-medium wd-150">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
while(Rs.next())
{
	...
	String onlinetime = convertNull(Rs.getString("onlinetime"));
	onlinetime=Util.FormatDate("yyyy-MM-dd HH:mm:ss",onlinetime);
	String Revoketime = convertNull(Rs.getString("Revoketime"));
	Revoketime=Util.FormatDate("yyyy-MM-dd HH:mm:ss",Revoketime);
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<%}%>
			<td class="hidden-xs-down"><%=position_%></td>
			<td class="hidden-xs-down"><%=onlinetime%></td>
			<td class="hidden-xs-down"><%=Revoketime%></td>
