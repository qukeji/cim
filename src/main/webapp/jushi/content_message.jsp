...
<script>
	...
	if (pageName == "") pageName = "content_message.jsp";
...
function approve2_message(id,status) {
	if(status == "1"){
		var	dialog = new top.TideDialog();
		dialog.showAlert("请选择草稿状态的文章进行发布。","danger");
		return;
	}
	approve_(id);
}
...
{
	var url= "../jushi/message_approvedocument.jsp?ItemID=" + id + Parameter;
	$.ajax({
		type: "GET",
		url: url,
...
function editDocument_message(id,status) {
	if(status == "1"){
		var	dialog = new top.TideDialog();
		dialog.showAlert("请选择草稿状态的文章进行编辑。","danger");
		return;
	}
	editDocument(id);
}
	...
	function getStatusDesc(){
		...
		jQuery("#content-table input:checked").each(function(i){
			...
			if(i==0)
				StatusDesc+=jQuery(obj).attr("status");
			else
				StatusDesc+=","+jQuery(obj).attr("status");
		});
...
function approveMessage(){
	...
	if(obj.length==0){
		var	dialog = new top.TideDialog();
		dialog.showAlert("请先选择要发布的文章。","danger");
		return;
	}else{
		if(StatusDesc.indexOf("1")!=-1){
			var	dialog = new top.TideDialog();
			dialog.showAlert("请选择草稿状态的文章进行发布。","danger");
			return;
		}
...
function editDocument_Message() {
	var obj = getCheckbox();
	var StatusDesc = getStatusDesc().StatusDesc;
	if(obj.length==0){
		var	dialog = new top.TideDialog();
		dialog.showAlert("请先选择要编辑的文章。","danger");
		return;
	}else if(obj.length>1){
		var	dialog = new top.TideDialog();
		dialog.showAlert("请选择一篇文章进行编辑。","danger");
		return;
	}else{
		if(StatusDesc.indexOf("1")!=-1){
			var	dialog = new top.TideDialog();
...
String ListSql = "select id,Title,Summary,Photo,Weight,GlobalID,PublishDate,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
%>
		...
		<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
			...
			<div class="btn-group mg-l-auto hidden-sm-down">
					...
					<a href="javascript:approveMessage();" class="btn btn-outline-info">发布</a>
					<a href="javascript:Preview_();" class="btn btn-outline-info">预览</a>
					<a href="javascript:editDocument_Message();" class="btn btn-outline-info">编辑</a>
			</div>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down ">消息标题</th>
...
while(Rs.next())
{
	...
	PublishDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Long.parseLong(PublishDate+"000"));
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
    				<a href="javascript:approve2_message('<%=id_%>','<%=Status%>');" class="btn pd-0 mg-r-5" title="发布"><i class="fa fa-cloud-upload tx-18 handle-icon" aria-hidden="true"></i></a>
					...
					<a href="javascript:editDocument_message('<%=id_%>','<%=Status%>');" class="btn pd-0 mg-r-5" title="编辑"><i class="fa fa-edit tx-18 handle-icon" aria-hidden="true"></i></a>
				</div>
				<!-- dropdown-menu -->
			</td>
		</tr>
...
<script>
	...
	$(function(){
		...
		var menu = [
			{'<i class="fa fa-search mg-r-5"></i>发布':function(menuItem,menu) {approveMessage();} },
			{'<i class="fa fa fa-eye mg-r-5"></i>预览':function(menuItem,menu) {Preview_(); }},
			{'<i class="fa fa-edit mg-r-5 fa"></i>编辑':function(menuItem,menu) {editDocument_Message(); }},
		];
