...
<%
	...
	int parentChannelId = channel.getParent();
...
<style>
	...
	#ShowBtnsPicsType img{min-height: 100px}
</style>
...
$.getScript("../common/controlfield_module.js");
...
//		TideAlert("提示","列表页显示位置不允许为空和0！");
//		return;
//	}
	if(serialNo=="s53_module_button"||serialNo=="s53_module_auto_recommend"){
		var title_ =  document.form.Title.value;
		var l = title_.length;
		var n = 0;
		for (var i=0;i<l;i++)
		{
			if (title_.charCodeAt(i)<0||title_.charCodeAt(i)>255)
				n++;
			else
				n = n + 0.5;
		}
		if(n>10){
			ddalert("标题字数不能大于10个!","(function dd(){getDialog().Close({suffix:'html'});})()");
			document.form.Title.focus();
			return false;
		}
		var MoreWindowIsCheck_ = $("input[type=radio][name=IsAddMoreButton]:checked").val();
		var MoreWindowText = $("#href").val();
		if(MoreWindowIsCheck_=="1"&&MoreWindowText==""){
			ddalert("请填写'更多'跳转页面!","(function dd(){getDialog().Close({suffix:'html'});})()");
			document.form.Title.focus();
			return false;
		}
		var list_num_ = $("#list_num").val();
		var regu = /^([1-9]||10)$/ig;
	    var re = new RegExp(regu);
	    if((!re.test(list_num_))||list_num_==""){
	    	ddalert("列表数据条数为1-10.","(function dd(){getDialog().Close({suffix:'html'});})()");
	       return false; 
	    }
	}
...
<%for (int i = 0; i < fieldGroupArray.size(); i++){
	...
	fieldGroup_name = fieldGroup.getName();
	%>
	<div class="br-sideleft-menu">
		<a href="javascript:showTab('<%=i+1%>')" <%=(i==0?"class='br-menu-link active'":"class='br-menu-link'")%> id="form<%=i+1%>_td" groupid="<%=fieldGroup.getId()%>" data-toggle="tooltip" data-placement="right" title="<%=fieldGroup.getName()%>">
			<div class="br-menu-item">
				...
				<span class="menu-item-label"><%=fieldGroup_name%></span>
			</div><!-- menu-item -->
		</a><!-- br-menu-link -->
...
<script>
	...
	$(function(){
		var _tr_showType = $("#tr_ShowType");
		var _Caption_ShowType = $("#Caption_ShowType");
		_Caption_ShowType.find(".d-flex.align-items-center.tx-gray-800").append('<font id="clicktosee" class="tx-danger cursor-pointer mg-l-10">点击查看示例图</font>');
		var exPicsHtml = '<div class="row flex-row align-items-top mg-t--10" id="ShowBtnsPicsType" style="display:none;">'+
			'<label class="left-fn-title wd-150"> </label>'+
			'<label class="d-flex align-items-center wd-content-lable ">'+
				'<label class="wd-25p mg-r-20"><img class="wd-100p" src="/tcenter/img/2019/btn-group-example1.jpg" /></label>'+
				'<label class="wd-25p"><img class="wd-100p" src="/tcenter/img/2019/btn-group-example2.jpg" /></label>'+
			'</label>'+
		'</div>' ;
		_Caption_ShowType.after( $(exPicsHtml) )
		$("#clicktosee").on("click",function(){
			$("#ShowBtnsPicsType").toggle(0)
		})
	})
