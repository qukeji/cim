...
<%
	...
	//用户注册信息频道id
	int Channel_register = CmsCache.getChannel("register").getId();
%>
<!DOCTYPE html>
<html>
<head>
...
{ 
	...
	$.ajax({
         type: "POST",
         url:"../jushi/document_message_submit.jsp",
         data:$('#form').serialize(),
		 dataType:'json',
...
<form name="form" action="../jushi/document_message_submit.jsp" method="post" id = "form">
...
$(function() {
	...
	$(function(){
		...
		$("#tr_users").css("display","none");
		var val=$('input:radio[name="target"]:checked').val();
		if(val==1){
			$("#tr_users").css("display","");
		}
		//切换
	...
	function getUserid(){
		var url = "../content/company_user_index.jsp?ChannelID=<%=Channel_register%>&fieldName=users&type=1";
		var	dialog = new top.TideDialog();
