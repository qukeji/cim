...
if(CmsCache.hasValidLicense()) diff = 1000000;
 ...
 if(childGroup==1){
    ...
    dir="interactive";	   
	   ...
	   dir = request.getServletPath().substring(1,request.getServletPath().lastIndexOf("/"));
   }
%>
<!DOCTYPE html>
<html>
...
//=============================================================================================================
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				for(var i=0;i<jsonLength;i++){
					...
					{			
                        if(childGroup>0){                   				
						html += '<ul class="treeview-menu menu-open">' + get_menu_html(json[i],childGroup) + '</ul>';
						}else{
		...
		function get_menu_html(json,childGroup)
		{
			var html = "";
			var childNum=childGroup - 1;
			if(json.child && json.child.length>0)
			{										
				var json_ = json.child;
				for(var i=0;i<json_.length;i++)
				{								
				    if(childGroup>0 && childNum==i){
...
//				$(this).addClass("clicked");//设置不可重复点击
    ...
    <script>
    	$(function(){
			...
			var activeChannelId  = activeChannel.attr("channelid") ;
			activeChannelId[0].click();
    	})
    </script>
</body>
</html>