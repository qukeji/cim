...
<body class="collapsed-menu email" id="withSecondNav" onLoad="init();">
    ...
    <div class="br-mainpanel with-second-nav br-mainpanel-file " id="js-source">
        <iframe src="setup2018.jsp?SiteId=53" id="content_frame" style="width: 100%;height: 100%;"  frameborder="0" onload="changeFrameHeight(this)"></iframe>
    </div><!-- br-mainpanel -->
	...
	<script>
        ...
        $(function(){
		'use strict';
        $(".menu-publish").addClass("active");
        $(".menu-system").removeClass("active");
        $("#system-menu-cms-publish").addClass("active");
        $("#system-menu-publish").addClass("active");
			...
			$(document).on('mouseover', function(e){
			  ...
			  if($('body').hasClass('collapsed-menu')) {
				...
				if(targ) {
                  ...
                  $("#system-menu-publish").css("display","block");
				} else {
				  ...
				  $("#system-menu-publish").css("display","none");
				}
			  }
			});
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				for(var i=0;i<json.length;i++){
					...
					if( json[i].load==1 || (json[i].child!=null && json[i].child.length>0) ){
					    html += '<a href="javascript:;" SiteId="'+json[i].id+'" load="'+json[i].load+'" type="'+json[i].type+'"><i class="fa fisrtNav fa-home " have="1"></i> <span>'+json[i].name+'</span></a>';
                     }else{
						html += '<a href="javascript:;" SiteId="'+json[i].id+'" load="'+json[i].load+'" type="'+json[i].type+'"><i class="fa fisrtNav fa-home " have="0"></i> <span>'+json[i].name+'</span></a>'; 
				     }
					if(json[i].child && json[i].child.length>0)
					{
...
//========================================================================
		$.sidebarMenu = function(menu) {
			...
			$(menu).on('click', 'li a', function(e) {
				...
				if(type==1){
					changeFrameSrc( window.frames["content_frame"] , "../publish/setup2018.jsp?SiteId="+SiteId )						
				}else if(type==2){
					changeFrameSrc( window.frames["content_frame"] , "../publish/to_be_published2018.jsp?SiteId="+SiteId )						
				}else if(type==3){
					changeFrameSrc( window.frames["content_frame"] , "../publish/published2018.jsp?SiteId="+SiteId )						
				}
