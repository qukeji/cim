...
String searchTitle="to_be_published2018.jsp?rowsPerPage="+rowsPerPage+queryString1;
String endSearch="to_be_published2018.jsp?rowsPerPage="+rowsPerPage+queryString1;
if(Action.equals("Del"))
{
   ...
   // System.out.println("clear sql :"+Sql);
	...
	response.sendRedirect("to_be_published2018.jsp?currPage="+currPage+"&rowsPerPage="+rowsPerPage+queryString);return;
}
else if(Action.equals("Clear"))
{
	...
	*/
	...
	response.sendRedirect("to_be_published2018.jsp?a=b"+queryString);return;
}
...
function ClearItems()
{
	var url="publish_clear2018.jsp?queryString=<%=queryString%>&currPage=<%=currPage%>&rowsPerPage=<%=rowsPerPage%>";
...
/*	if(confirm("确实要清空所有待发布记录吗？")) 
	{
		this.location = "to_be_published2018.jsp?Action=Clear+queryString";
	}
*/	
}
function change(obj)
{
	if(obj!=null)
		this.location = "to_be_published2018.jsp?rowsPerPage="+obj.value+"<%=queryString%>";
}

function PublishItems()
{
	
	var url="publish_publish2018.jsp?queryString=<%=queryString%>&SiteId=<%=SiteId%>&currPage=<%=currPage%>&rowsPerPage=<%=rowsPerPage%>";
	var	dialog = new top.TideDialog();
		dialog.setWidth(320);
		dialog.setHeight(240);
		dialog.setUrl(url);
		dialog.setTitle("发布待发布任务");
		dialog.show();	
	
/*	if(confirm("确实要发布所有待发布记录吗？")) 
	{
		this.location = "to_be_published2018.jsp?Action=Publish+queryString";
	}	
	*/
}
...
function MM_jumpMenu(obj){ //v3.0
	if(obj!=null)
	{
		if(document.all("num_perpage")!=null)
			this.location = "to_be_published2018.jsp?Scheme=" + obj.value+"&rowsPerPage="+document.all("num_perpage").value+"&SiteId=<%=SiteId%>";
		else
			this.location = "to_be_published2018.jsp?Scheme=" + obj.value+"&SiteId=<%=SiteId%>";
	}
}
...
function DeleteItems2(id,currPage,rowsPerPage,queryString)
{
	
  //原方法alert 是删除之后回显到删除数据所在的分页
	var url="publish_delete2_2018.jsp?id="+id+"&currPage="+currPage+"&rowsPerPage="+rowsPerPage+"&queryString="+queryString;
	var	dialog = new top.TideDialog();
		dialog.setWidth(320);
		dialog.setHeight(260);
		dialog.setUrl(url);
		dialog.setTitle("删除待发布任务");
		dialog.show();	
}	
...
function DeleteItems(){
	...
	if(ids!="")
{
	var url="publish_delete2018.jsp?ItemID="+ids;
	var	dialog = new top.TideDialog();
		dialog.setWidth(320);
		dialog.setHeight(260);
		dialog.setUrl(url);
...
{
	var url = "to_be_published2018.jsp?currPage=" + currpage + "&rowsPerPage=<%=rowsPerPage%><%=queryString%>";
	this.location = url;
}
</script>
...
<div class="br-mainpanel br-mainpanel-file" id="js-source">  
	...
	<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
		...
		<div class="btn-group mg-l-10 hidden-xs-down">
		  <a href="javascript:PublishItems();" class="btn btn-outline-info list_draft" >发布</a>       
        </div><!-- btn-group -->
		<div class="btn-group mg-l-10 hidden-xs-down">
		  <a href="javascript:ClearItems();" class="btn btn-outline-info list_draft" >清空</a>       
        </div><!-- btn-group -->
		<div class="btn-group mg-l-10 hidden-xs-down">
		  <a href="javascript:DeleteItems();" class="btn btn-outline-info list_draft" >删除</a>       
        </div><!-- btn-group -->
<%
...
<tr>
            <td class="valign-middle">
			  <label class="ckbox mg-b-0">
				<input name="checkbox_id" value="<%=id%>" id="checkbox_id" type="checkbox"/><span></span>
			  </label>
			</td>
			<td class="valign-middle">
			  <label class="ckbox mg-b-0">
		   ...
		   <td class="hidden-xs-down"><a href="#" class="btn btn-danger btn-sm tx-13" onclick="DeleteItems2(<%=id%>,<%=currPage%>,<%=rowsPerPage%>,'<%=queryString%>');">删除</a></td>
		</tr>                          
<%
			}			
}
...
<!--分页-->
			<div id="tide_content_tfoot">	
			    <span class="mg-r-20 " id="selectAll" onClick="$('input[id=checkbox_id]').attr('checked','checked');">全部</span><span class="mg-r-20 " id="selectNo" onClick="$('input[id=checkbox_id]').attr('checked','');">无</span>
...
jQuery(document).ready(function(){
	...
	jQuery("#goToId").click(function(){
		...
		if(num<1)
			num=1;
		var href="to_be_published2018.jsp?currPage="+num+"&rowsPerPage=<%=rowsPerPage%><%=queryString%>";
