...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,sourceUrl";
...
while(Rs.next())
{
	...
	String sourceUrl	= convertNull(Rs.getString("sourceUrl"));
	...
	{
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" sourceUrl="<%=sourceUrl%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}%>
				<%if(active==1){%>
					<a href="javascript:Preview2_(0,'<%=sourceUrl%>');" class="btn pd-0 mg-r-5" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
				<%}%>
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  ...
					  <div class="row mg-l-0 mg-r-0 mg-t-5">				                  					                  
						  ...
						  <%}else{%>
						  <a href="javascript:Preview2_(<%=sourceUrl%>);" class="btn pd-0 mg-r-5" title="预览"><i class="fa fa-search tx-18 handle-icon" aria-hidden="true"></i></a>
						  <%}%>
					  </div>
					</div>
				  </div>
...
<script>
	...
	function Preview2_(id,sourceUrl)
	{
		if(typeof(sourceUrl)=="undefined"&&id!=0){
			sourceUrl = $("#item_"+id).attr("sourceUrl");
		}
