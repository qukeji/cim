...
<%!
   ...
   public JSONArray listChannel_JS(int id, String s, UserInfo user, int channelnum) throws JSONException,MessageException,SQLException{
		...
		if ((childs != null) && (childs.size() > 0)) {
			...
			for (int i = 0; i < childs.size(); ++i) {
				...
				if (user.hasChannelShowRight(channelid)) {
                    ...
                    JSONArray xx=listChannel_JS1(channelid, varName, user, channelnum);
                    o.put("child",xx);
	 ...
	 public JSONArray listChannel_JS1(int id, String s, UserInfo user, int channelnum) throws JSONException,MessageException,SQLException{
		...
		if ((childs != null) && (childs.size() > 0)) {
			...
			for (int i = 0; i < childs.size(); ++i) {
				...
				if(user.getCompany()!=0&&user.getCompany()!=child.getCompany()){
					
					continue;
				}
				System.out.println("租户："+(user.getCompany()!=0&&user.getCompany()!=child.getCompany()));
