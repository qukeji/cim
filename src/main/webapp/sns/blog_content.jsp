...
function deleteFile(){
	...
	if(obj.length==0){
		alert("请先选择要删除的文件！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=1&table=uchome_blog&c_id=blogid";
			  $.ajax({
				 type: "GET",
				 url: url,
...
function deleteFile2(){
	...
	if(obj.length==0){
		alert("请先选择要撤稿的文档！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=3&table=uchome_blog&c_id=blogid";
			  $.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
{
	 var url= "sns_operate_action.jsp?id=" + id + "&action=2&table=uchome_blog&c_id=blogid";
		$.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
function Preview22(id, uid){
	window.open("<%=domainname%>space.php?uid="+uid+"&do=blog&id="+id);
}
...
function double_click()
{
	jQuery("#oTable .tide_item").dblclick(function(){
		...
		obj.trigger("click");
		window.open("<%=domainname%>cp.php?ac=blog&blogid=" + id + "&op=edit");
	});
}
</script>
</head>
