...
}catch(Exception e)
{
	...
	{
		out.println("<font size=3 color=red><b>连接不上数据源，请检查是否正确配置.</b></font><br><br>");
	}
}
...
%>
  <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" id="item_<%=id_%>" class="tide_item">
	...
	<a href="http://sns.cutv.com/space.php?uid=<%=authorid%>" target="_blank"><%=author%></a><!-- 这里的链接是用户的sns链接，如http://sns.cutv.com/space.php?uid=<authorid> --><br /><%=IP%>
	</td>
