...
function deleteFile(){
	...
	if(obj.length==0){
		alert("请先选择要删除的文件！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=1&table=uchome_event&c_id=eventid";
			  $.ajax({
				 type: "GET",
				 url: url,
...
function deleteFile2(){
	...
	if(obj.length==0){
		alert("请先选择要撤稿的文档！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=3&table=uchome_event&c_id=eventid";
			  $.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
{
	 var url= "sns_operate_action.jsp?id=" + id+ "&action=2&table=uchome_event&c_id=eventid";
		$.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
function Preview22(id){
	window.open("<%=domainname%>space.php?do=event&id=" + id);
}
...
function recommendOut()
{
	...
	*/
	...
	dialog.setUrl("sns/sns_recommend_out_index.jsp?ChannelID="+13960+"&ids="+id+"&sns_tablename=<%=tableprefix%>_event&userid="+<%=userid%>);
	dialog.setTitle("推荐");
	dialog.show();	
}
...
String ListSql = "select FROM_UNIXTIME(dateline) as dateline,eventid,username,uid,title,status from "+tableprefix+"_event";
String CountSql = "select count(*) from "+tableprefix+"_event";
...
//http://tv.sxrtv.com:81/sns/space.php?uid=1&do=podcast&picid=154
	...
	{
%>
		<tr No="<%=j%>"  id="item_<%=id_%>" id_="<%=id_%>"  class="tide_item">
...
var page={id:'<%=id%>',currPage:'<%=currPage%>',rowsPerPage:'<%=rowsPerPage%>',querystring:'<%=querystring%>',TotalPageNumber:<%=TotalPageNumber%>};
		...
		<%if(TotalPageNumber>0){%> 
			...
			<div id="tide_content_tfoot">
				...
				<div class="each-page-num mg-l-20 ">
          			<span class="">每页显示:</span>
          			<select name="rowsPerPage" class="form-control select2 wd-80" data-placeholder="" onChange="changeRowsCols();" id="rows">
						<option value="10">10</option>
						<option value="15">15</option>
						<option value="20">20</option>
						<option value="25">25</option>
				...
				<div class="each-page-num mg-l-20 ">
          			<span class="">行:</span>
          			<select name="rowsPerPage" class="form-control select2 wd-80" data-placeholder="" onChange="changeRowsCols();" id="cols">
