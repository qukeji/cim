...
function deleteFile(){
	...
	if(obj.length==0){
		alert("请先选择要删除的文件！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=1&table=uchome_feed&c_id=feedid";
			  $.ajax({
				 type: "GET",
				 url: url,
...
function deleteFile2(){
	...
	if(obj.length==0){
		alert("请先选择要撤稿的文档！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=3&table=uchome_feed&c_id=feedid";
			  $.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
{
	 var url= "sns_operate_action.jsp?id=" + id + "&action=2&table=uchome_feed&c_id=feedid";
		$.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
function Preview22(uid){
	window.open("<%=domainname%>space.php?uid=" + uid);
}
...
function recommendOut()
{
	...
	dialog.setUrl("sns/sns_recommend_out_index.jsp?ChannelID="+13962+"&ids="+id+"&sns_tablename=<%=tableprefix%>_blog&userid="+<%=userid%>);
	dialog.setTitle("推荐");
	dialog.show();	
}
...
function double_click()
{
	jQuery("#oTable .tide_item").dblclick(function(){
		...
		obj.trigger("click");
		window.open("<%=domainname%>admincp.php?ac=feed&op=edit&feedid" + id);
	});
}
</script>
</head>
...
String ListSql = "select FROM_UNIXTIME(dateline) as dateline,feedid,username,uid,status,title_template from "+tableprefix+"_feed";
String CountSql = "select count(*) from "+tableprefix+"_feed";
...
<%if(listType==1){%>
				<thead>
				  <tr>
					...
					<th class="tx-12-force tx-mont tx-medium">用户动态(feed)</th>
...
while(Rs!=null && Rs.next())
{
	...
	ResultSet rs2 = tu2.executeQuery(sql2);
	tu2.closeRs(rs2);
	String title_template = convertNull(Rs.getString("title_template"));
    title_template = title_template.replace("{actor}","");
	String dateline	= convertNull(Rs.getString("dateline"));
	dateline=Util.FormatDate("yyyy-MM-dd HH:mm:ss",dateline);
	long now = System.currentTimeMillis();
    long s = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse(dateline).getTime();
	int k = 0;
	if((now-s)<= Integer.MAX_VALUE){
	k =(int)(now - s);
	}
	int b=0;
	String c = "";
	if(k/1000 < 60){
	 b= (int)(k/1000);
     c = b+"秒前";
	}else if(k/1000 >60 && k/1000/60 <60){
	 b=(int)k/1000/60;
	 c = b+"分钟前";
	}else if(k/1000/60/60 >0 && k/1000/60/60 <24 ){
	 b=(int)k/1000/60/60;
	 c = b+"小时前";
	}else if(k/1000/60/60/24 >0){
	 c =dateline;
	}
...
%>
		<tr No="<%=j%>"  id="item_<%=id_%>" id_="<%=id_%>"  uid="<%=uid%>" username="<%=username%>" class="tide_item">
			...
			<img id="img_<%=j%>" src="<%=domainname%>image/icon/profile.gif"/><%=username%><%=title_template%>&nbsp<%=c%>
			</td>
			<td class="hidden-xs-down"><%=StatusDesc%></td>
			<td class="hidden-xs-down"><%=dateline%></td>
			<td class="dropdown hidden-xs-down">			
			<a href="javascript:Preview22(<%=uid%>);" class="nav-link">正式地址预览</a>
			</td>
		</tr>
  <%}
}
