...
function deleteFile(){
	...
	if(obj.length==0){
		alert("请先选择要删除的文件！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=1&table=uchome_thread&c_id=tid";
			  $.ajax({
				 type: "GET",
				 url: url,
...
function deleteFile2(){
	...
	if(obj.length==0){
		alert("请先选择要撤稿的文档！");
		 ...
		 if(confirm(message)){
			 var url="sns_operate_action.jsp?id=" + obj.id+"&action=3&table=uchome_thread&c_id=tid";
			  $.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
{
	 var url= "sns_operate_action.jsp?id=" + id +"&action=2&table=uchome_thread&c_id=tid";
		$.ajax({
				 type: "GET",
				 url: url,
				 success: function(msg){document.location.href=document.location.href;}   
...
function Preview22(id){
	window.open("<%=domainname%>space.php?do=thread&id="+id);
}
...
function recommendOut()
{
	...
	dialog.setUrl("sns/sns_recommend_out_index.jsp?ChannelID="+13958+"&ids="+id+"&sns_tablename=<%=tableprefix%>_thread&userid="+<%=userid%>);
	dialog.setTitle("推荐");
	dialog.show();	
}
...
function double_click()
{
	jQuery("#oTable .tide_item").dblclick(function(){
		...
		obj.trigger("click");
		window.open("<%=domainname%>cp.php?ac=thread&op=edit&pid=" + id + "&tagid=" + tagid);
	});
}
</script>
</head>
...
String ListSql = "select FROM_UNIXTIME(dateline) as dateline,tid,username,uid,subject,tagid,status from "+tableprefix+"_thread";
String CountSql = "select count(*) from "+tableprefix+"_thread";
...
<%if(listType==1){%>
				<thead>
				  <tr>
					...
					<th class="tx-12-force tx-mont tx-medium">话题名称</th>
...
while(Rs!=null && Rs.next())
{
	...
	{		
%>
		<tr No="<%=j%>"  id="item_<%=id_%>" id_="<%=id_%>"  uid="<%=uid%>" tagid="<%=tagid%>" username="<%=username%>"  class="tide_item">
			...
			<a href="javascript:Preview22(<%=id_%>);" class="nav-link">正式地址预览</a>
			</td>
		</tr>
<%}
}
...
var page={id:'<%=id%>',currPage:'<%=currPage%>',rowsPerPage:'<%=rowsPerPage%>',querystring:'<%=querystring%>',TotalPageNumber:<%=TotalPageNumber%>};
		...
		<%if(TotalPageNumber>0){%> 
			...
			<div id="tide_content_tfoot">
				...
				<div class="each-page-num mg-l-20 ">
          			<span class="">每页显示:</span>
          			<select name="rowsPerPage" class="form-control select2 wd-80" data-placeholder="状态" onChange="changeRowsCols();" id="rows">
						<option value="10">10</option>
						<option value="15">15</option>
						<option value="20">20</option>
						<option value="25">25</option>
				...
				<div class="each-page-num mg-l-20 ">
          			<span class="">行:</span>
          			<select name="rowsPerPage" class="form-control select2 wd-80" data-placeholder="状态" onChange="changeRowsCols();" id="cols">
