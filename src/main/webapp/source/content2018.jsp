...
%>
	...
	<div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
		...
		<%}%>
		  ...
		  <a href="javascript:RefreshItem();" class="btn btn-outline-info">刷新Cache</a>
		<%if(canDelete){%>
...
<%if(listType==1){%>
				<thead>
				  <tr>
					...
					<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150 ad-author">作者</th>
					<%if(IsComment==1){%>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td ondragstart="OnDragStart(event)">
			  <i class="fa fa-newspaper-o tx-16 tx-primary" aria-hidden="true"></i>
			  ...
			  <%}else{%>
				...
				<div class="dropdown-menu dropdown-menu-right pd-10">
					<nav class="nav nav-style-1 flex-column">
						...
						<%}%>
						<a href="javascript:RefreshItem();" class="nav-link">刷新Cache</a>
						<%if(canDelete){%>
			...
			<div class="row">
				...
				<div class="card bd-0">
					...
					<div class="card-body bd-t-0 rounded-bottom">
					  <p class="card-text"><%=Title%>(<%=StatusDesc%>)</p>
