<%@ page import="tidemedia.cms.system.*,java.util.Date,tidemedia.cms.spider.*,tidemedia.cms.util.*,
				tidemedia.cms.user.*"%>
...
<body class="collapsed-menu email" id="withSecondNav" onLoad="init();">
    ...
    <div class="br-mainpanel with-second-nav br-mainpanel-file " id="js-source">
        <iframe src="list2018.jsp" id="content_frame" style="width: 100%;height: 100%;"  frameborder="0" onload="changeFrameHeight(this)"></iframe>
    </div><!-- br-mainpanel -->
...
//=========================================================================================
		...
		function menu_init(type,obj){
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				var html = '<li class="treeview"><a href="#" load="0" GroupID=""><i class="fa fisrtNav fa-home" have="1"></i> <span>采集</span></a><ul class="treeview-menu" style="display: block;">';
				for(var i=0;i<json.length;i++){
				 	
					if( json[i].load==1 || (json[i].child!=null && json[i].child.length>0) ){
					  html += '<li><a href="#" load="'+json[i].load+'" GroupID="'+json[i].id+'"><i class="fa fa-angle-double-right" have="1"></i> <span>'+json[i].name+'</span></a>';
                    }else{
					  html += '<li><a href="#" load="'+json[i].load+'" GroupID="'+json[i].id+'"><i class="fa fa-angle-right" have="0"></i> <span>'+json[i].name+'</span></a>';
				     }
				...
				if(type==1){
					...
					changeFrameSrc( window.frames["content_frame"] , "../spider/list2018.jsp" )
				}
			}});
		}
...
//========================================================================
		...
		$.sidebarMenu = function(menu) {
			...
			$(menu).on('click', 'li a', function(e) {
                ...
                changeFrameSrc( window.frames["content_frame"] , "../spider/list2018.jsp?GroupID="+GroupID)	
		...
		function newGroup()
		{
			...
			var dialog = new top.TideDialog();
				...
				dialog.setUrl("group_add.jsp?GroupID="+GroupID);
				dialog.setTitle("新建组");
				dialog.show();
		}
		function editGroup()
		{
			...
			var	dialog = new top.TideDialog();
				dialog.setWidth(500);
				dialog.setHeight(300);
			dialog.setUrl("group_edit.jsp?GroupID="+GroupID);
				dialog.setTitle("编辑组");
				dialog.show();
		}
		function Order()
		{
			...
			if (getActiveNav()){ 
				...
				dialog.setUrl("group_order.jsp?GroupID="+GroupID);
				dialog.setTitle("排序");
				dialog.show();
			...
			if (getActiveNav()){ 
				...
				if(confirm("确实要删除\"" + GroupName + "\"吗?\r\n")) 
				{
					$.ajax({
						...
						top.TideDialogClose({refresh:'left',returnNavValue:{group:'"+SuperGroup+"',SuperGroup:"+parentGroup.getParent()+"}});
						}
					});
				}
