...
//弹窗确定事件
var totalSubmit = true ;  //是否是搜索项目的整体提交
                ...
                for (var i = 0; i < d.data.length; i++) {
                    if (d.data[i].status == 1) {
                        ...
                        curItem += '</div>' +
                            ...
                            '<i class="icon ion-android-create tx-20 valign-middle icon-color list-menu-edit"></i>'+
                            '</span>' +
...
$('.left-menu').on('click', '.menu-delete', function () {
    ...
    var menuAddBtn = '<i class="icon ion-android-create tx-20 valign-middle icon-color list-menu-edit"></i><i class="icon ion-android-add-circle tx-16 menu-add"></i>';
...
$('.right-menu').on('click', '.menu-add', function () {
    var listShow = '<span class="list-show float-right">' +
        '<span>列表显示</span>' +
        '<div class="toggle-wrapper ">' +
...
//点击编辑功能菜单项目
$("body").delegate(".list-menu-edit", "click", function () {
