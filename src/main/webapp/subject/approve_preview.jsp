...
<%
    ...
    int task_status=item.getIntValue("task_status");
...
<head>
    ...
    <script>
        ...
        function Save(action){
            ...
            var approveendid = $("#approve_end").attr("end");
            if(endApprove!=1){
                ...
                if(user==1&&actionMessage==""){
                    alert("请输入驳回理由");
                    return false ;
                }
            }
            var url= "task_approve_submit.jsp?globalid="+GlobalID+"&title="+title+"&action="+action+"&user="+user+"&approveId="+approveId+"&endApprove="+endApprove+"&actionMessage="+actionMessage+"&approveendid="+approveendid;
            $.ajax({
                type: "GET",
                url: url,
