...
function getRadio(){
	...
	result = $("#item_"+id).attr("subjectname");

	result2 = $("#item_"+id).attr("subjectid");
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,users_id,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
while(Rs.next())
{
	...
	String users_id	= convertNull(Rs.getString("users_id"));
	...
	{
%>
		 <tr No="<%=j%>" ItemID="<%=id_%>" contentHref="<%=contentHref%>" subjectid="<%=GlobalID%>" users_id="<%=users_id%>" subjectname="<%=Title%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
