...
<%
    ...
    int type1 = getIntParameter(request,"type");
    String globalids="";
    if(currPage<1)
        currPage = 1;
...
<head>
    ...
    <script>
        ...
        {
            window.open("project_preview.jsp?ItemID="+id+"&ChannelID=<%=ChannelID%>");
        }
        //预览
        function Preview()
        {
        ...
        {
            window.open("project_preview.jsp?ItemID=" + id + Parameter);
        }
...
//System.out.println("time:"+weightTime);
        ...
        String ListSql
                ="select id,Title,Photo,start_time,end_time,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
if(S_IsIncludeSubChannel==1)
{
    WhereSql += " and ChannelCode like '"+channel.getChannelCode() + "%'";
        ...
        if (time==1)
        {
            WhereSql += " and start_time>'"+nowtime+"'";
        }
        if (time==2)
        {
            WhereSql +=
    ...
    <div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
        ...
        <div class="btn-group mg-l-10 hidden-xs-down">
            ...
            <a href="javascript:list('time=1');" class="btn btn-outline-info list_draft">未开始</a>
            <a href="javascript:list('time=2');" class="btn btn-outline-info list_publish">进行中</a>
            <a href="javascript:list('time=3');" class="btn btn-outline-info list_submit">已结束</a>
            ...
            <form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
                <div class="row">
                    ...
                    <div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
                        <select class="form-control select2" data-placeholder="状态" name="time">
                            <option label="Choose one"></option>
                            <option value="0">全部</option>
                            <option value="1" <%=(time==1?"selected":"")%>>未开始</option>
                            <option value="2" <%=(time==2?"selected":"")%>>进行中</option>
                            <option value="3" <%=(time==3?"selected":"")%>>已结束</option>
                        </select>
                    </div>
        ...
        <div class="card bd-0 shadow-base">
            <table class="table mg-b-0 <%=listType==2?"table-fixed":""%>" id="content-table">
                ...
                <tr>
                    ...
                    <%}%>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-250">开始时间</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-350">结束时间</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">状态</th>

                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">立项人</th>
                    <%if(IsComment==1){%>
                ...
                <%
                    ...
                    while(Rs.next())
                    {
                        ...
                        String start_time = convertNull(Rs.getString("start_time"));
                        start_time=Util.FormatDate("yyyy-MM-dd HH:mm",start_time);
                        String end_time = convertNull(Rs.getString("end_time"));
                        end_time=Util.FormatDate("yyyy-MM-dd HH:mm",end_time);
                        String UserName = CmsCache.getUser(user).getName();//convertNull(Rs.getString("userinfo.Name"));
                        String StatusDesc = "";
                        String StatusDesc2 = "";

                        String nowtime2=df.format(new Date());
                        if (nowtime2.compareTo(start_time)<0){
                            StatusDesc2="未开始";
                        }else if (nowtime2.compareTo(end_time)>0){
                            StatusDesc2="已结束";
                        }else {
                            StatusDesc2="进行中";
                        }
                ...
                <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
                    ...
                    <td class="dropdown hidden-xs-down">
                        <a href="javascript:Previewdtial('<%=id_%>');" class="svg-icon fa-svg-icon svg-fa-check-circle" viewBox="0 0 512 512" title="预览"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
                        ...
                        <div class="dropdown-menu dropdown-menu-right pd-10">
                            <nav class="nav nav-style-1 flex-column">
                                ...
                                <a href="javascript:Previewdtial(<%=id_%>);" class="nav-link">预览</a>
                                <%}%>
                                <%if(active==0 && canDelete){%>
                                <a href="javascript:resume(<%=id_%>);" class="nav-link">恢复</a>
                                <%}else{%>
                                <a href="javascript:editDocument1(<%=id_%>);" class="nav-link">编辑</a>
