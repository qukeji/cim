...
<%
    ...
    int		project_gid	=	getIntParameter(request,"project_gid");//项目管理id
    int		project_channeldid	=	getIntParameter(request,"LinkChannelID");//项目频道id
    int fieldgroup=2;
    ...
    String sql = "select id from channel where company = "+userinfo_session.getCompany() +" and parent = "+bespeaks;
    TableUtil tableUtil = new TableUtil();
    ResultSet resultSet = tableUtil.executeQuery(sql);
    int Category = 0;
...
<head>
    ...
    <script>
        ...
        var timer = null;//定时器
        var ChannelID = "<%=ChannelID%>";
        var project_channeldid="<%=project_channeldid%>";
        var project_gid="<%=project_gid%>";
        ...
        function SaveApprove(action){
            if(action==1){
                if($("#Image1Href").hasClass("dis")){
                    return false;
                }
                $("#Image1Href").addClass("dis");
            }
            if(action==0){
                if($("#Image2Href").hasClass("dis")){
                    return false;
                }
                $("#Image2Href").addClass("dis");
            }
            ...
            $.ajax({
                ...
                success: function(msg){
                    $("#Image1Href").removeClass("dis");
                    $("#Image2Href").removeClass("dis");
                    if(msg.trim()!=""){
                        alert(msg.trim());
                    }else{
                        if(action==0){
    ...
    <script>
        ...
        function Save_approve()
        {
            ...
            document.form.task_status.value = 0;
            Save();
        }
        ...
        function Save()
        {
            if(check())
            {
                ...
                if(GlobalID==0)
                {
                    ...
                    $.getJSON("../content/document_getnew.jsp?id="+channelid, function(res){
                        itemid = res.itemid;
                        GlobalID = res.globalid;
                        console.log(top);
                        /*	$(top).find("#iframe2").document.form.GlobalID.value = res.globalid;
                            $(top).find("#iframe2").document.form.ItemID.value = res.itemid;
                            $(top).find("#iframe2").document.form.Action.value = "Update";*/
                        parent.document.form.GlobalID.value = res.globalid;
                        parent.document.form.ItemID.value = res.itemid;
                        parent.document.form.Action.value = "Update";
                        opener.top.document.form.GlobalID.value =project_gid;
                        //opener.top.document.form.ItemID.value = res.itemid;
                        opener.top.document.form.Action.value = "Update";
                ...
                $.getJSON("../content/select_child_submit_items.jsp?GlobalID="+project_gid+"&ChannelID=" +project_channeldid + "&LinkChannelID=16258&ChildGlobalID=" + GlobalID+"&fieldgroup="+fieldgroup, function(res){
                    alert(res);
                });
...
<form name="form" action="../content/document_submit.jsp" method="post" id = "form">
    ...
    <div class="br-sideleft overflow-y-auto">
        ...
        <div class="br-sideleft-menu">
            <a href="javascript:showTab('<%=i+1%>')" <%=(i==0?"class='br-menu-link active'":"class='br-menu-link'")%> id="form<%=i+1%>_td" groupid="<%=fieldGroup.getId()%>" data-toggle="tooltip" data-placement="right" title="<%=fieldGroup.getName()%>">
                <div class="br-menu-item">
                    <%
                        //判断显示对应图片chensiding 19/07/17
                    %>

                    <% if(i==0){%>
                    <i class="menu-item-icon fa fa-edit tx-22"></i>
                    <%}else if(i==1){%>
                    <i class="menu-item-icon fa fa-link tx-22"></i>
                    <% }else if(i==2){%>
                    <i class="menu-item-icon fa fa-gg tx-20" ></i>
                    <%}else if(i==3){%>
                    <i class="fa fa-info-circle tx-20" ></i>
                    <%}else{%>
                    <i class="menu-item-icon fa fa-edit tx-22"></i>
                    <%}%>
        ...
        <div class="br-sideleft-menu">
            <a href="javascript:showTab('<%=vercount%>')" class="br-menu-link" id="form<%=vercount%>_td" data-toggle="tooltip" data-placement="right" title="历史版本">
                <div class="br-menu-item">
                    <i class="fa fa-info-circle tx-22" aria-hidden="true"></i>
        ...
        <div class="br-header-right">
            <div class="btn-box" >
                ...
                <a class="btn btn-secondary tx-size-xs mg-r-5" href="javascript:SaveApprove(1);" id="Image1Href">驳回</button>
                    <a class="btn btn-primary tx-size-xs mg-r-5" href="javascript:SaveApprove(0);" id="Image2Href">通过</button>
                            <%}%>
    ...
    %>
    <div class="br-mainpanel br-mainpanel-file overflow-hidden" url="<%=url%>" id="form<%=j+1 %>" <%if(j!=0)out.print("style=\"display:none;\"");%> <%if(j==0)out.print("data-approve=\""+data_approve+"\" data-yl=\""+data_yl+"\"");%>>
            <%
	...
	if(fieldGroupID>0 && !fieldGroup.getUrl().equals(""))
	{
		out.println("<iframe id=\"iframe"+(j+1)+"\" frameborder=\"0\" style=\"width:100%;height:100%\" marginheight=\"0\" marginwidth=\"0\" scrolling=\"auto\" src=\""+url+"\"></iframe>");//<!--onload=\"changeFrameHeight(this)\"-->
...
%>
	...
	out.println("</div></div></div></div>");j++;
        ...
        <input type="hidden" id="publisher_cmsid" name="publisher_cmsid" value="<%=(item==null)?userinfo_session.getId():item.getIntValue("publisher_cmsid")%>">
</form>
...
//			});
    ...
    */
    ...
    function getusers(){
        ...
        dialog.setUrl("task_userselect.jsp?fieldName=users_id&content="+content_);
    ...
    function getcars(){
        ...
        dialog.setWidth(900);
        dialog.setHeight(650);
        dialog.setUrl("task_carselect.jsp?id=<%=bespeaks%>&fieldName=bespeak&content="+content2_+"&Category=<%=Category%>");
    ...
    function getdevices(){
        ...
        dialog.setWidth(900);
        dialog.setHeight(650);
        dialog.setUrl("task_deviceselect.jsp?id=<%=devices%>&fieldName=device&content="+content3_);
...
//显示参与人
        ...
        $.ajax({
            type : "GET",
            url : "../api/jurong/v2.0/juxian_userinfo.jsp",
            dataType:"json",
