...
int  SubjectUserId= getIntParameter(request,"userid");
%>
<!DOCTYPE HTML>
<html>
...
function getDataList(page,SubjectId){
    var page = page;
     var url="";
    if(SubjectId==0||SubjectUserId==0){
    url="../api/jurong/v2.0/user_list.jsp";
    }else{
     url="renwu_userlist.jsp?SubjectId="+SubjectId ;  
    }
        ...
        console.log(res);
            ...
            if(res.code==200){
				...
				datalistHtml += '<tr No="'+(index+1)+'" ItemID="'+val.id+'"   id="item_'+val.id+'" >'
						...
						+'<span class="pd-l-5 tx-black" id="name_'+val.id+'">'+val.name+'</span>'
						+'</td>'	
...
function addioscode(){
     ...
     var s2 = "";
     var s3 = "";
     	var len = $("#datalist input:checked").length;
     if(len==0)
	{
		alert("请选择任务一个人！");
	    return "false";	
