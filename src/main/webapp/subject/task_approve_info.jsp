...
<%@ include file="task_approve_config.jsp"%>
...
String result = "<div class=\"card-body bg-white hide\">";
String html = "";
int step = 0 ;//最后一次操作步骤
...
while(rs.next()){
	...
	if(action==1){
		result += "<div class=\"sh-reason mg-t-5\"><p class=\"bd pd-x-10 pd-y-5\">理由："+convertNull(rs.getString("ActionMessage"))+"</p></div></div><div class=\"hide card-body mg-t-20 bg-white shadow-base bd-0\">";
	}
}
tu.closeRs(rs);

if(action==2&&end==1){//最后一次操作是驳回或者是终审，不继续显示
