	...
	*/
	...
	System.out.println("carselectid=="+id);
	...
	String S_brand				=	getParameter(request,"brand");
	String S_number			    =	getParameter(request,"number");
	String S_driver			    =	getParameter(request,"driver");
	...
	String querystring = "";
	querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&number="+S_brand+"&driver="+S_driver;
	         ...
	         for( var i = 0; i < s.length; i++ )
	         {
	           if ( s[i].checked ){
	             ...
	             console.log(s3);
						...
						parent.$("#bespeak_name").attr("value",s3);
						top.TideDialogClose();                                      
	}
		</script>
	</head>
	...
	System.out.println("pindaoname="+ChannelName);
	String ListSql = "select id,Title,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,state,brand,number,driver,phone";
	...
	if(!listAll)
	{
		...
		{
			ListSql += " where state=0 and Active=" + IsActive;
			CountSql += " where state=0 and Active=" + IsActive;
		}
		else if(channel.getType()==Channel.MirrorChannel_Type)
		{
			...
			{
				System.out.println("wlss1-----");
				ListSql += " where Category=0 and state=0 and Active=" + IsActive;
				CountSql += " where Category=0 and state=0 and Active=" + IsActive;
			}
			else
			{
				System.out.println("wlss2-----");
				ListSql += " where Category=0 and state=0 and Active=" + IsActive;
				CountSql += " where Category=0 and state=0 and Active=" + IsActive;
			}
		}
		...
		{	
			System.out.println("wlss3-----");
			ListSql += " where "+ (!listAll?Table+".Category="+Category+" and ":"")+" Active=" + IsActive;
			CountSql += " where "+(!listAll?Table+".Category="+Category+" and ":"")+" Active=" + IsActive;
		}
	}
	else
	{
		System.out.println("wlss4-----");
		ListSql += " where Category=0 and state=0 and Active=" + IsActive;
		CountSql += " where Category=0 and state=0 and Active=" + IsActive;
	}
	...
	if(!S_number.equals("")){
		String tempnumber=S_number.replaceAll("%","\\\\%");
		WhereSql += " and number like '%" + channel.SQLQuote(tempnumber) + "%'";
	}
	
	if(!S_driver.equals("")){
		String tempdriver=S_driver.replaceAll("%","\\\\%");
		WhereSql += " and driver like '%" + channel.SQLQuote(tempdriver) + "%'";
	}
					...
					<form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
						<div class="row">
							...
							<div class="mg-r-10 mg-b-30 search-item">
								<input class="form-control search-title" placeholder="车辆名称" type="text" name="Title" value="<%=S_Title%>" onClick="this.select()">
							</div>
							<!-- wd-200 -->
							<!--车牌-->
							<div class="mg-r-10 mg-b-30 search-item">
								<div class="input-group">
									<input type="text" class="form-control search-author" placeholder="车牌" name="number" value="<%=S_number%>">
								</div>
							</div>
							<!--司机-->
							<div class="mg-r-10 mg-b-30 search-item">
								<div class="input-group">
									<input type="text" class="form-control search-title" placeholder="司机" name="driver" value="<%=S_driver%>">
								</div>
							</div>
					...
					<%if(listType==1){%>
						<thead>
							<tr>
								...
								<th class="tx-12-force tx-mont tx-medium">车辆名称</th>
								<th class="tx-12-force tx-mont tx-medium">状态</th>
	                            <th class="tx-12-force tx-mont tx-medium">车牌</th>
	                            <th class="tx-12-force tx-mont tx-medium">司机</th>
	                            <th class="tx-12-force tx-mont tx-medium">联系电话</th>
								<%if(IsWeight==1){%>
	...
	while(Rs.next())
	{
	    ...
	    String brand	= convertNull(Rs.getString("brand"));
	    String number	= convertNull(Rs.getString("number"));
	    String driver	= convertNull(Rs.getString("driver"));
			...
			<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" id="item_<%=id_%>" >
	            <td class="valign-middle">
	              ...
	              <label class="rdiobox mg-r-15">
	                <input type="radio" name="id" value="<%=id_%>"><span></span>
	              </label>
	              <%}else{%>
	              <label class="ckbox mg-b-0">
	            ...
	            <td ondragstart="OnDragStart (event)">
	              ...
	              <%}%>
	              <span class="pd-l-5 tx-black" id="name_<%=id_%>"><%=Title%></span>
	            </td>
	            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=stateDesc%></span></td>
	            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=number%></span></td>
	            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=driver%></span></td>
	            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=phone%></span></td>
				<%if(IsWeight==1){%>
