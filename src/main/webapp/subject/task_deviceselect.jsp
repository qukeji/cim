...
String S_manager			=	getParameter(request,"manager");
...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&brand="+S_brand+"&manager="+S_manager;
        ...
        function addioscode()
{
         ...
         for( var i = 0; i < s.length; i++ )
         {
          ...
          //s2 = s2.substr(0,s2.length-1);
					...
					parent.$("#device_name").attr("value",s3);
					top.TideDialogClose();                                      
}
	</script>
</head>
...
String ListSql = "select id,Title,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,state,brand,model,manager,phone";
...
if(!S_brand.equals("")){
	String tempbrand=S_brand.replaceAll("%","\\\\%");
	WhereSql += " and brand like '%" + channel.SQLQuote(tempbrand) + "%'";
}

if(!S_manager.equals("")){
	String tempmanager=S_manager.replaceAll("%","\\\\%");
	WhereSql += " and manager like '%" + channel.SQLQuote(tempmanager) + "%'";
}
				...
				<form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onsubmit="return check();">
					<div class="row">
						...
						<div class="mg-r-10 mg-b-30 search-item">
							<input class="form-control search-title" placeholder="设备名称" type="text" name="Title" value="<%=S_Title%>" onClick="this.select()">
						</div>
						<!-- wd-200 -->
						<!--品牌-->
						<div class="mg-r-10 mg-b-30 search-item">
							<div class="input-group">
								<input type="text" class="form-control search-author" placeholder="品牌" name="brand" value="<%=S_brand%>">
							</div>
						</div>
						<!--设备联系人-->
						<div class="mg-r-10 mg-b-30 search-item">
							<div class="input-group">
								<input type="text" class="form-control search-title" placeholder="设备联系人" name="manager" value="<%=S_manager%>">
							</div>
						</div>
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium">设备名称</th>
							<th class="tx-12-force tx-mont tx-medium">状态</th>
                            <th class="tx-12-force tx-mont tx-medium">设备型号</th>
                            <th class="tx-12-force tx-mont tx-medium">设备联系人</th>
...
while(Rs.next())
{
    ...
    String model	= convertNull(Rs.getString("model"));
    String manager	= convertNull(Rs.getString("manager"));
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" id="item_<%=id_%>" >
            ...
            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=brand%><%=(brand!=""&&model!="")?"":"_"%><%=model%></span></td>
            <td class="hidden-xs-down"><span class="pd-l-5 tx-black"><%=manager%></span></td>
