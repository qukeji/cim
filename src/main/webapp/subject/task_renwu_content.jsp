...
<%
    ...
    int id = topic_task.getInt("renwu");//
    int currPage = getIntParameter(request,"currPage");
    int rowsPerPage = getIntParameter(request,"rowsPerPage");
    int sortable = getIntParameter(request,"sortable");
    int rows = getIntParameter(request,"rows");
    int cols = getIntParameter(request,"cols");
    int SubjectId = getIntParameter(request,"globalid");
    if(SubjectId ==0){
        SubjectId = getIntParameter(request,"SubjectId");
    }
...
//当前登录用户绑定了租户,此频道下面是租户频道(聚融，大屏)
    ...
    int TaskStatus_			=	getIntParameter(request,"TaskStatus_");
    int mytask			=	getIntParameter(request,"mytask");
...
<head>
    ...
    <script>
        ...
        function list(str) {
            ...
            var url = pageName + "?id=<%=id%>&rowsPerPage="+currPage+"&LinkChannelID="+LinkChannelID+"&globalid="+globalid;
        ...
        function addrenwu(id,ChannelID)
        {
            window.open("../subject/renwu_add.jsp?SubjectId="+id+"&ChannelID="+ChannelID);
        }

        function Previewdtial(id)
        {
            window.open("../subject/renwu_operation.jsp?ItemID="+id+"&ChannelID=<%=ChannelID%>");
        }
        //预览
        function Preview()
        {
        ...
        {
            window.open("../subject/renwu_operation.jsp?ItemID=" + id + Parameter);
        }
        function Preview3(id)
        {
            window.open("../subject/renwu_operation.jsp?ItemID=" + id + Parameter);
        }
        function editDocument3(itemid)
        {
            var url="../subject/renwu_add.jsp?ItemID="+itemid+"&ChannelID=" + ChannelID;
            window.open(url);
        }
        function editDocument2()
        {
            var obj=getCheckbox();
            if(obj.length==0){
                alert("请先选择要编辑的文件！");
                ...
                editDocument3(obj.id);
            }
        }

        function editDocument4(id)
        {
            var url="../subject/renwu_add.jsp?ItemID="+id+"&ChannelID=" + ChannelID;
            window.open(url);
        }
        function selectItem()
        {
            if(globalid==0)
            {
                $.getJSON("../content/document_getnew.jsp?id="+parent.channelid, function(o){
                    ...
                    var url="renwu_choose.jsp?ChannelID="+ChannelID+"&LinkChannelID="+LinkChannelID+"&GlobalID="+globalid+"&fieldgroup="+fieldgroup;
                    window.open(url);
                });
            }
            else
...
//System.out.println("time:"+weightTime);
        ...
        String ListSql ="select id,Title,Photo,users_id,UserID,TaskStatus,users,TaskType,SubjectName,SubjectId,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews";
...
//if(!S_User.equals(""))
        ...
        String WhereSql = "";
        WhereSql += " and SubjectId="+SubjectId ;
        if(!S_Title.equals("")){
...
if(S_IsIncludeSubChannel==1)
{
	WhereSql += " and ChannelCode like '"+channel.getChannelCode() + "%'";
        ...
        if(TaskStatus_!=0){
            WhereSql += " and TaskStatus=" + (TaskStatus_-1);
        }
        if(mytask!=0){
            WhereSql += " and UserID=" + userinfo_session.getId();
        }
...
// out.println(ListSql);
    ...
    <div class="d-flex align-items-center justify-content-start pd-x-20 pd-sm-x-30 pd-t-25 mg-b-20 mg-sm-b-30">
        ...
        <div class="dropdown hidden-sm-up">
            ...
            <div class="dropdown-menu pd-10">
                <nav class="nav nav-style-1 flex-column">
                    ...
                    <a href="javascript:list('Status1=-1');" class="btn btn-outline-info list_draft">已完成</a>
                    <a href="javascript:list('Status1=1');" class="btn btn-outline-info list_publish">进行中</a>
        ...
        <div class="btn-group mg-l-10 hidden-xs-down">
            ...
            <a href="javascript:list('TaskStatus_=1');" class="btn btn-outline-info list_draft">未开始</a>
            <a href="javascript:list('TaskStatus_=2');" class="btn btn-outline-info list_publish">执行中</a>
            <a href="javascript:list('TaskStatus_=3');" class="btn btn-outline-info list_submit">已提交确认</a>
            <a href="javascript:list('TaskStatus_=4');" class="btn btn-outline-info list_finish">已完成</a>
            <a href="javascript:list('mytask=1');" class="btn btn-outline-info list_myrenwu">我的任务</a>
        ...
        <div class="btn-group mg-l-auto hidden-sm-down">
            <a href="javascript:addrenwu('<%=SubjectId%>','<%=ChannelID%>');" class="btn btn-outline-info">新建</a>
            <a href="javascript:selectItem();" class="btn btn-outline-info">选择</a>

            <a href="javascript:editDocument2();" class="btn btn-outline-info">编辑</a>
            <%if(canDelete){%>
        ...
        <div class="dropdown hidden-md-1500-up hidden-sm-down mg-l-10">
            ...
            <div class="dropdown-menu dropdown-menu-right pd-10">
                <nav class="nav nav-style-1 flex-column">
                    ...
                    <a href="javascript:selectItem();" class="btn btn-outline-info">选择</a>
                    <a href="javascript:editDocument2();" class="nav-link">编辑</a>
                    <%if(canDelete){%>
                ...
                <div class="row">
                    ...
                    <div class="mg-r-10 mg-b-30 search-item">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control search-author" placeholder="提交人" name="User" value="<%=S_User%>">
                        </div>
                    </div>
                    <!--状态-->
                    <div class=" mg-lg-t-0 mg-r-10 mg-b-30 search-item">
                        <select class="form-control select2" data-placeholder="状态" name="TaskStatus_">
                            <option label="Choose one"></option>
                            <option value="0">全部</option>
                            <option value="1" <%=(TaskStatus_==1?"selected":"")%>>未执行</option>
                            <option value="2" <%=(TaskStatus_==2?"selected":"")%>>执行中</option>
                            <option value="3" <%=(TaskStatus_==3?"selected":"")%>>已提交确认</option>
                            <option value="4" <%=(TaskStatus_==4?"selected":"")%>>已完成</option>
                        </select>
                    </div>
        ...
        <div class="card bd-0 shadow-base">
            <table class="table mg-b-0 <%=listType==2?"table-fixed":""%>" id="content-table">
                ...
                <tr>
                    ...
                    <%}%>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-250">创建日期</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-150">所属选题</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">任务类型</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">任务状态</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120">发布状态</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">提交人</th>
                    <th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-350 wd-author">任务执行人</th>
                ...
                <%
                    ...
                    while(Rs.next())
                    {
                        ...
                        int TopStatus = 0;
                        int SubjectId1 = Rs.getInt("SubjectId");//选题id
                        int TaskStatus = Rs.getInt("TaskStatus");//任务状态
                        String users = Rs.getString("users");//执行人姓名
                        String users_id = Rs.getString("users_id");//执行人id
                        String SubjectName = Rs.getString("SubjectName");//选题名称
                ...
                <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
                    ...
                    <td class="dropdown hidden-xs-down">
                        ...
                        <a href="javascript:Previewdtial('<%=id_%>');" class="svg-icon fa-svg-icon svg-fa-check-circle" viewBox="0 0 512 512" title="查看详情"><i class="fa fa-check-circle tx-20" aria-hidden="true"></i></a>
                        ...
                        <div class="dropdown-menu dropdown-menu-right pd-10">
                            <nav class="nav nav-style-1 flex-column">
                                ...
                                <%}else{%>
                                <a href="javascript:editDocument4(<%=id_%>);" class="nav-link">编辑</a>
    ...
    <script>
        ...
        //设置高亮
        //设置高亮
        var Status_approve = <%=TaskStatus_%>;
        ...
        var mytask=<%=mytask%>
            $(function() {
                ...
                if (Status_approve == 1) {
                    ...
                    $(".list_submit").addClass("active");

                }else if (Status_approve == 4) {

                    $(".list_finish").addClass("active");

                }else if (IsDelete_==1) {
                    $(".list_delete").addClass("active");
                }else if (mytask==1) {
                    $(".list_myrenwu").addClass("active");
                }else{
        ...
        $(function(){
            ...
            var menu = [
                ...
                {'<i class="fa fa-edit mg-r-5 fa"></i>编辑':function(menuItem,menu) {editDocument2(); }},
