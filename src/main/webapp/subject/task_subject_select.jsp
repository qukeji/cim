        ...
        function addioscode()
{
         ...
         for( var i = 0; i < s.length; i++ )
         {
          ...
          //s2 = s2.substr(0,s2.length-1);
					...
					parent.$("#<%=fieldidName%>").attr("value",s2);
					parent.$("#<%=fieldName%>").attr("value",s3);
					top.TideDialogClose();                                      
}
	</script>
</head>
...
if(user_id!=0){
	WhereSql += " and FIND_IN_SET('"+user_id+"', users_id)";
}
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" id="item_<%=id_%>" >
            ...
            <td ondragstart="OnDragStart (event)">
              ...
              <%}%>
              <span class="pd-l-5 tx-black" id="name_<%=globalid_%>"><%=Title%></span>
            </td>
			<%if(IsWeight==1){%>
