function TideDialog(){
	...
	this.show=function(){
		...
		if(this.type==0){			
		    ...
		    var _boxH = this.height - 50 - 72  ;//减去标题和底部高度
...
TideDialog.prototype.ShowMsg=function(){
	...
	this.type=2;
	this.suffix="html";
    this.html= '<div class="modal-body pd-25" id="alert_boy" style="position:static;top:0;bottom:0;height:100%">'+                 
	              '<p class="mg-b-5">'+ this.msg +'</p>'+
	            '</div>' +
	            '<div class="modal-footer">'
	if(this.msgJs){
		confirmJs = this.msgJs ;
		confirmPosition = this._position ;
		this.html +='<button id="startButton" type="button" class="btn btn-primary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" onclick="comfirmClose()">确认</button>'
	}else{
		this.html +='<button id="startButton" type="button" class="btn btn-primary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" onclick="getDialog().Close({suffix:\'html\'});">确认</button>'
	     ...
	     //         '<button id="startButton" type="button" class="btn btn-primary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" onclick="'+this.msgJs+'">确认</button>'+
	    this.html += '<button id="btnCancel1" type="button" class="btn btn-secondary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium" data-dismiss="modal" onclick="getDialog().Close({suffix:\'html\'});">取消</button>'+
	            '</div>'	
	this.show();
}
var confirmJs = "" ;
...
function comfirmClose(){
	var suffix=tidecms.dialognumber ;	
	suffix--;
	var jsString = "" ;
	if(suffix>0){
		console.log(confirmPosition)
		if(confirmPosition=="content"){
			//当是在内容页所在的iframe中点击时			
			if(window.frames["content_frame"].contentWindow){
				jsString = 'window.frames["content_frame"].contentWindow.'+confirmJs+'()' ;
			}else{
				jsString = 'window.frames["content_frame"].'+confirmJs+'()' ;
			}
		}else{
			if(window.frames["popiframe_"+suffix].contentWindow){
				jsString = 'window.frames["popiframe_'+suffix+'"].contentWindow.'+confirmJs+'()' ;
			}else{
				jsString = 'window.frames["popiframe_'+suffix+'"].'+confirmJs+'()' ;
			}			
		}		
		eval(jsString);
		//window.frames["popiframe_"+suffix].setReturnValue(o.returnValue);
	}else{
		if(confirmPosition=="content"){
			if(window.frames["content_frame"].contentWindow){
				jsString = 'window.frames["content_frame"].contentWindow.'+confirmJs+'()' ;	
			...
			jsString = 'window.'+confirmJs+'()' ;
		}
		eval(jsString);
	}
	getDialog().Close({suffix:'html'});
	console.log(suffix)    
}
