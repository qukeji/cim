...
if(GroupID==0 && SerialNo.length()>0)
{
	String groupsql = "select id from template_group where SerialNo='" + tu.SQLQuote(SerialNo) + "'";
	ResultSet groupRs = tu.executeQuery(groupsql);
	if(groupRs.next())
	{	
...
function double_click()
{
	jQuery("#oTable tr:gt(0)").dblclick(function(){
			...
			var fileName = jQuery(this).attr("fileName");//alert(fileName);
			//top.window.frames["popiframe_"+top.tidecms.dialognumber].setReturnValue({TemplateID:tid,FileName:fileName});
			//top.TideDialogClose({suffix:'_1',recall:true,returnValue:{TemplateID:tid,FileName:fileName}});
<%if(SerialNo.equals("page_frame_template")){%>
			top.window.frames["popiframe_1"].setReturnValue({TemplateID:tid,FileName:fileName});
<%}else{%>
			top.TideDialogClose({returnValue:{TemplateID:tid,FileName:fileName}});
<%}%>
	});
...
<thead>
		<tr>
    				<th class="v3"	align="left">名称</th>
    				<th class="v1"	align="left" valign="middle">文件名</th>
    				<th class="v4"  align="left" valign="middle">最后修改时间</th>
		</tr>
</thead>
 <tbody> 
...
if(tu.pagecontrol.getRowsCount()>0)
{
	...
	<tr id="jTip<%=j%>_id"  tid="<%=tid%>" fileName="<%=tf.getFileName()%>">
    <td class="v3" style="font-weight:700;"><img src="../images/tree6.png"/><%=tf.getName()%></td>
	<td class="v1" align="left" valign="middle"><%=tf.getFileName()%></td>
...
%>
<tr><td colspan=3 align=center>没有文件。</td></tr>
<%}
tu.closeRs(Rs);
