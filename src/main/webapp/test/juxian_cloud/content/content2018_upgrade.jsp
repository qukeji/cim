...
System.out.println("获取的channel.getId()==="+ChannelID);
System.out.println("获取的channel.getParent()==="+channel.getParent());
...
if(doc_type_channel== -1){
	 ListSql = "select id,Title,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,success_num";
}else{
	 ListSql = "select id,Title,doc_type,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,Status,Active,Category,IsPhotoNews,success_num";
}	
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<th class="tx-12-force tx-mont tx-medium hidden-xs-down wd-120 wd-author">成功使用次数</th>
							<%if(IsComment==1){%>
...
while(Rs.next())
{
	...
	int successNum=Rs.getInt("success_num");
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>" status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down">
				...
				<%}%>
				<a href="javascript:showlog(<%=id_%>);" class="btn pd-0 mg-r-5" title="查看日志"><i class="fa fa-eye tx-18 handle-icon" aria-hidden="true"></i></a>
...
<script>
	...
	function showlog(id){
		window.open("http://jushi.tidemedia.com/cms/lib/webIndex.jsp?channelid=15826&itemid="+id);
	}
