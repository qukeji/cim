...
//更新平台管理员运营中心权限
public void UpdateUserYunyingPerm() throws SQLException,MessageException{
    TableUtil tu = new TableUtil("user");
    String Sql = "select * from userinfo where Role=1 and company=0";
    ResultSet Rs = tu.executeQuery(Sql);
    while(Rs.next())
    {
    	int id_ = Rs.getInt("id");
    	...
    	if(!ui.hasPermission("OperateSystem")){			
    		TableUtil tu1 = new TableUtil();
    		tu1.executeUpdate("insert into user_perm(PermName,PermValue,User) values ('OperateSystem',1,"+id_+")");
    	}
    }
    tu.closeRs(Rs);
}	
...
while(Rs.next())
{
	Channel ch = CmsCache.getChannel(Rs.getInt("id"));
	try{
	ch.getTableUtil().executeUpdate("ALTER TABLE channel_" + Rs.getString("SerialNo")+" ADD ModifiedUser int");
	}catch(Exception e){System.out.println(e.getMessage());}
...
//2016-08 -18 增加 
//tu.executeUpdate("alter table publish_scheme add  accessKeyID varchar(255),add  secretKey varchar(255),add  bucketName varchar(255),add  endPoint varchar(255),add  TenantName varchar(255),add  CredentialsUserName varchar(255),add  CredentialsPassword varchar(255),add  TokenUrl varchar(255)");

//2016-08 -18 增加 
//tu.executeUpdate("alter table publish_scheme add  Ossendpoint varchar(255)");
//tu.executeUpdate("alter table publish_scheme add  OssLocalenpoint varchar(255)");
//tu.executeUpdate("alter table publish_scheme add  OssaccessKeyId varchar(255),add  OssaccessKeySecret varchar(255),add  OSSbucketName varchar(255),add  Ossdomain varchar(255),add  BosaccessKeyId varchar(255),add  BosaccessKeySecret varchar(255),add  BosbucketName varchar(255),add  Bosdomain varchar(255)");

// 2017 -01-17 增加定时发布表
//tu.executeUpdate("CREATE TABLE tidecms_to_publish (id int(10) unsigned NOT NULL AUTO_INCREMENT,globalid int(10) DEFAULT 0,publishtime varchar(255) DEFAULT '',republishtime varchar(255) DEFAULT NULL,PRIMARY KEY (id)) ENGINE=MyISAM AUTO_INCREMENT=350 DEFAULT CHARSET=utf8 ");



//2017-12-22
//publish_task增加Message字段
//tu.executeUpdate("alter table publish_task add Message text");

//2018-03-15
//field_desc增加EditorType字段
//tu.executeUpdate("alter table field_desc add EditorType tinyint default 0");

//2018-05-21
//photo_scheme表增加Describe，type字段
//tu.executeUpdate("alter table photo_scheme add summary text, add Type int");
//tu.executeUpdate("insert into photo_scheme(Name,Width,Height,Type) values ('内容页图片',768,0,1)");

//2018-06-29
//spider表增加PrevRunTime，NextRunTime字段
//tu.executeUpdate("alter table spider add PrevRunTime int(10) unsigned default 0, add NextRunTime int(10) unsigned default 0");

//2018-07-15
//添加聚视产品
//INSERT INTO `tide_products` VALUES (10,'内容汇聚发布系统','jushi_cms','',NULL,1,NULL,'/jushi/','聚视平台 ','Evaluation','1538406000000')

//2018-07-15
//item_snap增加SiteId字段
//tu.executeUpdate("alter table item_snap add SiteId tinyint default 0");

//2018-08-24
//userinfo表增加SiteId字段
//tu.executeUpdate("alter table userinfo add Site varchar(255)");

//2018-09-12
//field_desc表增加Caption字段
//tu.executeUpdate("alter table field_desc add Caption text");

//2018-09-14
//增加"relation_button"
//tu.executeQuery("CREATE TABLE `relation_button` (id int(11) NOT NULL auto_increment,  GlobalID int(11), ChildGlobalID int(11),  OrderNumber int(11),UNIQUE KEY `id` (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;");

//2018-10-11
//tidemedia_user.parameter 建表语句
//tu_user.executeUpdate("CREATE TABLE parameter (id int(10) unsigned NOT NULL auto_increment,Name varchar(255), Code varchar(255),CreateDate long,Content text,SystemContent text,Type tinyint,Type2 tinyint,IsTemplate tinyint,IsJson tinyint,Comment text,IntValue int,PRIMARY KEY  (`id`))");

//2018-10-15
//工作流
//channel表增加ApproveScheme字段
//tu.executeUpdate("alter table channel add ApproveScheme int(11)");
//增加审核方案表approve_scheme
//tu.executeUpdate("CREATE TABLE approve_scheme (id int(10) unsigned NOT NULL auto_increment,Title varchar(255),CreateDate datetime, Status int(11) default 0,PRIMARY KEY  (`id`))");
//增加审核环节表approve_items
//tu.executeUpdate("CREATE TABLE approve_items (id int(10) unsigned NOT NULL auto_increment,Title varchar(255), Type int(11) default 0,users varchar(255),parent int(11),step int(11),PRIMARY KEY  (`id`))");
//增加审核动作表approve_actions
//tu.executeUpdate("CREATE TABLE approve_actions (id int(10) unsigned NOT NULL auto_increment,Title varchar(255), parent int(11),userid int(11),Action int(11),ApproveId int(11),ActionMessage text,CreateDate datetime,PRIMARY KEY  (`id`))");
//tu.executeUpdate("alter table approve_actions CHANGE GlobalID parent int(11)");
//tu.executeUpdate("alter table approve_actions CHANGE step ApproveId int(11)");
//tu.executeUpdate("alter table approve_actions add endApprove int(11)");
...
//2018-11-9
//增加"base64"
//tu.executeUpdate("alter table template_files add ContentMd5 varchar(255),add SystemMd5 varchar(255)");

//2018-11-12
//rows字段名修改，新版本mysql不支持
//tu.executeUpdate("ALTER TABLE channel_template CHANGE COLUMN `Rows` `RowsNumber` INT(11) NULL DEFAULT NULL ;");
...
//tu_user.executeUpdate("CREATE TABLE company (id int(10) unsigned NOT NULL auto_increment,Name varchar(255),ExpireDate datetime, Status int(11) default 0,PRIMARY KEY  (`id`))");
//用户表，用户组表，站点表，都增加一个company字段，对应租户编号
//tu_user.executeUpdate("alter table userinfo add company int(11) default 0");
...
//tu.executeUpdate("alter table article_replica add Status int(11) default 0,add ModifiedUser int(11)");

//2019-01-28
//site表增加SiteType字段
//tu.executeUpdate("alter table site add SiteType int(11) default 0");

//2019-03-05
//审核操作表增加审核环节名称
//tu.executeUpdate("alter table approve_actions add ApproveName varchar(255)");

//2019-03-26
//租户表新建字段:联系人，联系人电话，聚现登录手机号，logo，空间
...
//2019-03-27
//用户表增加字段
//tu_user.executeUpdate("alter table userinfo add jurong int(11) default 0");
//tu_user.executeUpdate("alter table tide_products add OrderNumber int(11) default 0,add Type tinyint default 0");
...
//2019-05-15
//租户表增加聚现token字段
//tu_user.executeUpdate("alter table company add JuxianToken varchar(255)");

//2019-05-23
//channel_template增加允许预览字段
//tu.executeUpdate("alter table channel_template add AllowPreview int(11) default 0");//cms库

//2019-5-27 补充api_token的创建语句
//tu.executeUpdate("create table api_token(id int(10) unsigned NOT NULL AUTO_INCREMENT,Token varchar(255),Name varchar(255),CreateDate int(10) default 0,PRIMARY KEY (`id`))");

//2019-06-13
//审核方案表添加租户id字段
//tu.executeUpdate("alter table approve_scheme add company int(11) default 0");

//2019-06-19
//审核环节表添加允许编辑字段  默认为0   0：不开启编辑  1：开启编辑
//tu.executeUpdate("alter table approve_items add editable int(1) default 0");

//2019-07-13
//审核环节表添加回调地址字段
//tu.executeUpdate("alter table approve_items add url varchar(255)");

//2019-06-24
//产品表增加字段控制编辑是否可见(0不显示，1不显示)
//tu_user.executeUpdate("alter table tide_products add groupId int(11) default 0,add Isview int(11) default 0");
//tu_user.executeUpdate("alter table tide_products add logo varchar(255),add newpage tinyint default 0");
//tu_user.executeUpdate("update tide_products set Isview=1 where Name in('文稿管理','选题任务','APP管理','网站管理','微信矩阵','电子报','网络电视','电子地图','PGC内容','UGC内容','全媒体资源','内容采集','收录剪辑','云快编','H5云制作')");

//2019-07-09
//增加快捷导航管理表
//tu_user.executeUpdate("CREATE TABLE navigation (id int(10) unsigned NOT NULL auto_increment,Title varchar(255),Href varchar(255),Parent int(11) default 0,Level int(11) default 0,hasNextLevel int(11) default 0,newpage int(11) default 0,PRIMARY KEY  (`id`))");

//2019-07-23
//增加微信-群发图文_图文素材关联表
//tu.executeUpdate("CREATE TABLE weixin_groupdoc_document (id int(10) unsigned NOT NULL auto_increment,GroupDocId varchar(255),DocumentId varchar(255),OrderNumber int(11) default 0,type int(11) default 0,PRIMARY KEY  (`id`))");
//2019-07-24
//微信图文素材表增加字段留言
//tu.executeUpdate("alter table weixin_newsitem add leave_word int(11) default 0,add imagepath1 varchar(255),add content_md5 varchar(255)");

//2019-07-29
//tu.executeUpdate("CREATE TABLE approve_document (id int(10) unsigned NOT NULL auto_increment,UserId int(10) default 0,UserName varchar(255),Title varchar(255),GlobalID int(10) default 0,ItemID int(10) default 0,ChannelID int(10) default 0,ApproveItemId int(10) default 0,ApproveItemName varchar(255),Action int(10) default 0,CreateDate datetime,ActionDate datetime,ActionMessage varchar(255),EndApprove int(10) default 0,PRIMARY KEY  (`id`))");

//2019-08-02
//字段分组表增加图标字段
//tu.executeUpdate("alter table field_group add Icon varchar(255)");

//2019-08-20
//一稿多发增加推荐设置表
//tu.executeUpdate("CREATE TABLE channel_recommend (id int(11) unsigned NOT NULL auto_increment,ChannelID int(11) NOT NULL,RelationID int(11) NOT NULL,FieldRelation int(1) default 0,RelationchannelType int(1) NOT NULL default 0,Type int(1) NOT NULL,Relationship mediumtext NOT NULL,CreateDate datetime,PRIMARY KEY  (`id`))");
//2019-08-27
//新增七牛云字段
//tu.executeUpdate("alter table publish_scheme add `QiniuAccessKey` varchar(255) DEFAULT '', add  `QiniuSecrectKey` varchar(255) DEFAULT '',  add `QiniubucketName` varchar(255) DEFAULT ''");

/*
tu.executeUpdate("update channel set application=\"app\" where id=15893");
tu.executeUpdate("update channel set application=\"app_config\" where id=16319");
tu.executeUpdate("update channel set application=\"user\" where id=15963");
tu.executeUpdate("update channel set application=\"interaction\" where id=15964");
tu.executeUpdate("update channel set application=\"web\" where id=15933");
tu.executeUpdate("update channel set application=\"pgc\" where id=16266");
tu.executeUpdate("update channel set application=\"pgc_doc\" where id=16269");
tu.executeUpdate("update channel set application=\"pgc_live\" where id=16270");
tu.executeUpdate("update channel set application=\"publishsite\" where id=8");
*/
//2019-9-4 稿件发布表新增 ChannelType
//tu.executeUpdate("alter table recommend add `ChannelType` int(1) DEFAULT 0");
//2019-09-19 推荐设置表 将operation 字段更改为Type
//tu.executeUpdate("alter table channel_recommend change Operation Type int(1) NOT NULL");

//2019-09-19 帖子管理允许插入表情
//tu.executeUpdate("ALTER TABLE channel_s53_community_topic MODIFY `txtContent` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
//tu.executeUpdate("ALTER TABLE channel_s53_community_topic MODIFY `Title` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");

//2019-10-11  推荐表新增微信文章id字段 weixinItemID
//tu.executeUpdate("alter table recommend add weixinItemID varchar(255)");

//2019-10-30 cms添加水印方案表  》watermark
//tu.executeUpdate("CREATE TABLE watermark ( `id` int(11) NOT NULL AUTO_INCREMENT, `Name` varchar(255) DEFAULT NULL,`companyid` int(11) default 0,  `Position` varchar(255) DEFAULT '0',  `Watermark` text,  `LogoTop` int(11) DEFAULT '0', `LogoLeft` int(11) DEFAULT '0', `CreateDate` int(11) unsigned DEFAULT '0', width int(11)  DEFAULT '0',height int(11)  DEFAULT '0',dissolve int(11)  DEFAULT '0',status int(11)  DEFAULT '0',PRIMARY KEY (`id`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8");

//修改绩效明细表application字段为jixiao_detail
//tu.executeUpdate("update channel set application='jixiao_detail' where id=17770");
//tu.executeUpdate("alter table channel_jixiao_report change TongjiDate TongjiDate date");
//tu.executeUpdate("alter table channel_jixiao_tongji change TongjiDate_end TongjiDate_end date");
// 2019-11-08  创建列表项目表
//tu.executeUpdate("CREATE TABLE channel_list_header (id int(10) unsigned NOT NULL auto_increment,Title varchar(255),Status int(11) default 0,Fieldid int(11),Width varchar(255),Script text,Channelid int(11),OrderNumber int(11),PRIMARY KEY  (`id`))");

//2019-11-11
//导航增加排序
//tu_user.executeUpdate("alter table navigation add ordernumber int(11) default 0");

//2019-09-25  稿件审核表新增审核类型字段
//tu.executeUpdate("alter table approve_document add ApproveType int(11)");

	//2019-12-06 增加产品简介字段
	//tu.executeUpdate("alter table tide_products add Summary text");

	//2019-12-11 增加租戶文件使用空間字段
	//tu.executeUpdate("alter table company add useFileSpace int(11) default 0");

//2019-12-16
//更新平台管理员运营中心权限
//UpdateUserYunyingPerm();
