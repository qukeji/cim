...
if(id==0) 
	id= CmsCache.getParameter("sys_vote_channel").getIntValue();
...
function getRadio(){
	...
	if(len!=1)
	{
	   ...
	   id = $("#oTable input:checked").val();
	var html = "";
	
	$.ajax({
		url:"editor_vote_form.jsp",
		type:"POST",
		data:"id="+id,
		async:false, 
		dataType:"JSON",
...
%>
		 <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			<td class="valign-middle">
			  <label class="checkbox mg-b-0">
				<input name="id" value="<%=GlobalID%>" type="checkbox" /><span></span>
			  </label>
			</td>
