...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&CreateDate="+S_CreateDate+"&CreateDate1="+S_CreateDate1+"&User="+S_User+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1 +"&SiteId="+SiteId;
...
function uploadFile()
{
		window.open("document_vote.jsp?ItemID=0&ChannelID="+ChannelID+"&Global_id=" + <%=Global_id%>);
}
</script>
</head>
<body>
<div class="content_t1">
    <div class="content_new_post">
		<div class="top_button button_inline_block" onClick="uploadFile();">
...
<thead>
		<tr id="oTable_th">
    				...
    				<th class="v3" style="padding-left:10px;text-align:left;">选项名称</th>
					<%if(IsWeight==1){%>
					<th class="v5" align="center" valign="middle">权重</th>
					<%}%>
                    <th class="v1"	align="center" valign="middle">被选个数</th>
    				<th class="v8"  align="center" valign="middle">日期</th>
    				<th class="v4"  align="left" valign="middle" style="padding-left:10px;">发起人</th>
    				<%if(IsComment==1){%>
...
String ListSql = "select id,Title,Photo,Weight,GlobalID,User,parent,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,number,Status,Active,Category";
...
%>
 <tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
	...
	<td class="v1" align="center" valign="middle"><%=number%></td>
