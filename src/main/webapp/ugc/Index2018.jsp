...
<body class="collapsed-menu email" id="withSecondNav" onLoad="init();">
	...
	<%@include file="ugc_tree.jsp"%><!--静态包含-->
    ...
    <div class="br-mainpanel with-second-nav br-mainpanel-file " id="js-source">
        <iframe src="../content/content2018.jsp?id=14270" id="content_frame" style="width: 100%;height: 100%;"  frameborder="0" onload="changeFrameHeight(this)"></iframe>
    </div><!-- br-mainpanel -->
				...
				for(var i=0;i<json.length;i++){
				    ...
				    console.log(json1);
                       	...
                       	//如果等于需要的id
                           	    ...
                           	    //栏目2互动管理
                           	    var json_=json1[n].child;
                           	    console.log(json_);
                           	    
                               	    for(var j=0;j<json_.length;j++){
                					var html = '';
                    					if (json_[j].id==14270){
                    					   
                    				
                    					if(json_[j].child.length>0)
                    					{					
                    					...
                    					if( json_[j].load==1 || (json_[j].child!=null && json_[j].child.length>0) ){	
                    						if(json_[j].type==0){  //判断是否独立表单
                    							html += '<a href="javascript:;" channelid="'+json_[j].id+'" load="'+json_[j].load+'" cloud="'+json_[j].cloud+'"><i class="fa fisrtNav fa-home" have="1"></i> <span>'+json_[j].name+'</span></a>';
                    						...
                    						if(json_[j].type==0){
                    							html += '<a href="javascript:;" channelid="'+json_[j].id+'" load="'+json_[j].load+'" cloud="'+json_[j].cloud+'"><i class="fa fisrtNav fa-home" have="0"></i> <span>'+json_[j].name+'</span></a>';							
                    						}else{
                    							html += '<a href="javascript:;" channelid="'+json_[j].id+'" load="'+json_[j].load+'" cloud="'+json_[j].cloud+'"><i class="fa fisrtNav fa-home" have="0"></i> <span>'+json_[j].name+'</span></a>';							
                    						}	
                    					...
                    					if(json_[j].child.length>0)
                    					{			
                                            html += '<ul class="treeview-menu menu-open">' + get_menu_html(json_[j],childChannel) + '</ul>';					
                    					}
                    					html += '</li>';
                    					menu.append(html);
