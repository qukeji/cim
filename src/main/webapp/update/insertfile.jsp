...
String filepath = getParameter(request, "path");
...
<script type="text/javascript" src="../swfupload/handlers.js"></script>
	...
	window.onload = function() {
			var settings = {
				...
				upload_url: "<%=request.getContextPath()%>/update/insertfile_submit.jsp",	// Relative to the SWF file
				post_params: {},
				file_size_limit : "0",
...
function startSWFU(){
	...
	if(jQuery.browser.msie){
		browser="msie";
	option={"browser":browser,"ChannelID":"<%=ChannelID%>","globalid":"<%=globalid%>","Type":"<%=Type%>","action":"<%=action%>","fieldname":"<%=fieldname%>","itemid":"<%=itemid%>","filepath":"<%=filepath%>"};
	}else{
		browser="mozilla";
		option={"browser":browser,"ChannelID":"<%=ChannelID%>","globalid":"<%=globalid%>","Type":"<%=Type%>","action":"<%=action%>","fieldname":"<%=fieldname%>","itemid":"<%=itemid%>","filepath":"<%=filepath%>","Username":"<%=userinfo_session.getUsername()%>","Password":"<%=userinfo_session.getMd5Password()%>"};
	}
...
<div class="form_button">
	<input name="startButton" type="button" class="tidecms_btn2" value="确定"  id="startButton"  onclick="startSWFU();" disabled="disabled"/>
