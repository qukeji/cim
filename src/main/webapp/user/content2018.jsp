...
String querystring = "";
querystring = "&Title="+java.net.URLEncoder.encode(S_Title,"UTF-8")+"&startDate="+S_startDate+"&endDate="+S_endDate+"&phone="+S_phone+"&Status="+S_Status+"&IsPhotoNews="+S_IsPhotoNews+"&OpenSearch="+S_OpenSearch+"&IsDelete="+IsDelete+"&Status1="+Status1;
...
String ListSql = "select id,Title,phone,avatar,IP,siteflag,wechat,weibo,Photo,Weight,GlobalID,User,FROM_UNIXTIME(ModifiedDate) as ModifiedDate,FROM_UNIXTIME(CreateDate) as CreateDate,Status,Active,Category,IsPhotoNews";
				...
				<%if(listType==1){%>
					<thead>
						<tr>
							...
							<%}%>
							<th class="tx-12-force wd-150 tx-mont tx-medium hidden-xs-down" style="text-align:center">操作</th>
						</tr>
					</thead>
                  <%}%>
					<tbody>
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>" GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
			...
			<td class="dropdown hidden-xs-down" style="text-align:center">
					...
					<a href="javascript:editDocument(<%=id_%>);" class=" btn btn-info btn-sm mg-r-8 mg-b-8 tx-13">编辑</a>
					<a href="javascript:deleteFile1(<%=id_%>);" class="btn btn-info btn-sm mg-r-8 mg-b-8 tx-13">删除</a>
			</td>
		</tr>
<%
	}
