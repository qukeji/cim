...
<body class="collapsed-menu email" id="withSecondNav" onLoad="init();">
    ...
    <div class="br-mainpanel with-second-nav br-mainpanel-file " id="js-source">
        <iframe src="tcenter_user_list2018.jsp?GroupID=-1" id="content_frame" style="width: 100%;height: 100%;"  frameborder="0" onload="changeFrameHeight(this)"></iframe>
    </div><!-- br-mainpanel -->
		...
		function menu_init(type,obj){
			...
			$.ajax({type:"GET",dataType:"json",url:url,success: function(json){
				...
				if(type==1){
					...
					changeFrameSrc( window.frames["content_frame"] , "../user/tcenter_user_list2018.jsp" )
				}
			}});
		}
...
//========================================================================
		...
		$.sidebarMenu = function(menu) {
			...
			$(menu).on('click', 'li a', function(e) {
                ...
                changeFrameSrc( window.frames["content_frame"] , "../user/tcenter_user_list2018.jsp?GroupID="+GroupID )	
			...
			if (getActiveNav()){ 
				...
				if(confirm("确实要删除\"" + GroupName + "\"吗?\r\n")) 
				{
					$.ajax({
						...
						error:function (res) {
							alert(res.responseText.replace(/[\r\n]/g,""));
                        }
					});
				}
