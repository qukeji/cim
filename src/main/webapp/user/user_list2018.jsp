...
<head>
    ...
    <script>
        ...
        function setPerm(id, role) {
            ...
            dialog.setUrl("user_edit2.jsp?id=" + id);
        ...
        function Disable(id) {
            var url = "user_company_action.jsp?userid=" + id + "&type=2";
            $.ajax({
                type: "GET",
                url: url,
        ...
        function gopage(currpage) {
            var url = "../user/user_list2018.jsp?GroupID=<%=GroupID%>&type=<%=type%>&company=<%=company%>&currPage=" + currpage + "&rowsPerPage=<%=rowsPerPage%>";
            this.location = url;
        }
...
<div class="br-mainpanel br-mainpanel-file" id="js-source">
    ...
    <div class="br-pagebody pd-x-20 pd-sm-x-30">
        <div class="card bd-0 shadow-base">
            <table class="table mg-b-0" id="content-table">
                ...
                %>
                <tr id="jTip<%=id%>_id">
                    <td class="hidden-xs-down"><%=id%>
                    </td>
                    <td class="hidden-xs-down"><%=RoleName%>
                    </td>
                    <td class="hidden-xs-down"><%=Name%>
                    </td>
                    <td class="hidden-xs-down"><%=Tel%>
                    </td>
                    <td class="hidden-xs-down">
                        <div class='toggle-wrapper'>
                            <div class='toggle toggle-light primary toggle_company' data-toggle-on='<%=jurongflag%>'
                                 userId='<%=id%>' field="jurong"></div>
                        </div>
                    </td>
                    ...
                    <td class="hidden-xs-down">
                        <div class='toggle-wrapper'>
                            <div class='toggle toggle-light primary toggle_company' data-toggle-on='<%=userflag%>'
                                 userId='<%=id%>' field="Status"></div>
                        </div>
                    </td>
                </tr>
                <% }
    ...
    <script>
        ...
        $(".toggle").click(function () {
            ...
            var url = "user_enable2018.jsp?id=" + id + "&flag=" + flag + "&field=" + field;
            $.ajax({
                type:"get",
                url:url,
                dataType:"json",
        ...
        function change(obj) {
            if (obj != null) this.location = "../user/user_list2018.jsp?GroupID=<%=GroupID%>&type=<%=type%>&company=<%=company%>&rowsPerPage=" + obj.value;
        }
        ...
        jQuery(document).ready(function () {
            ...
            jQuery("#goToId").click(function () {
                ...
                if (num < 1)
                    num = 1;
                var href = "../user/user_list2018.jsp?GroupID=<%=GroupID%>&type=<%=type%>&company=<%=company%>&currPage=" + num + "&rowsPerPage=<%=rowsPerPage%>";
