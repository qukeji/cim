...
<head>
<meta charset="utf-8">
<title>我的爆料_tidemedia用户中心</title>
...
function del(id){
	$.ajax({
		url:"../usercenter/baoliao_del.jsp",
		data:"id="+id,
		type:"post",
		datatype:"json",
		success:function(data){
...
function go(page){
	$.ajax({
		url:"../usercenter/query_baoliao.jsp",
		...
		success:function(data){
			...
			if(data.status == 1){
				...
				for(var i=0;i<arr.length;i++){
					...
					str += '<li><span>'+arr[i].Title.substring(0,10)+'</span> | <span>'+arr[i].Content.substring(0,30)+'</span><span class="date">'+state+date+'<a href="javascript:del('+arr[i].id+')" class="del" title="删除">删除</a></span></li>';
				}
    ...
    <div class="u_i_left" id="u_i_left" style="height:628px;">
        <ul class="u_i_menu">
           <li><a href="../usercenter/page.jsp" class="grzl">个人资料</a></li>
            ...
            <li><a href="../usercenter/baoliao.jsp" class="wdbl on">我的爆料</a></li>
...
<%
	...
	int totalsize=0;
    String sql_ = "select id from channel_tidehome_baoliao  where  Active=1 and userid =" + userid;
    ...
    String sql = "select * from channel_tidehome_baoliao  where userid = " + userid +" and Active=1  order by id desc  limit 0,12";
