...
<head>
<meta charset="utf-8">
<title>我的订单_tidemedia用户中心</title>
...
function del(id){
	$.ajax({
		url:"__APP__/Home/Feature/order_del",
		data:"id="+id,
		type:"post",
		datatype:"json",
		success:function(data){
...
function go(page){
	$.ajax({
		url:"__APP__/Home/Feature/order",
		...
		success:function(data){
			...
			if(data.status == 1){
				...
				for(var i=0;i<arr.length;i++){
					var dateStr = new Date(parseInt(arr[i].create_date) * 1000);
					...
					var desc = '';
					if(state==-1) desc = '已取消　';
					else if(state==0) desc = '<font color=red>待付款　</font>';
					else if(state==1) desc = '待发货　';
					else if(state==2) desc = '待收货　';
					else if(state==3) desc = '退货　';
					else if(state==4) desc = '已收货　';
					else if(state==5) desc = '退款　';
					else if(state==6) desc = '退款完成　';

					str += '<li><span>'+arr[i].code+'</span><span class="date">'+desc+date+'<a href="javascript:del('+arr[i].id+')" class="del" title="删除">删除</a></span></li>';
				}
				$('.u_i_f_txt').html(str);
...
<div class="user_info_main">
    <div class="u_i_left" id="u_i_left" style="height:628px;">
        <ul class="u_i_menu">
            ...
            <li><a href="../usercenter/order.jsp" class="wddd on">我的订单</a></li>
...
<%
    ...
    String sql = "select * from channel_order  where userid = " + userid;
...
%>	
					<li>
						<span>{$arr.code}</span>
						<span class="date"><?php if($arr['state']==-1)echo '已取消　'; elseif($arr['state']==0)echo '<font color=red>待付款　</font>'; elseif($arr['state']==1)echo '待发货　'; elseif($arr['state']==2)echo '待收货　'; elseif($arr['state']==3)echo '退货　'; elseif($arr['state']==4)echo '已收货　'; elseif($arr['state']==5)echo '退款　';elseif($arr['state']==6)echo '退款完成　';?>{$arr.create_date|date='Y-m-d H:i:s',###}<a href="javascript:del({$arr.id})" class="del" title="删除">删除</a></span>
					</li>
<%}
tu.closeRs(rs);
%>					
