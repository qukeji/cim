...
if(Type.equals("recommend_out_js.jsp"))
{
	...
	response.sendRedirect("../util/recommend_out_js_discuz.jsp?ItemID="+ItemID+"&ChannelID="+ChannelID+"&sourceChannelID="+SChannelID);
	return;
}
...
function Preview()
{
	...
	if(obj.length==0){
		...
		alert("请先选择一个预览的文件！");
		 ...
		 window.open("discuz_preview.jsp?ItemID=" + obj.id + Parameter);
	}
}

function Preview2(id)
{
		 window.open("discuz_preview.jsp?ItemID=" + id + Parameter);
}
...
<div class="viewpane_c1" id="SearchArea" style="display:<%=(S_OpenSearch==1?"":"none")%>">
  ...
  <tr>
    <td width="20">&nbsp;</td>
    <td><form name="search_form" action="discuz_content.jsp?rowsPerPage=<%=rowsPerPage%>" method="post" onSubmit="document.search_form.OpenSearch.value='1';">检索：标题
...
String ListSql = "select tid,subject,author,FROM_UNIXTIME(dateline) as dateline from cdb_threads where displayorder >=0";
String CountSql = "select count(*) from cdb_threads where displayorder >=0";
...
<%if(TotalPageNumber>0){%> 
        <div class="viewpane_pages">
            ...
            <div class="center"><a href="discuz_content.jsp?currPage=1&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="首页"><img src="../images/viewpane_pages1.png" alt="首页" /></a><%if(currPage>1){%><a href="discuz_content.jsp?currPage=<%=currPage-1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="上一页"><img src="../images/viewpane_pages2.png" alt="上一页" />上一页</a><%}%><%if(currPage<TotalPageNumber){%><a href="discuz_content.jsp?currPage=<%=currPage+1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="下一页">下一页<img src="../images/viewpane_pages3.png" alt="下一页" /></a><%}%><a href="discuz_content.jsp?currPage=<%=TotalPageNumber%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="末页"><img src="../images/viewpane_pages4.png" alt="末页" /></a></div>
		 <%}%>
            <div class="right">
