...
*/
	...
	if(status==1)
	{
			...
			tu.closeRs(rs);
			//获取是否允许评论
			String sql1 = "select * from channel_docshowconfig where Active=1 and siteflag="+SiteID;
			TableUtil tu1 = new TableUtil();
			ResultSet rs1 = tu1.executeQuery(sql1);
			/** 获取是否显示评论、是否显示阅读数、是否允许评论、是否显示发布时间、是否显示内容类型 **/
			int showcomment=0;
			int showread=0;
			int allowcomment=0;
			int showtime=0;
			int showtype=0;
			if(rs1.next()){
				showcomment=rs1.getInt("showcomment");
				showread=rs1.getInt("showread");
				allowcomment=rs1.getInt("allowcomment");
				showtime=rs1.getInt("showtime");
				showtype=rs1.getInt("showtype");
			}
			tu1.closeRs(rs1);
...
*/			
			xml.append("</push>");
System.out.println("推送数据："+xml.toString());
