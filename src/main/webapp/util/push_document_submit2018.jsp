...
*/
	...
	if(status==1)
	{
			...
			//获取是否允许评论
			String sql1 = "select allowcomment from channel_s"+document.getChannel().getSite().getId()+"_config where Active=1";
			...
			int RealDoc_Type=0;
			int jumptype=0;
			switch(document.getIntValue("doc_type")){
				case 0:
					RealDoc_Type=0;
					break;
				case 1:
...
//			System.out.println(document.getValue("device_tag").equals(""));
			...
			extra.put("title",document.getTitle());
			/*int   source_gid = document.getIntValue("source_gid");
			if(source_gid != 0){
				extra.put("contentID","null");
			}else{
				extra.put("contentID",'0');
			}*/
			extra.put("contentID",document.getIntValue("source_gid"));
			extra.put("sharepicurl",document.getValue("sharepicurl"));
			extra.put("shareliveurl",document.getValue("shareliveurl"));
			extra.put("contentUrl",document.getValue("source_url"));
			extra.put("doc_type",RealDoc_Type+"");
			extra.put("mediatype",document.getIntValue("mediatype"));
			extra.put("juxian_liveid",document.getIntValue("juxian_liveid"));
			extra.put("allowcomment",allowcomment);
			extra.put("frame",document.getIntValue("frame"));
			extra.put("secondcategory",document.getIntValue("secondcategory"));
			extra.put("jumptype",jumptype);
...
//			System.out.println("推送结果："+result);
		    System.out.println("推送结果："+map.toString());
			JSONObject obj = new JSONObject(result);
			map.put("state",obj.get("state"));
			map.put("xiaoxiID",obj.getString("Summary"));
			map.put("source_gid",document.getIntValue("source_gid")+"");
