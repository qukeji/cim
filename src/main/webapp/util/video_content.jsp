...
function getCheckbox2(){
	...
	jQuery("#oTable input:checked").each(function(i){
		if(i==0){
			id+=jQuery(this).val();
			vid+=jQuery(this).attr("videoname");
			piccount+=jQuery(this).attr("piccount");
		}else{
			id+=","+jQuery(this).val();
			vid+=","+jQuery(this).attr("videoname");
			piccount+=","+jQuery(this).attr("piccount");
	...
	var obj={length:jQuery("#oTable input:checked").length,id:id,vid:vid,piccount:piccount};
	return obj;
}


function showImg(){
var obj=getCheckbox2();
	if(obj.length==0){
		alert("请选择要查看的截图！");
	}else if(obj.length>1){
		alert("请选择一个要查看的截图！");
	}else{
		 window.open("show_club_pic.jsp?vid="+obj.vid+"&piccount="+obj.piccount);
	}
}
...
<div class="viewpane_c1" id="SearchArea" style="display:<%=(S_OpenSearch==1?"":"none")%>">
  ...
  <tr>
    <td width="20">&nbsp;</td>
    <td><form name="search_form" action="video_content.jsp?rowsPerPage=<%=rowsPerPage%>" method="post" onSubmit="document.search_form.OpenSearch.value='1';">检索：标题
    	...
    	<div class="toolbar_l">
            ...
            <ul class="toolbar2">
               <!--<li><a href="#">归档</a></li> -->
				...
				<li><a href="javascript:showImg();">视频截图</a></li>
            </ul>
        </div>
    </div>
  	<div class="viewpane">
...
String ListSql = "SELECT x.itemid as itemid,x.subject as subject,x.uid as uid,x.username as username,FROM_UNIXTIME(x.dateline) as dateline,y.videoname as videoname FROM supe_spaceitems x left join supe_spacevideos y on y.itemid=x.itemid WHERE x.type='video'";

String CountSql = "SELECT count(*) FROM supe_spaceitems x left join supe_spacevideos y on y.itemid=x.itemid WHERE x.type='video'";

String WhereSql = "";

if(!S_Title.equals("")){
	String tempTitle=S_Title.replaceAll("%","\\\\%");
	WhereSql += " and x.subject like '%" + channel.SQLQuote(tempTitle) + "%'";
}

ListSql += WhereSql;
CountSql += WhereSql;

ListSql += " order by x.dateline desc";
...
int j = 0;
Channel channel2 = CmsCache.getChannel(5017);
while(Rs.next())
{
	...
	String videoname=convertNull(Rs.getString("videoname"));
	TableUtil tu2 = new TableUtil();
	String sql2="select * from "+channel2.getTableName()+" where vid='"+videoname+"'";
	String piccount="";
	ResultSet Rs2=tu2.executeQuery(sql2);
	if(Rs2.next()){
		piccount=convertNull(Rs2.getString("piccount"));
	}
	tu2.closeRs(Rs2);
	j++;
%>
  <tr No="<%=j%>" ItemID="<%=id_%>" id="jTip<%=j%>_id">
    <td class="v1" width="25" align="center" valign="middle"><input name="id" value="<%=id_%>"  videoname="<%=videoname%>"  piccount="<%=piccount%>" type="checkbox"/></td>
...
<%if(TotalPageNumber>0){%> 
        <div class="viewpane_pages">
            ...
            <div class="center"><a href="video_content.jsp?currPage=1&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="首页"><img src="../images/viewpane_pages1.png" alt="首页" /></a><%if(currPage>1){%><a href="video_content.jsp?currPage=<%=currPage-1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="上一页"><img src="../images/viewpane_pages2.png" alt="上一页" />上一页</a><%}%><%if(currPage<TotalPageNumber){%><a href="video_content.jsp?currPage=<%=currPage+1%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="下一页">下一页<img src="../images/viewpane_pages3.png" alt="下一页" /></a><%}%><a href="video_content.jsp?currPage=<%=TotalPageNumber%>&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>" title="末页"><img src="../images/viewpane_pages4.png" alt="末页" /></a></div>
		 <%}%>
            <div class="right">
