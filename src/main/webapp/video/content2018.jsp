...
{	
  	var url="document_video.jsp?ItemID="+itemid+"&ChannelID=" + ChannelID;
  	window.open(url);
}
...
{
	//tidecms.dialog("../video/video_player_byvideoid.jsp?id="+id+"&channelid="+ChannelID,860,510,"视频预览");
    
	var url = "../video/video_player_byvideoid2018.jsp?id="+id+"&channelid="+ChannelID;
				var	dialog = new top.TideDialog();
					dialog.setWidth(950);
					dialog.setHeight(580);
					dialog.setUrl(url);
					dialog.setTitle('视频预览');
					//dialog.setMsgJs('ffff');				
...
%>
		<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  status="<%=Status%>"  GlobalID="<%=GlobalID%>" id="item_<%=id_%>" class="tide_item">
            ...
            <td ondragstart="OnDragStart (event)" title="位置：<%=parentChannelPath%>">            
			...
			<%}%>
			<%if(IsClick==1){%>
			<td class="hidden-xs-down"><span ></span></td>
			<%}%>
