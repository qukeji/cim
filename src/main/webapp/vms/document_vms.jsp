...
<%
	 ...
	 *		修改人		日期		备注
	 *
	 *		王海龙		20131121	引入ui.draggable.js
	 *
	 */
...
<title><%=item!=null?"编辑文档 " + item.getTitle():"新建文档"%> <%=channel.getParentChannelPath()%> TideVMS</title>
...
var html = '<embed name="TIDE_PLAYER_0" width="'+width+'" height="'+height+'" src="http://123.125.148.3:228/js/v.swf" wmode="opaque" allowfullscreen="true" allowscriptaccess="always" flashvars="video='+flv+'&skin=0,0,0,0&autoplay=true" type="application/x-shockwave-flash" />';
...
<tr>
	...
	<%}else if(ItemID>0){
		...
		Channel chan = CmsCache.getChannel(id);
		String tableName=chan.getTableName();
		String sqlPhoto = "select Photo from "+tableName+" where Parent="+GlobalID+" and Status=1 limit 0,5";
		ResultSet rs = tu.executeQuery(sqlPhoto);
		//List list = new ArrayList();
		int temp =0;
		while(rs.next()){
			String photo = rs.getString("Photo");
			%>
			<img height="120" width="160" src="<%=photo %>" />
		<%}tu.closeRs(rs);
	}%>
	</div>
	</td>
