...
////id = CmsCache.getParameter("channelid_video_photo_ugc").getIntValue();
...
//System.out.println("getSiteFolder==="+channel.getSite().getSiteFolder());
%>
...
{
	var url = pageName + "?currPage="+currpage+"&id=<%=id%>&rowsPerPage=<%=rowsPerPage%><%=querystring%>&globalid=<%=globalid%>";
	this.location = url;
}
...
{
	//alert("this is video_photo_list.jsp");
var rows = $("#rows").val();
var cols = $("#cols").val();
...
document.location.href = pageName + "?id="+ChannelID+"&rows="+rows+"&cols="+cols+"&globalid=<%=globalid%>";
} 
 
</script>
<style>
.viewpane_pic_list .txt .title,.view_table .v9_button{float:none;margin:5px 0 0 0;}
.view_table .v9_button{margin:0;text-align:center;}
</style>
</head>
...
//String Table = "channel_s3_a_g";//图片库对应的数据库表
...
<%
	...
	if(listType==3)
	{
		...
		m++;
%>
    			<td id="item_<%=id_%>" status="<%=Status%>" class="tide_item_" align="center" valign="top" class="c">
                	<div class="pic">
                    ...
                    <div class="txt">
                        ...
                        <span class="title"><%=Title%><!--(<%=StatusDesc%>)--><div class="v9_button"><img onclick="changePhoto('<%=photoAddr%>');" src="../images/v9_button_9.gif" title="设为封面" /> <img  src="../images/v9_button_11.gif" onclick="window.open('../photo/imageselect.html?imgsrc=<%=photoAddr%>');" title="编辑"/></div></span>
                    </div>
                </td>
<%
