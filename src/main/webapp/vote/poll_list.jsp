...
//int id = CmsCache.getParameter("sys_channelid_poll_option").getIntValue();
//if(id==0) id = 4;//如果没有定义投票选项频道编号，则编号为4
int id = getIntParameter(request,"channel_poll");
...
function addOption()
{
	if(itemid==0)
	{
		$.getJSON("../content/document_getnew.jsp?id="+parent.channelid, function(o){
			...
			tidecms.dialog("../vote/poll_add.jsp?channel_poll=<%=id%>&addOptionitemid="+itemid+"&globalid="+globalid,400,200,"添加投票选项",2);
		}); 
	}
	else
	{
		tidecms.dialog("../vote/poll_add.jsp?channel_poll=<%=id%>&itemid="+itemid+"&globalid="+globalid,400,200,"添加投票选项",2);
	}
}
