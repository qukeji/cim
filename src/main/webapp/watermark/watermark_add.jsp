...
<%
    ...
    if(!Submit.equals(""))
    {
        ...
        new Log().WatermarkLog(LogAction.watermark_add, Name, t.getId(), userinfo_session.getId());
...
<div class="bg-white modal-box">
    <form name="form" action="watermark_add.jsp" method="post" onSubmit="return check();">
                ...
                <ul>
                    <li class="block">
                        <div class="row">
                            ...
                            <label class="wd-230">
                                <input class="form-control" id="Name" placeholder="" type="text" name="Name">
                            </label>
                        </div>
                        <div class="row">
                            <label class="left-fn-title">水印文件：</label>
                            <label class="wd-230">
                                <input class="form-control"  id="filename" placeholder="" type="text" name="filename">
                               <input class="form-control"  id="watermark" placeholder="" type="hidden" name="watermark"> 
                            </label>
                            <input class="btn btn-primary tx-size-xs mg-l-10" href="javascript:;" name="" type="button" value="选择" onclick="selectImage();">
						...
						<div class="row">
                            ...
                            <td valign="middle">
	                 			<input   class="form-control"  type=text style="width: 70px ;height: 30px"  name="width" size="6"><label class="mg-r-10"></label><span>px</span>
	                 		<label class="left-fn-title">水印高度：</label>
                            <td valign="middle">
	                 			<input   class="form-control"  type=text style="width: 70px ;height: 30px"  name="height" size="6"><label class="mg-r-10"></label><span>px</span>
                        </div>
                       <div class="row">
                            <label class="left-fn-title">水平边距：</label>
                            <td valign="middle">
	                 	<input   class="form-control"  type=text style="width: 70px ;height: 30px"  name="LogoLeft" size="6"><label class="mg-r-10"></label><span>px</span>
	                 	<label class="left-fn-title">垂直边距：</label>
	                 	<td valign="middle">
                                <input  class="form-control"  type=text  style="width: 70px ;height: 30px" name="LogoTop" size="6" class="form-control" ><label class="mg-r-10"></label><span>px</span>
                            
                        </div>
                         <div class="row">
                            <label class="left-fn-title">水印透明度：</label>
                            <td valign="middle">
	                 			<input   class="form-control"  type=text style="width: 70px ;height: 30px"  name="dissolve" size="6"><label class="mg-r-10"></label><span>%</span>
                        </div>
                         <div class="row">
                            <label class="left-fn-title">是否应用：</label>
                            <label class="ckbox">
			                <input type="checkbox" name="status" id="status" value="1" ><span class="d-inline-block"></span>
			              </label>
                        </div>
                        <div class="row">
                        	<label class="rdiobox mg-l-50">
				                <input type="radio" name="DataType" id="isAll" value="0" checked="checked"><span>全局使用</span>
				              </label>
                          <label class="rdiobox mg-l-50">
			                <input type="radio" name="DataType" id="notIsAll" value="1" "><span>指定使用</span>
			              </label>
			              <label id="f_1" style="display:none">
							<select id="company" name = "company" class="form-control wd-150 ht-20 select2">
							<%
							...
							JSONArray arr = new Company().getGroup(companyid);
								for(int i=0;i<arr.length();i++)
								{
									...
									%>
							<option value="<%=json.getInt("company")%>" ><%=json.getString("name")%></option>
							<%} %>
						</select>
                           </label> 
                        </div>
