...
id = CmsCache.getParameter("sys_tidehome_config").getJson().getInt("comment");
if(currPage<1)
	currPage = 1;
...
<div class="content_t1">
	<div class="content_t1_nav">当前位置：评论</div>
...
<table width="100%" height="38" border="0" cellpadding="0" cellspacing="0" class="box-tint">
  <tr>
    <td width="20"> </td>
    <td><form name="search_form" action="<%=pageName%>?id=<%=id%>&rowsPerPage=<%=rowsPerPage%>" method="post" onSubmit="document.search_form.OpenSearch.value='1';">检索：文章
...
<div class="content_2012">
 	<div class="toolbar">
    	<div class="toolbar_l">
            ...
            <li><a href="javascript:list('Status1=-1');">待审核</a></li>
...
String ListSql = "select id,Title,item_gid,userid,Content,Status,Active,FROM_UNIXTIME(CreateDate) as CreateDate,userid,url,type from "+Table+"  where  Active= "+IsActive;
...
//if(!S_author.equals("")){
//	String tempauthor=S_author.replaceAll("%","\\\\%");
//	WhereSql += " and userid like '%" +tempauthor+"%'";
//}
...
while(Rs.next())
{
	...
	//String url	= convertNull(Rs.getString("url"));//暂不处理
...
%>
  <tr No="<%=j%>" ItemID="<%=id_%>"  id="item_<%=id_%>" id_="<%=id_%>" class="tide_item">
	...
	<td align="center" valign="middle"><%=from_type%></td>
    <td align="center" valign="middle"><%=CreateDate%></td>
    <td>
		<div class="v9_button" onclick="javascript:window.open('<%=url%>');"><img src="../images/channel_icon/49.png" title="预览" /></div>
		<div class="v9_button" onclick="search(<%=item_gid%>)"><img src="../images/channel_icon/preview2.gif" title="查看所有评论" /></div>
	<%if(IsDelete==1){%><div class="v9_button" onclick="resume(<%=id_%>);"><img src="../images/icon/16.png" title="恢复" /></div><%}else if(Status==0){%>
	<div class="v9_button" onclick="approve2(<%=id_%>);"><img src="../images/v9_button_1.gif" title="审核" /></div>
	<%} %>
