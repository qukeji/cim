...
<%!
    ...
    //问政列表
    public HashMap<String, Object> getPoliticList(int pages,int pagesize,String phone,int status,int type){
        ...
        try {
                ...
                if(phone.length()!=0 ){//手机号不为空，说明查询的是自己的
                    ...
                    tu.closeRs(rs1);
                    map.put("falsephone",falsephone1);
                }else{
					whereSql += " and Status=1 and ispublic=0";
				}
                ...
                tu.closeRs(rs);
                map.put("total",gettotal(phone,type,status));
                map.put("status", 200);
                map.put("message", "成功");
                map.put("result",arr);
...
<%!
    ...
    //问政总数
    public  int gettotal(String phone,int type,int status ) throws MessageException, SQLException{
        int num = 0;
        TideJson politics = CmsCache.getParameter("politics").getJson();//问政接口信息
        int channelid = politics.getInt("politicsid");//问政编号信息
        Channel channel = CmsCache.getChannel(channelid);
        ...
        if(status!=0){
            ...
            String sql = "select * ,count(*) as num from " + channel.getTableName() + whereSql+" order by id desc";
        TableUtil tu = new TableUtil();
...
//	System.out.println("审核列表参数："+type+":"+column_id+":"+data_type+":"+userid);
    HashMap<String, Object> map = getPoliticList( pages, pagesize,phone,status,type);
